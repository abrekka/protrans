alter table customer_order add levert datetime;

alter table transport add levert datetime;

alter table colli add levert datetime;

alter table post_shipment add levert datetime;
...
WHERE     (dbo.Customer_order.Levert IS NOT NULL) AND (COALESCE (dbo.Customer_order.Do_Assembly, 0) = 0) AND 
(dbo.Transport.Transport_name <> 'Historisk') OR
(dbo.Customer_order.Levert IS NOT NULL) AND (dbo.Transport.Transport_name <> 'Historisk') AND (dbo.Customer_order.Do_Assembly = 1) 
AND 
                      EXISTS
                          (SELECT     1 AS Expr1
                            FROM          dbo.[Assembly]
