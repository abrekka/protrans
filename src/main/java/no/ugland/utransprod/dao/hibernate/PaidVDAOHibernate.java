 ...
 * 
 */
public class PaidVDAOHibernate extends BaseDAOHibernate<PaidV> implements PaidVDAO {
	...
	return (List<PaidV>) getHibernateTemplate().execute(new HibernateCallback() {

	    @SuppressWarnings("unchecked")
	    public Object doInHibernate(Session session) throws HibernateException {
		return session.createCriteria(PaidV.class).add(Restrictions.eq("customerNr", customerNr)).list();
	    }
	...
	return (List<PaidV>) getHibernateTemplate().execute(new HibernateCallback() {
	    ...
	    @SuppressWarnings("unchecked")
	    public Object doInHibernate(Session session) throws HibernateException {
		return session.createCriteria(PaidV.class).add(Restrictions.eq("orderNr", orderNr)).list();
	    }
    ...
    public List<PaidV> findByCustomerNrAndProductAreaGroup(final Integer customerNr, final ProductAreaGroup productAreaGroup) {
	return (List<PaidV>) getHibernateTemplate().execute(new HibernateCallback() {

	    @SuppressWarnings("unchecked")
	    public Object doInHibernate(Session session) throws HibernateException {
		Criteria criteria = session.createCriteria(PaidV.class).add(Restrictions.eq("customerNr", customerNr));
    ...
    public List<PaidV> findByOrderNrAndProductAreaGroup(final String orderNr, final ProductAreaGroup productAreaGroup) {
	return (List<PaidV>) getHibernateTemplate().execute(new HibernateCallback() {

	    @SuppressWarnings("unchecked")
	    public Object doInHibernate(Session session) throws HibernateException {
		Criteria criteria = session.createCriteria(PaidV.class).add(Restrictions.eq("orderNr", orderNr));
