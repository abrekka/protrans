 ...
 * 
 */
public class TakstolProductionVDAOHibernate extends BaseDAOHibernate<TakstolProductionV> implements TakstolProductionVDAO {
	...
	return getHibernateTemplate().find("from TakstolProductionV order by orderNr,defaultArticle,numberOfItems desc,orderLineId");
    }
	...
	return (List<Produceable>) getHibernateTemplate().execute(new HibernateCallback() {
		...
		TakstolProductionV.class).add(Restrictions.eq("orderNr", orderNr)).addOrder(Order.asc("orderNr"))
			.addOrder(Order.asc("defaultArticle")).addOrder(Order.desc("numberOfItems")).addOrder(Order.asc("orderLineId")).list();
	    }
	...
	getHibernateTemplate().load(takstolProductionV, takstolProductionV.getOrderLineId());
	...
	return (List<Produceable>) getHibernateTemplate().execute(new HibernateCallback() {
	    ...
	    @SuppressWarnings("unchecked")
	    public Object doInHibernate(Session session) throws HibernateException {
		return session.createCriteria(TakstolProductionV.class).add(Restrictions.eq("customerNr", customerNr)).addOrder(Order.asc("orderNr"))
			.addOrder(Order.asc("defaultArticle")).addOrder(Order.desc("numberOfItems")).addOrder(Order.asc("orderLineId")).list();
	    }
    ...
    public List<Produceable> findByOrderNrAndArticleName(final String orderNr, final String articleName) {
	return (List<Produceable>) getHibernateTemplate().execute(new HibernateCallback() {
		...
		TakstolProductionV.class).add(Restrictions.eq("orderNr", orderNr)).add(Restrictions.eq("articleName", articleName))
			.addOrder(Order.asc("orderNr")).addOrder(Order.asc("defaultArticle")).addOrder(Order.desc("numberOfItems"))
			.addOrder(Order.asc("orderLineId")).list();
	    }
	...
	return (List<Produceable>) getHibernateTemplate().execute(new HibernateCallback() {
	    ...
	    @SuppressWarnings("unchecked")
	    public Object doInHibernate(Session session) throws HibernateException {
		Criteria criteria = session.createCriteria(TakstolProductionV.class).add(Restrictions.eq("customerNr", customerNr));
		...
		criteria.addOrder(Order.asc("orderNr")).addOrder(Order.asc("defaultArticle")).addOrder(Order.desc("numberOfItems"))
			.addOrder(Order.asc("orderLineId"));
	...
	return (List<Produceable>) getHibernateTemplate().execute(new HibernateCallback() {
		...
		TakstolProductionV.class).add(Restrictions.eq("orderNr", orderNr));
		if (productAreaGroup != null && !productAreaGroup.getProductAreaGroupName().equalsIgnoreCase("Alle")) {
		    criteria.add(Restrictions.eq("productAreaGroupName", productAreaGroup.getProductAreaGroupName()));
		}
		criteria.addOrder(Order.asc("orderNr")).addOrder(Order.asc("defaultArticle")).addOrder(Order.desc("numberOfItems"))
