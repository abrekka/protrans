...
public class AssemblyDAOHibernate extends BaseDAOHibernate<Assembly> implements AssemblyDAO {
	...
	public final List<Assembly> findBySupplierYearWeek(final Supplier supplier, final Integer year,
			final Integer week) {
		return (List<Assembly>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session) {
				...
				String query = "select new Assembly(assembly.assemblyId, "
						+ "                         assembly.supplier, " + "                         assembly.order,"
						+ "                         assembly.assemblyYear, "
						+ "                         assembly.assemblyWeek, "
						+ "                         assembly.planned, "
						+ "                         assembly.firstPlanned, "
				...
				assemblies.addAll(session.createCriteria(Assembly.class).add(Restrictions.eq("supplier", supplier))
						...
						.add(Restrictions.or(Restrictions.isNull("inactive"), Restrictions.eq("inactive", 0)))
						.createCriteria("deviation").add(Restrictions.eq("doAssembly", 1))
	...
	public List<Assembly> findByYear(final Integer year) {
		return (List<Assembly>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session) {
				...
				List<Assembly> assemblies = session.createCriteria(Assembly.class)
						.add(Restrictions.eq("assemblyYear", year)).addOrder(Order.desc("assemblyWeek")).list();
	...
	public void oppdaterMontering(final Integer assemblyId, final Date assembliedDate, final String planned) {
		getHibernateTemplate().execute(new HibernateCallback() {

			public Object doInHibernate(final Session session) {

				String sql = "update Assembly a set a.assembliedDate=:assembliedDate,a.planned=:planned where a.assemblyId=:assemblyId";

				session.createQuery(sql).setInteger("assemblyId", assemblyId).setDate("assembliedDate", assembliedDate)
						.setString("planned", planned).executeUpdate();
				return null;
			}
