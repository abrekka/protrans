 ...
 * @author atle.brekka
 */
public class AssemblyDAOHibernate extends BaseDAOHibernate<Assembly> implements
        AssemblyDAO {
    ...
    public final List<Assembly> findBySupplierYearWeek(final Supplier supplier,
            final Integer year, final Integer week) {
        return (List<Assembly>) getHibernateTemplate().execute(
                new HibernateCallback() {
                    ...
                    public Object doInHibernate(final Session session){
                        ...
                        String query = "select new Assembly(assembly.assemblyId, "
                                ...
                                + "where assembly.supplier=:supplier and "
                                + "      assembly.assemblyYear=:year and "
                                + "      assembly.assemblyWeek=:week and "
                                + "      assembly.order.doAssembly=1 and "
                                + "     (assembly.inactive is null or "
                                + "      assembly.inactive=0) ";

                        assemblies = session.createQuery(query).setParameter(
                                "supplier", supplier)
                        ...
                        assemblies
                                .addAll(session
                                        ...
                                        .add(
                                                Restrictions.eq("supplier",
                                                        supplier))
                                        .add(
                                                Restrictions.eq("assemblyYear",
                                                        year))
                                        .add(
                                                Restrictions.eq("assemblyWeek",
                                                        week))
                                        .add(
