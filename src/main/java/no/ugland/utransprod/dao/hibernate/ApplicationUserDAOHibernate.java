...
public class ApplicationUserDAOHibernate extends BaseDAOHibernate<ApplicationUser> implements ApplicationUserDAO {
	...
	return (List<ApplicationUser>) getHibernateTemplate().execute(new HibernateCallback() {
		...
		return session.createCriteria(ApplicationUser.class).add(Restrictions.eq("groupUser", "Nei")).addOrder(Order.asc("lastName")).list();
	    }
	...
	return (List<ApplicationUser>) getHibernateTemplate().execute(new HibernateCallback() {
	    ...
	    public Object doInHibernate(final Session session) {
		Criteria crit = session.createCriteria(ApplicationUser.class).add(Restrictions.ilike("userName", userName));
     ...
     * 
     * public Object doInHibernate(Session session) throws HibernateException {
     * if (!session.contains(applicationUser)) { session.load(applicationUser,
     * applicationUser.getUserId()); }
     * 
     * Set<?> set;
     * 
     * for (LazyLoadApplicationUserEnum lazyEnum : enums) { switch (lazyEnum) {
     * case USER_ROLE: set = applicationUser.getUserRoles(); if (set != null) {
