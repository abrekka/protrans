 ...
 * @author atle.brekka
 */
public class ApplicationUserDAOHibernate extends BaseDAOHibernate<ApplicationUser> implements ApplicationUserDAO {
	/**
	 * Konstruktør
	 */
	...
	public final List<ApplicationUser> findAllNotGroup() {
		return (List<ApplicationUser>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session) {
				return session.createCriteria(ApplicationUser.class).add(Restrictions.eq("groupUser", "Nei"))
	...
	public final List<ApplicationUser> findByUserNameAndPassword(final String userName, final String password) {
		return (List<ApplicationUser>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session) {
				Criteria crit = session.createCriteria(ApplicationUser.class)
	...
	public List<String> findAllPackers() {
		return (List<String>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session) throws HibernateException {
				Criteria criteria = session.createCriteria(ApplicationUser.class)
						.add(Restrictions.eq("groupUser", "Nei"));
	 ...
	 */
	/*
	 * public void lazyLoad(final ApplicationUser applicationUser, final
	 * LazyLoadApplicationUserEnum[] enums) { if (applicationUser != null &&
	 * applicationUser.getUserId() != null) { getHibernateTemplate().execute(new
	 * HibernateCallback() {
	 * 
	 * public Object doInHibernate(Session session) throws HibernateException { if
	 * (!session.contains(applicationUser)) { session.load(applicationUser,
	 * applicationUser.getUserId()); }
	 * 
	 * Set<?> set;
	 * 
	 * for (LazyLoadApplicationUserEnum lazyEnum : enums) { switch (lazyEnum) { case
	 * USER_ROLE: set = applicationUser.getUserRoles(); if (set != null) {
	 * set.iterator(); } break; case USER_PRODUCT_AREA_GROUP: set =
	 * applicationUser.getUserProductAreaGroups(); if (set != null) {
	 * set.iterator(); } break; } } return null; }
	 * 
	 * });
	 * 
	 * }
	...
	private enum ProductAreaGroupSelector {
		...
		private static void setRestrictionForProductAreaGroup(Criteria criteria, ProductAreaGroup productAreaGroup) {
			criteria.createAlias("productArea", "area").add(Restrictions.eq("area.productAreaGroup", productAreaGroup));
	...
	public ApplicationUser findByFullName(final String fullName) {
		return (ApplicationUser) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session) {
				String sql = "select applicationUser " + "       from ApplicationUser applicationUser "
