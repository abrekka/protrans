 ...
 * 
 */
public class PostShipmentCountVDAOHibernate extends
		BaseDAOHibernate<PostShipmentCountV> implements PostShipmentCountVDAO {
	/**
	 * Konstruktør
	 */
	...
	public List<PostShipmentCountSum> findByParams(final ExcelReportSetting params) {
		return (List<PostShipmentCountSum>) getHibernateTemplate().execute(
	...
	@SuppressWarnings("unchecked") List<PostShipmentCountV> getCountPostShipment(final ExcelReportSetting params, Session session) {
		List<PostShipmentCountV> list = session.createCriteria(
				PostShipmentCountV.class).add(
				Restrictions.eq(
						"postShipmentCountVPK.sentYear", params
								.getYear())).add(
				Restrictions.between(
						"postShipmentCountVPK.sentWeek", params
								.getWeekFrom(), params
								.getWeekTo())).add(Restrictions.eq("postShipmentCountVPK.productArea", params.getProductAreaName())).addOrder(
				Order.asc("postShipmentCountVPK.sentWeek"))
				.list();
		return list;
	}
