		...
		try {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				if (field.getType().getCanonicalName().indexOf("java.util.Date") >= 0) {
					Method method = example.getClass().getMethod("get" + StringUtils.capitalize(field.getName()),
							(Class[]) null);
					if (method.invoke(example, (Object[]) null) != null) {
						...
						detachedCriteria.add(Restrictions.sqlRestriction(dateSql,
								method.invoke(example, (Object[]) null), Hibernate.DATE));

						Method setMethod = example.getClass().getMethod("set" + StringUtils.capitalize(field.getName()),
				...
				if (field.getType().getCanonicalName().indexOf("no.ugland.utransprod.model") >= 0) {
					if (!field.getName().equalsIgnoreCase(UNKNOWN)) {
						...
						if (method.invoke(example, (Object[]) null) != null) {
							...
							DetachedCriteria childCriteria = detachedCriteria.createCriteria(field.getName())
