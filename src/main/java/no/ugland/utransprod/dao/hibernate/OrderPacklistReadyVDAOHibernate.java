 ...
 * 
 */
public class OrderPacklistReadyVDAOHibernate extends BaseDAOHibernate<OrderPacklistReadyV> implements OrderPacklistReadyVDAO {
    /**
     * Konstruktør
     */
    ...
    public List<OrderPacklistReadyV> findByParams(final ExcelReportSetting params) {
	return (List<OrderPacklistReadyV>) getHibernateTemplate().execute(new HibernateCallback() {
		...
		return session
			...
			.setParameter("year", params.getYear()).setParameter("weekFrom", params.getWeekFrom())
			.setParameter("weekTo", params.getWeekTo()).setParameter("productArea", params.getProductAreaName()).list();
	    }
