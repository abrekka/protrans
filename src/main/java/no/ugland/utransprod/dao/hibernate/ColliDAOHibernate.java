...
public class ColliDAOHibernate extends BaseDAOHibernate<Colli> implements ColliDAO {
	 ...
	 */
	/*
	 * public void lazyLoadColli(final Colli colli, final LazyLoadEnum[][] enums) {
	 * if (colli != null && colli.getColliId() != null) {
	 * getHibernateTemplate().execute(new HibernateCallback() {
	 * 
	 * public Object doInHibernate(Session session) throws HibernateException { //
	 * if (!session.contains(colli)) { session.load(colli, colli.getColliId()); // }
	 * Set<OrderLine> set = null;
	 * 
	 ...
	 * 
	 * break; case ORDER_LINE_ATTRIBUTES: if (set != null) { Set<OrderLineAttribute>
	 * attributes; for (OrderLine orderLine : set) { attributes = orderLine
	 * .getOrderLineAttributes(); attributes.iterator(); } } break; } } return null;
	 * }
	 * 
	 * });
	 * 
	...
	public void oppdaterTransportId(final Colli colli, final Integer transportId) {
		getHibernateTemplate().execute(new HibernateCallback() {

			public Object doInHibernate(final Session session) {

				String sql = "update Colli c set c.transportId=:transportId where c.colliId=:colliId";

				session.createQuery(sql).setInteger("transportId", transportId).setInteger("colliId", colli.getColliId())
						.executeUpdate();
				return null;
			}
