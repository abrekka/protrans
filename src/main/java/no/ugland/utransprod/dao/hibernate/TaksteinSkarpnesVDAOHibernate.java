 ...
 * 
 */
public class TaksteinSkarpnesVDAOHibernate extends
		BaseDAOHibernate<TaksteinSkarpnesV> implements TaksteinSkarpnesVDAO {
	...
	public List<Produceable> findAll() {
		return getHibernateTemplate()
				.find(
						"from TaksteinSkarpnesV order by transportYear,transportWeek,loadingDate,transportDetails,loadTime");
	}
	...
	public void refresh(Produceable taksteinSkarpnesV) {
		...
		getHibernateTemplate().load(taksteinSkarpnesV,
				taksteinSkarpnesV.getOrderLineId());
	...
	public List<Produceable> findByCustomerNr(final Integer customerNr) {
		return (List<Produceable>) getHibernateTemplate().execute(
				new HibernateCallback() {
					...
					public Object doInHibernate(Session session)
							throws HibernateException {
						return session.createCriteria(TaksteinSkarpnesV.class)
	...
	public List<Produceable> findByCustomerNrAndProductAreaGroup(
		...
		return (List<Produceable>) getHibernateTemplate().execute(
				new HibernateCallback() {
					...
					public Object doInHibernate(Session session)
							throws HibernateException {
						Criteria criteria= session.createCriteria(TaksteinSkarpnesV.class)
								.add(Restrictions.eq("customerNr", customerNr));
