 ...
 * 
 */
public class GulvsponPackageVDAOHibernate extends
		BaseDAOHibernate<GulvsponPackageV> implements GulvsponPackageVDAO {
	...
	public List<PackableListItem> findAll() {
		return getHibernateTemplate()
				.find(
						"from GulvsponPackageV order by transportYear,transportWeek,loadingDate,transportDetails,loadTime");
	}
		...
		return (List<PackableListItem>) getHibernateTemplate().execute(
				new HibernateCallback() {
					...
					public Object doInHibernate(Session session)
						...
						return session.createCriteria(GulvsponPackageV.class)
	...
	public void refresh(GulvsponPackageV gulvsponPackageV) {
		...
		getHibernateTemplate().load(gulvsponPackageV,
				gulvsponPackageV.getOrderLineId());
	...
	@SuppressWarnings("unchecked")
	public List<PackableListItem> findByCustomerNrProductAreaGroup(
		...
		return (List<PackableListItem>) getHibernateTemplate().execute(
				new HibernateCallback() {
					...
					public Object doInHibernate(Session session)
							throws HibernateException {
						Criteria criteria=  session.createCriteria(GulvsponPackageV.class)
								.add(Restrictions.eq("customerNr", customerNr));
