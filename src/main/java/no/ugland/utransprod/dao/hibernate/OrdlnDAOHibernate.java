...
public class OrdlnDAOHibernate extends BaseDAOHibernate<Ordln> implements OrdlnDAO {
	...
	// String[]{"Montering villa","KRANBIL"};
	// private static final Collection<String> costProdnoList=new
	// ArrayList<String>(Arrays.asList(costProdnoArray));
	/**
	 * Konstruktør
	 */
		...
		return (List<Ordln>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session) {
				String sql = "select ordln from Ordln ordln,Ord ord "
						+ "       where ordln.ordlnPK.ordno=ord.ordno and " + "               ord.inf6=:orderNr "
	...
	public final List<Ordln> findForFakturagrunnlag(final String orderNr) {
		return (List<Ordln>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session) {
				String sql = "select ordln from Ordln ordln,Ord ord "
						+ "       where ordln.ordlnPK.ordno=ord.ordno and " + "               ord.inf6=:orderNr "
						+ "       and (ordln.prod.prodNo='OVERSKRIFT' or ordln.free1<>0)"
						+ "       order by ordln.ordlnPK.lnno";
				return session.createQuery(sql).setParameter("orderNr", orderNr).list();
			}
	...
	public Ordln findByOrdNoAndLnNo(final Integer ordNo, final Integer lnNo) {
		return (Ordln) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session) {
				List<Ordln> list = session.createCriteria(Ordln.class).add(Restrictions.eq("ordlnPK.ordno", ordNo))
						.add(Restrictions.eq("ordlnPK.lnno", lnNo)).list();
	...
	public final BigDecimal getSumCstpr(final String orderNr, final Integer prCatNo) {
		return (BigDecimal) getHibernateTemplate().execute(new HibernateCallback() {
			...
			@SuppressWarnings("unchecked")
			public Object doInHibernate(final Session session) {
				String sql = "select sum(ordln.costPrice) " + "        from Ordln ordln,Ord ord,Prod prod"
						+ "        where ordln.ordlnPK.ordno=ord.ordno and " + "              ord.inf6=:orderNr and "
						+ "              prod=ordln.prod and" + "        prod.prCatNo=:prCatNo";
				...
				List<Object> list = session.createQuery(sql).setParameter("orderNr", orderNr)
						.setParameter("prCatNo", prCatNo).list();
	...
	public Ordln findByOrderNrProdCatNo(final String orderNr, final Integer prodCatNo, final Integer prodCatNo2) {
		return (Ordln) getHibernateTemplate().execute(new HibernateCallback() {
			...
			@SuppressWarnings("unchecked")
			public Object doInHibernate(final Session session) {
				StringBuilder sqlBuilder = new StringBuilder("select ordln from Ordln ordln,Ord ord,Prod prod "
						+ "       where ordln.ordlnPK.ordno=ord.ordno and " + "             ord.inf6=:orderNr and "
						+ "             ordln.prod=prod");
	...
	public BigDecimal getSumCcstpr(final String orderNr, final Integer prodCatNo) {
		return (BigDecimal) getHibernateTemplate().execute(new HibernateCallback() {
			...
			@SuppressWarnings("unchecked")
			public Object doInHibernate(final Session session) {
				String sql = "select sum(ordln.ccstPr*ordln.noOrg) " + "        from Ordln ordln,Ord ord,Prod prod"
						+ "        where ordln.ordlnPK.ordno=ord.ordno and " + "              ord.inf6=:orderNr and "
						+ "              prod=ordln.prod and" + "        prod.prCatNo=:prCatNo";

				List<Object> list = session.createQuery(sql).setParameter("orderNr", orderNr)
						.setParameter("prCatNo", prodCatNo).list();
				BigDecimal sum = (BigDecimal) list.get(0);
				if (sum != null) {
					return sum;
				}
	...
	public Integer isOrderLineInStorage(final Integer ordNo, final Integer lnNo) {
		return (Integer) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session) {
				String sql = "select count(stcBal.prodNo) " + "         from Ordln ordln,StcBal stcBal "
						+ "       where ordln.ordlnPK.ordno=:ordno and " + "       ordln.ordlnPK.lnno=:lnno and "
						+ "       ordln.prod.prodNo=stcBal.prodNo and " + "             stcBal.minBal > 0";
				List<Integer> list = session.createQuery(sql).setParameter("ordno", ordNo).setParameter("lnno", lnNo)
						.list();
	...
	public Ordln findByOrdnoAndPrCatNo2(final Integer ordno, final Integer prodCatNo2) {
		return (Ordln) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session) {
				List<Ordln> list = session.createCriteria(Ordln.class).add(Restrictions.eq("ordlnPK.ordno", ordno))
						.createCriteria("prod").add(Restrictions.eq("prCatNo2", prodCatNo2)).list();
				return list.size() == 1 ? list.get(0) : null;
			}
		});
	}
	...
	public List<Ordln> findCostLines(final String orderNr) {
		return (List<Ordln>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(final Session session) {
				String sql = "select ordln from Ordln ordln,Ord ord "
						+ "       where ordln.ordlnPK.ordno=ord.ordno and " + "             ord.inf6=:orderNr and "
						+ "             ordln.prod.prodNo in (:prodnoList) " + "       order by ordln.ordlnPK.lnno";
				return session.createQuery(sql).setParameterList("prodnoList", Ordln.CostLine.getCostProdnoList())
	...
	public List<Ordln> findOrdLnByOrdNo(final Integer ordno) {
		return (List<Ordln>) getHibernateTemplate().execute(new HibernateCallback() {

			public Object doInHibernate(final Session session) {
				return session.createCriteria(Ordln.class).add(Restrictions.eq("ordlnPK.ordno", ordno)).list();
			}
		});
	}
