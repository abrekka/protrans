...
public class OrdlnDAOHibernate extends BaseDAOHibernate<Ordln> implements OrdlnDAO {
	...
	return (List<Ordln>) getHibernateTemplate().execute(new HibernateCallback() {
		...
		String sql = "select ordln from Ordln ordln,Ord ord " + "       where ordln.ordlnPK.ordno=ord.ordno and "
			+ "               ord.inf6=:orderNr " + "       order by ordln.ordlnPK.lnno";
	...
	return (List<Ordln>) getHibernateTemplate().execute(new HibernateCallback() {
	    ...
	    public Object doInHibernate(final Session session) {
		String sql = "select ordln from Ordln ordln,Ord ord " + "       where ordln.ordlnPK.ordno=ord.ordno and "
			+ "               ord.inf6=:orderNr " + "       and (ordln.prod.prodNo='OVERSKRIFT' or ordln.free1<>0)"
	...
	return (Ordln) getHibernateTemplate().execute(new HibernateCallback() {
		...
		StringBuilder sqlBuilder = new StringBuilder("select ordln from Ordln ordln,Ord ord,Prod prod "
			+ "       where ordln.ordlnPK.ordno=ord.ordno and " + "             ord.inf6=:orderNr and " + "             ordln.prod=prod");
	...
	return (Ordln) getHibernateTemplate().execute(new HibernateCallback() {
	    ...
	    public Object doInHibernate(Session session) {
		List<Ordln> list = session.createCriteria(Ordln.class).add(Restrictions.eq("ordlnPK.ordno", ordno)).createCriteria("prod")
			.add(Restrictions.eq("prCatNo2", prodCatNo2)).list();
		return list.size() == 1 ? list.get(0) : null;
	    }
	});
    }
	...
	return (List<Ordln>) getHibernateTemplate().execute(new HibernateCallback() {
	    ...
	    public Object doInHibernate(final Session session) {
		String sql = "select ordln from Ordln ordln,Ord ord " + "       where ordln.ordlnPK.ordno=ord.ordno and "
			+ "             ord.inf6=:orderNr and " + "             ordln.prod.prodNo in (:prodnoList) "
			+ "       order by ordln.ordlnPK.lnno";
		return session.createQuery(sql).setParameterList("prodnoList", Ordln.CostLine.getCostProdnoList()).setParameter("orderNr", orderNr)
			.list();
	    }
