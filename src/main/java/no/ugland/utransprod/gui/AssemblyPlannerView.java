...
//	private boolean useListView = true;
	...
	private void initComponents(final WindowInterface window) {
		...
		orderPanelView = viewHandler.getOrderPanelView().buildPanel(window, "150", viewHandler.getRightClickListener(),
				"130", true);
	...
	public final JComponent buildPanel(final WindowInterface window) {
		...
		builder.add(ButtonBarFactory.buildCenteredBar(buttonExcel, buttonPrint, buttonRefresh, buttonCancel),
				cc.xy(2, 4));
...
//		 PanelBuilder builder = new PanelBuilder(new FormDebugPanel(),layout);
		...
		scrollPaneAssembly.getVerticalScrollBar()
				.addAdjustmentListener(viewHandler.getVerticalAdjustmentListener(teamVerticalScrollBar));
		...
		for (Supplier supplier : suppliers) {
			...
			for (int i = weekStart; i <= weekStop; i++) {
				...
				if (yearWeek.getWeek() == 1 && i == 0) {
					...
					builder.appendRow(new RowSpec("fill:p"));
					builder.append(
							viewHandler.getAssemblyTeamOrderView(supplier, currentYearWeek, weekCounter, starting)
									.buildPanel(window, supplierRowSizes.get(supplier)));
				}
			}
		}
