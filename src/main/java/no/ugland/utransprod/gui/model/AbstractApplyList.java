 ...
 * @param <T>
 */
public abstract class AbstractApplyList<T extends Applyable> implements
		ApplyListInterface<T> {
	 ...
	 * @param manager
	 */
	public AbstractApplyList(Login aLogin, IApplyListManager<T> manager,
	...
	public Collection<T> getObjectLines() {
		...
		if (lines != null) {
			for (T obj : lines) {
				obj.setOrdln(applyListManager.findOrdlnByOrderLine(obj
						.getOrderLineId()));
			}
		}
	 ...
	 *      java.lang.String)
	 */
	public List<T> doSearch(String orderNr, String customerNr,
			ProductAreaGroup productAreaGroup) throws ProTransException {
		if (orderNr != null) {
			return applyListManager.findApplyableByOrderNrAndProductAreaGroup(
	...
	public void setProductionUnit(ProductionUnit productionUnit, T object,
			WindowInterface window) {
	}

	protected String createVismaFile(List<OrderLine> orderLines)
			throws ProTransException {
		return vismaFileCreator != null ? vismaFileCreator
				.createVismaFile(orderLines) : null;

	}

	protected boolean ignoreVismaFile(OrderLine orderLine,
			WindowInterface window, boolean isMainArticle) {
		return !isMainArticle ? true
				: vismaFileCreator != null ? vismaFileCreator.ignoreVismaFile(
						orderLine, window) : true;
	}
