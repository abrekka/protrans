...
/*     */ public class TakstolProductionApplyList extends ProductionApplyList {
	...
	/*     */
	/*     */ @Inject
	/*     */ public TakstolProductionApplyList(@Named("takstolColliName") String colliName, Login login,
			@Named("takstol_article") String aMainArticleName, ManagerRepository managerRepository,
			ArticlePackageViewFactory aArticlePackageViewFactory) {
		/* 34 */ super(login, managerRepository.getTakstolProductionVManager(), colliName, "Takstol", (Integer[]) null,
				managerRepository, (VismaFileCreator) null, Arrays.asList("Takstoler"));
		/*     */
		/* 36 */ this.mainArticleName = aMainArticleName;
		/* 37 */ this.articlePackageView = aArticlePackageViewFactory
				.create(managerRepository.getArticleTypeManager().findByName("Takstoler"), this, colliName);
		/*     */
		/*     */
		/* 40 */ }
	...
	/*     */ public void settStartetKapping(Produceable produceable, Date startetKappingDato) {
		/* 61 */ OrderLine orderLine = this.managerRepository.getOrderLineManager()
	...
	/*     */ public void settKappingFerdig(Produceable produceable, Date kappingFerdigDato) {
		/* 72 */ OrderLine orderLine = this.managerRepository.getOrderLineManager()
				.findByOrderLineId(produceable.getOrderLineId());
		/*     */
	...
	/*     */ private void setStartedDate(List<Applyable> objects, Date startedDate) {
		/* 82 */ Iterator var3 = objects.iterator();
		while (var3.hasNext()) {
			Applyable object = (Applyable) var3.next();
			/* 83 */ OrderLine orderLine = this.managerRepository.getOrderLineManager()
		...
		/*     */ try {
			/* 98 */ if (object != null) {
				...
				/* 105 */ if (relatedArticles != null && relatedArticles.size() != 0) {
					...
					/* 107 */ Iterator var6 = objects.iterator();
					while (var6.hasNext()) {
						Applyable item = (Applyable) var6.next();
	...
	/*     */ private void applyAllObjects(boolean applied, WindowInterface window, List<Applyable> objects) {
		...
		/* 133 */ Iterator var5 = objects.iterator();
		while (var5.hasNext()) {
			Applyable item = (Applyable) var5.next();
	...
	/*     */ private void applyObject(Produceable object, boolean applied, WindowInterface window) {
		...
		/*     */
		/* 165 */ OrderLine orderLine = this.managerRepository.getOrderLineManager()
				.findByOrderLineId(object.getOrderLineId());
		/*     */
	...
	/*     */ public Produceable getApplyObject(Transportable transportable, WindowInterface window) {
		/* 183 */ List<Produceable> list = ((TakstolProductionVManager) this.applyListManager)
				.findApplyableByOrderNrAndArticleName(transportable.getOrder().getOrderNr(), this.mainArticleName);
		/*     */
		/*     */
		/*     */
		/* 187 */ return list != null && list.size() != 0
				? (list != null && list.size() == 1 ? (Produceable) list.get(0) : this.selectApplyObject(list, window))
				: null;
		/*     */ }
