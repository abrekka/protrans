...
/*     */ 
/*     */    public PackageApplyList(Login login, IApplyListManager<PackableListItem> manager, String aColliName, String aWindowAccessName, ReportEnum aReportEnum, VismaFileCreator vismaFileCreator, ManagerRepository aManagerRepository) {
...
/*     */ 
/*     */ 
/*     */    protected OrderLine handleApply(boolean applied, WindowInterface window, OrderLineManager orderLineManager, OrderLine orderLine, String aColliName, boolean isMainArticle) {
/*  83 */       OrderLine appliedOrderLine = null;
/*     */       try {
/*  85 */          if (applied) {
/*  86 */             appliedOrderLine = orderLine.getColli() != null ? null : this.setObjectApplied(orderLineManager, orderLine, aColliName);
/*     */ 
/*     */ 
/*     */ 
...
/*     */ 
/*     */ 
/*     */    private void setObjectUnapplied(OrderLineManager orderLineManager, OrderLine orderLine) throws ProTransException {
...
/*     */ 
/*     */ 
/*     */    private OrderLine setObjectApplied(OrderLineManager orderLineManager, OrderLine orderLine, String aColliName) throws ProTransException {
/* 111 */       ColliManager colliManager = (ColliManager)ModelUtil.getBean("colliManager");
/*     */ 
/* 113 */       String currentColliName = aColliName != null ? aColliName : this.colliName;      Colli colli;
...
/* 121 */             colli = new Colli((Integer)null, (Order)null, currentColliName, (Transport)null, (Date)null, (Set)null, orderLine.getPostShipment(), (Integer)null, Util.getCurrentDate(), "PackageApplyList");
/*     */ 
/*     */          } else {
/* 124 */             colli = new Colli((Integer)null, orderLine.getOrder(), currentColliName, (Transport)null, (Date)null, (Set)null, (PostShipment)null, (Integer)null, Util.getCurrentDate(), "PackageApplyList");
/*     */ 
/*     */ 
/*     */          }
/* 128 */       } else if (!Hibernate.isInitialized(colli.getOrderLines())) {
/* 129 */          colliManager.lazyLoad(colli, new LazyLoadEnum[][]{{LazyLoadEnum.ORDER_LINES, LazyLoadEnum.NONE}});
/*     */       }
/*     */ 
...
/* 161 */          PostShipmentManager postShipmentManager = (PostShipmentManager)ModelUtil.getBean("postShipmentManager");
...
/* 180 */                   orderManager.lazyLoadOrder(order, new LazyLoadOrderEnum[]{LazyLoadOrderEnum.ORDER_LINES});
/*     */                }
/*     */ 
...
/* 196 */                PostShipmentManager postShipmentManager = (PostShipmentManager)ModelUtil.getBean("postShipmentManager");
/*     */ 
/* 198 */                if (applied) {
/* 199 */                   postShipmentManager.lazyLoad(postShipment, new LazyLoadPostShipmentEnum[]{LazyLoadPostShipmentEnum.ORDER_LINES});
/*     */ 
...
/* 233 */       List<PackableListItem> list = this.applyListManager.findApplyableByOrderNr(transportable.getOrder().getOrderNr());
...
/* 258 */       Iterator var3 = objects.iterator();
/*     */       while(var3.hasNext()) {         Applyable object = (Applyable)var3.next();
/* 260 */          OrderLine orderLine = this.managerRepository.getOrderLineManager().findByOrderLineId(object.getOrderLineId());
/* 261 */          if (orderLine != null) {
/* 262 */             orderLine.setActionStarted(startedDate);
/*     */ 
/* 264 */             this.managerRepository.getOrderLineManager().saveOrderLine(orderLine);
/* 265 */             this.applyListManager.refresh((PackableListItem)object);
/*     */          }      }
/*     */ 
/* 268 */    }
/*     */ 
