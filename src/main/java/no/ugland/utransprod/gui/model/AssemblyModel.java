...
public class AssemblyModel extends AbstractModel<Assembly, AssemblyModel> {
	...
	public AssemblyModel(Assembly object, String aUserName) {
		...
		if (object.getOrder() != null
				&& object.getOrder().getAssemblyComments() != null) {
	...
	public void setAssembliedDate(Date assembliedDate) {
		...
		firePropertyChange(PROPERTY_ASSEMBLIED_DATE, oldAssemblied,
				assembliedDate);
	}
	...
	public void setCommentList(List<OrderComment> comments) {
		List<OrderComment> oldComments = new ArrayList<OrderComment>(
				getCommentList());
	...
	public void addBufferChangeListener(PropertyChangeListener listener,
		...
		presentationModel.getBufferedModel(PROPERTY_ASSEMBLIED_DATE)
		...
		presentationModel.getBufferedModel(PROPERTY_ASSEMBLY_YEAR)
				.addValueChangeListener(listener);
		presentationModel.getBufferedModel(PROPERTY_COMMENT_LIST)
				.addValueChangeListener(listener);
		presentationModel.getBufferedModel(PROPERTY_PLANNED)
				.addValueChangeListener(listener);
	...
	@Override
	public AssemblyModel getBufferedObjectModel(
			PresentationModel presentationModel) {
		AssemblyModel assemblyModel = new AssemblyModel(new Assembly(),
				userName);
		assemblyModel.setSupplier((Supplier) presentationModel
				.getBufferedValue(PROPERTY_SUPPLIER));
		assemblyModel.setAssemblyWeek((Integer) presentationModel
				.getBufferedValue(PROPERTY_ASSEMBLY_WEEK));
		assemblyModel.setAssemblyYear((Integer) presentationModel
				.getBufferedValue(PROPERTY_ASSEMBLY_YEAR));
		assemblyModel.setCommentList((List<OrderComment>) presentationModel
				.getBufferedValue(PROPERTY_COMMENT_LIST));
		assemblyModel.setPlanned((String) presentationModel
				.getBufferedValue(PROPERTY_PLANNED));
		assemblyModel.setAssembliedDate((Date) presentationModel
				.getBufferedValue(PROPERTY_ASSEMBLIED_DATE));
