...
public class OrderModel extends AbstractOrderModel<Order, OrderModel> {
	 ...
	 * @param statusChange
	 */
	public OrderModel(final Order order, final boolean search, final boolean changeInfo, final boolean statusChange,
			final String aProjectNr, final String aProjectName, boolean lettvekt) {
		super(order, lettvekt);
	...
	/**
	 * Henter artikler som ligger på ordre
	 * 
	 * @return artikler
	 */
	...
	/**
	 * @return dato for når takstol er pakket
	 */
	...
	public void viewToModel() {
		...
		if (!lettvekt && orderLineList != null) {
	...
	public static Set<OrderLine> cloneOrderLines(Set<OrderLine> orderLines) {
		...
		if (orderLines != null) {
			...
			for (OrderLine orderLine : orderLines) {
				...
				clonedLines.add(new OrderLine(orderLine.getOrderLineId(), orderLine.getOrder(),

						orderLine.getConstructionTypeArticle(), orderLine.getArticleType(),
						cloneOrderLineAttributes(orderLine.getOrderLineAttributes()), orderLine.getOrderLineRef(),
						cloneOrderLines(// order,
								orderLine.getOrderLines()),
						orderLine.getNumberOfItems(), orderLine.getDialogOrder(), orderLine.getProduced(),
						orderLine.getArticlePath(), orderLine.getColli(), orderLine.getHasArticle(),
						orderLine.getAttributeInfo(), orderLine.getIsDefault(), orderLine.getPostShipment(),
						orderLine.getExternalOrderLine(), orderLine.getDeviation(), orderLine.getActionStarted(),
						orderLine.getProductionUnit(), orderLine.getOrdNo(), orderLine.getLnNo()));
	...
	public static Set<OrderLineAttribute> cloneOrderLineAttributes(Set<OrderLineAttribute> attributes) {
		...
		if (attributes != null) {
			for (OrderLineAttribute attribute : attributes) {
				clonedAttributes.add(new OrderLineAttribute(attribute.getOrderLineAttributeId(),
						attribute.getOrderLine(), attribute.getConstructionTypeArticleAttribute(),
						attribute.getConstructionTypeAttribute(), attribute.getArticleTypeAttribute(),
						attribute.getAttributeValue(), attribute.getDialogOrder(), attribute.getAttributeName()));
			}
		}
	...
	public static OrderLine cloneOrderLine(Order order, OrderLine orderLine) {
		return new OrderLine(orderLine.getOrderLineId(), order, orderLine.getConstructionTypeArticle(),
				orderLine.getArticleType(), orderLine.getOrderLineAttributes(), orderLine.getOrderLineRef(),
				orderLine.getOrderLines(), orderLine.getNumberOfItems(), orderLine.getDialogOrder(),
				orderLine.getProduced(), orderLine.getArticlePath(), orderLine.getColli(), orderLine.getHasArticle(),
				orderLine.getAttributeInfo(), orderLine.getIsDefault(), orderLine.getPostShipment(),
				orderLine.getExternalOrderLine(), orderLine.getDeviation(), orderLine.getActionStarted(),
				orderLine.getProductionUnit(), orderLine.getOrdNo(), orderLine.getLnNo());
	}
	...
	public boolean canChangeStatus() {
		return !lettvekt&&canChangeStatus;
	}
	 ...
	 */
	public static List<OrderModel> convertOrderSetToList(List<Order> orders, boolean lettvekt) {
		List<OrderModel> orderModelList = new ArrayList<OrderModel>();
		if (orders != null) {
			for (Order order : orders) {
				orderModelList.add(new OrderModel(order, false, false, false, null, null, lettvekt));
			}
		}
	...
	/**
	 * @return dato for forhåndsbetaling
	 */
	...
	/**
	 * @return dato for når order er klar
	 */
	...
	public OrderModel getBufferedObjectModel(PresentationModel presentationModel) {
		OrderModel orderModel = new OrderModel(new Order(), false, canChangeInfo, canChangeStatus, projectNr,
				projectName, lettvekt);
		...
		orderModel
				.setConstructionType((ConstructionType) presentationModel.getBufferedValue(PROPERTY_CONSTRUCTION_TYPE));
		...
		orderModel.setOrderLineArrayListModel(
				(ArrayListModel) presentationModel.getBufferedValue(PROPERTY_ORDER_LINE_ARRAY_LIST_MODEL));
		...
		orderModel.setOrderReadyTakstolBool(
				(Boolean) presentationModel.getBufferedValue(PROPERTY_ORDER_READY_TAKSTOL_BOOL));
		...
		orderModel.setGarageColliHeight((Integer) presentationModel.getBufferedValue(PROPERTY_GARAGE_COLLI_HEIGHT));
		orderModel.setRegistrationDateString(
				(String) presentationModel.getBufferedValue(PROPERTY_REGISTRATION_DATE_STRING));
		...
		orderModel.setDefaultColliesGenerated(
				(Integer) presentationModel.getBufferedValue(PROPERTY_DEFAULT_COLLIES_GENERATED));
		orderModel.setMaxTrossHeight((String) presentationModel.getBufferedValue(PROPERTY_MAX_TROSS_HEIGHT));
		orderModel.setProductAreaGroup(
				(ProductAreaGroup) presentationModel.getBufferedValue(PROPERTY_PRODUCT_AREA_GROUP));
		return orderModel;
	}

	/**
	 * Legger lytter til å lytte på bufferendringer
	 * 
	 * @param listener
	 * @param presentationModel
	 */
	...
	public static Set<OrderLineAttribute> getOrderLinesAttributes(final Set<ArticleTypeAttribute> attributes,
		...
		if (attributes != null) {
			...
			for (ArticleTypeAttribute articleTypeAttribute : attributes) {
				orderLineAttributes.add(new OrderLineAttribute(null, orderLine, null, null, articleTypeAttribute, "",
	...
	public Enum[] getEnums() {
		return new LazyLoadOrderEnum[] { LazyLoadOrderEnum.COLLIES, LazyLoadOrderEnum.ORDER_LINES,
