...
public class OrderPanelView {
	 ...
	 * @param label
	 */
	public OrderPanelView(final OrderViewHandler handler, final OrderPanelTypeEnum orderPanelType, final String label,boolean lettvekt) {
		viewHandler = handler;
	...
	private void initComponents(final WindowInterface window) {
		orderSelectionList = viewHandler.initAndGetOrderPanelSelectionList(panelType);
	 ...
	 * @param window
	 */
	private void initEventHandling(final MouseListener rightMouseClickHandler, final WindowInterface window) {
		orderSelectionList.addPropertyChangeListener(SelectionInList.PROPERTYNAME_SELECTION_EMPTY,
				new SelectionEmptyHandler());
//		tableOrders.addMouseListener(viewHandler.getNewOrderDoubleClickHandler(window));
		viewHandler.getNewOrderDoubleClickHandler(window,tableOrders,lettvekt);
		if (rightMouseClickHandler != null) {
			tableOrders.addMouseListener(rightMouseClickHandler);
		}
	}

	public final Component buildPanel(final WindowInterface window, final String tableHeight,
			final MouseListener rightMouseClickHandler, final String tableWidth, final boolean showEditButton) {
		initComponents(window);
		initEventHandling(rightMouseClickHandler, window);
		FormLayout layout = new FormLayout(tableWidth + "dlu:grow", "p,3dlu,fill:" + tableHeight + ":grow,3dlu,p");
		...
		if (showEditButton) {
			builder.add(ButtonBarFactory.buildCenteredBar(buttonEditOrder), cc.xy(1, 5));
		}
