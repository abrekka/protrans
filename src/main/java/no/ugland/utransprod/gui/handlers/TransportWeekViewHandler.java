...
public class TransportWeekViewHandler implements Updateable, TransportSelectionListener, ProductAreaGroupProvider {
	...
	@Inject
	public TransportWeekViewHandler(final Login aLogin, final ManagerRepository aManagerRepository,
			TransportViewHandler aTransportViewHandler, OrderViewHandlerFactory aOrderViewHandlerFactory,
			final DeviationViewHandlerFactory aDeviationViewHandlerFactory,
			ShowTakstolInfoActionFactory aShowTakstolInfoActionFactory, @Assisted YearWeek aRouteDate,
		...
		if (index != -1) {
			...
			if (transportable instanceof Order) {
				...
				managerRepository.getOrderManager().lazyLoadOrder(order, new LazyLoadOrderEnum[] {
						LazyLoadOrderEnum.POST_SHIPMENTS, LazyLoadOrderEnum.COMMENTS, LazyLoadOrderEnum.COLLIES });
			} else {
			...
			if (transportable instanceof Order) {
				...
				managerRepository.getOrderManager().lazyLoadOrder(order, new LazyLoadOrderEnum[] {
						LazyLoadOrderEnum.COLLIES, LazyLoadOrderEnum.ORDER_LINES, LazyLoadOrderEnum.ORDER_COSTS });
	...
	private Transportable getSelectedTransport(int selectedIndex) {
		Transportable transportable = (Transportable) transportableSelectionList
				.getElementAt(tableOrdersList.convertRowIndexToModel(selectedIndex));
	...
	void openEditView(Transport transport) {
		...
		if (transport.getTransportYear() != null && transport.getTransportYear().equals(routeDate.getYear())
				&& transport.getTransportWeek() != null && transport.getTransportWeek().equals(routeDate.getWeek())) {
			fireTransportAdded();
		}
	...
	public TransportView getTransportView(Transport transport) {
		TransportViewHandler transportViewHandlerTmp = new TransportViewHandler(orderViewHandlerFactory, login,
	...
	public void setFilterSent(boolean sentFilter) {
		PrefsUtil.setInvisibleColumns(ProductAreaGroup.UNKNOWN.getProductAreaGroupName(),
				TableEnum.TABLETRANSPORTORDERSLIST.getTableName(), tableOrdersList);
	...
	public JXTable getTableOrders(WindowInterface aWindow) {
		...
		TableModel transportOrderTableModel = new TransportOrderTableModelList(transportableSelectionList,
				transportableList, Util.getGavlChecker(), Util.getTakstolChecker(managerRepository),
				Util.getSteinChecker(), Util.getGulvsponChecker(),
		...
		tableOrdersList.setSelectionModel(
		...
		List<TransportOrderTableModelList.TransportColumn> columns = TransportOrderTableModelList.TransportColumn
				.getTableColumns();
	...
	private Date showSentOrders(final WindowInterface aWindow, final Transportable transportable,
	...
	private Date handleTransportSending(final WindowInterface window1, final Transportable transportable,
			final boolean sentTransport) throws ProTransException {
	...
	private Date handleTransportNotSent(final Transportable transportable, final boolean saveTransport)
	...
	private void saveTransportable(PostShipmentManager postShipmentManager, Transportable transportable)
			throws ProTransException {
		if (transportable instanceof Order) {
	...
	final boolean setSent(final Boolean sent, final WindowInterface window, Transportable transportable,
