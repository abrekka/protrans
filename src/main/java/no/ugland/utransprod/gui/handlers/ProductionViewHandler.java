 ...
 * @author atle.brekka
 */
public class ProductionViewHandler extends
		AbstractProductionPackageViewHandler<Produceable> {
	...
	public ProductionViewHandler(
			final ApplyListInterface<Produceable> productionInterface,
			final String title, final Login login, final String aApplyText,
			final String aStartText, final TableEnum tableEnum,
		...
		super(login, managerRepository, deviationViewHandlerFactory,
				productionInterface, title, tableEnum);
	 ...
	 */
	@Override
	protected void searchOrder(final String orderNr, final String customerNr,
			final WindowInterface window) {
		try {
			List<Produceable> produceables = applyListInterface
					.doSearch(
							orderNr,
							customerNr,
	...
	final class ProductionTableModel extends AbstractTableAdapter {
		...
		public ProductionTableModel(final ListModel listModel) {
			super(listModel, new String[] { "Transport", "Ordre", "Prod.dato",
					"Antall", "Spesifikasjon", "Opplasting",
					StringUtils.capitalize(applyText), "Produktområde",
					"Prod.enhet" });
	 ...
	 */
	@Override
	protected void setApplied(final Produceable object, final boolean applied,
			final WindowInterface window) {
	...
	protected TableModel getTableModelReport() {
		return applyListInterface.getTableModelReport(getObjectSelectionList(),
				table, objectSelectionList);
	}
	...
	public Produceable getApplyObject(final Transportable transportable,
			final WindowInterface window) {
		produceable = produceable != null ? produceable : applyListInterface
				.getApplyObject(transportable, window);
		return produceable;
	}
	...
	protected boolean getButtonStartEnabled(Produceable produceable) {
		if (produceable != null
				&& produceable.getActionStarted() == null
				&& (produceable.getProbability() == null || produceable
						.getProbability() != 90)) {
			return true;
		}
		return false;
