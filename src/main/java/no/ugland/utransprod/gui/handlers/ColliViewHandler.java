...
public class ColliViewHandler extends AbstractViewHandlerShort<Colli, ColliModel> implements ColliListener {
	 ...
	 * @param userType
	 */
	public ColliViewHandler(String aHeading, Colli aColli, Packable aPackable, Login login,
			ManagerRepository aManagerRepository, WindowInterface currentWindow, VismaFileCreator aVismaFileCreator) {
	...
	public void initColli(Colli aColli) {
		...
		orderLineList = new ArrayListModel(
				(List<OrderLine>) presentationModel.getValue(ColliModel.PROPERTY_ORDER_LINES));
	...
	public JTextField getTextFieldColliName(PresentationModel aPresentationModel) {
		JTextField textField = BasicComponentFactory
				.createTextField(aPresentationModel.getBufferedModel(ColliModel.PROPERTY_COLLI_NAME), false);
...
//				managerRepository.getOrderLineManager().saveOrder(order);
				managerRepository.getOrderManager().settOrdreKomplett(order.getOrderNr(),Util.getCurrentDate());
			}
		} else {
	...
	public void initEventHandling() {
		...
		((ColliModel) presentationModel.getBean()).addBufferChangeListener(new ColliBufferingListener(),
				presentationModel);
	}
	...
	void fireBufferChange() {
		for (PropertyChangeListener listener : bufferingListeners) {
			listener.propertyChange(new PropertyChangeEvent(presentationModel.getBean(),
					String.valueOf(presentationModel.isBuffering()), presentationModel.getBean(),
...
//					managerRepository.getOrderLineManager().saveOrderLine(orderLine);
					...
					if (orderLine.getOrdNo() != null) {
						orderLine.setOrdln(managerRepository.getOrderLineManager()
...
//							managerRepository.getOrderLineManager().saveOrder(order);
							managerRepository.getOrderManager().fjernOrdreKomplett(order.getOrderNr());
				...
				Util.showMsgDialog(window.getComponent(), "Kan ikke fjerne artikkel",
	 ...
	 */
	@Override
	protected AbstractEditView<ColliModel, Colli> getEditView(AbstractViewHandler<Colli, ColliModel> handler,
			Colli object, boolean searching) {
	...
	public JTextField getTextFieldNumberOfCollies(PresentationModel aPresentationModel) {
		JTextField textField = BasicComponentFactory
				.createTextField(aPresentationModel.getBufferedModel(ColliModel.PROPERTY_NUMBER_OF_COLLIES), false);
	...
	private void refreshColli() {
		...
		managerRepository.getColliManager().lazyLoad(currentColli,
				new LazyLoadEnum[][] { { LazyLoadEnum.ORDER_LINES, LazyLoadEnum.NONE } });
	...
	public JTextField getTextFieldHeight(PresentationModel aPresentationModel) {
		JTextField textField = BasicComponentFactory
				.createTextField(aPresentationModel.getBufferedModel(ColliModel.PROPERTY_HEIGHT), false);
	...
	public JTextField getTextFieldLenght(PresentationModel aPresentationModel) {
		JTextField textField = BasicComponentFactory
				.createTextField(aPresentationModel.getBufferedModel(ColliModel.PROPERTY_LENGHT), false);
	...
	public JTextField getTextFieldWidht(PresentationModel aPresentationModel) {
		JTextField textField = BasicComponentFactory
				.createTextField(aPresentationModel.getBufferedModel(ColliModel.PROPERTY_WIDHT), false);
