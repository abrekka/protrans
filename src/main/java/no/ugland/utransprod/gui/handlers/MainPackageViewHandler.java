...
public class MainPackageViewHandler implements Closeable, Updateable, ListDataListener, ColliListener {
	 ...
	 */
	@Inject
	public MainPackageViewHandler(VismaFileCreator aVismaFileCreator, OrderViewHandlerFactory orderViewHandlerFactory,
			Login aLogin, ManagerRepository aManagerRepository,
			DeviationViewHandlerFactory aDeviationViewHandlerFactory, @Assisted Multimap<String, String> aColliSetup,
	...
	private void init() {
		...
		productionBudget = managerRepository.getBudgetManager().findByYearAndWeekPrProductAreaGroup(currentYear,
				currentWeek, BudgetType.PRODUCTION);
		...
		if (postShipments != null) {
			for (PostShipment postShipment : postShipments) {
				managerRepository.getOrderManager().lazyLoadOrder(postShipment.getOrder(),
	...
	public JXTable getTableOrders(WindowInterface window) {
		...
		ColorHighlighter readyPattern = new ColorHighlighter(new PatternPredicate("1", 3), Color.WHITE,
				ColorEnum.GREEN.getColor());
		ColorHighlighter probabilityPattern = new ColorHighlighter(new PatternPredicate("90", 6),
				ColorEnum.GREY.getColor(), Color.LIGHT_GRAY);
	...
	public JXTable getTablePostShipment(WindowInterface window) {
		...
		tablePostShipment
				.setSelectionModel(new SingleListSelectionAdapter(postShipmentSelectionList.getSelectionIndexHolder()));
	...
	public JXTable getTableOrderLines(WindowInterface window) {
		...
		ColorHighlighter patternStart = new ColorHighlighter(new PatternPredicate("[^---]", 7),
	...
	private void setTableOrderLinesFilters() {
		...
		List<PatternFilter> filters = Lists.newArrayList(new PatternFilter("Ja", Pattern.CASE_INSENSITIVE, 5),
	...
	private class PackageOrderTableModel extends AbstractTableAdapter {
		...
		public PackageOrderTableModel(ListModel listModel) {
			super(listModel, new String[] { "Ordre", "Transport", "Pakket", "Klar", "Opplasting", "Produktområde",
					"Sannsynlighet", "Prod.uke" });
		}
	...
	public void initEventHandling(MainPackageView aMainPackageView, WindowInterface window) {
		...
		orderSelectionList.addPropertyChangeListener(SelectionInList.PROPERTYNAME_SELECTION_INDEX,
				new OrderSelectionListener(window));
		postShipmentSelectionList.addPropertyChangeListener(SelectionInList.PROPERTYNAME_SELECTION_INDEX,
	...
	void refreshTableOrder(OrderLine updatedOrderLine, boolean removeColli, WindowInterface window, boolean combo) {
		...
		if (orderSelectionList.hasSelection()) {
			...
			MainPackageV mainPackageV = (MainPackageV) orderSelectionList
					.getElementAt(tableOrders.convertRowIndexToModel(orderSelectionList.getSelectionIndex()));
	...
	private Comparator<OrderLine> etterProdtp2OgTrlnf3() {
		return new Comparator<OrderLine>() {
			...
			private String hentTrInf3(OrderLine o1) {
				return o1 == null || o1.getOrdln() == null || o1.getOrdln().getTrInf3() == null ? ""
						: o1.getOrdln().getTrInf3();
			}

			private Integer hentProdTp2(OrderLine o1) {
				return o1 == null || o1.getOrdln() == null || o1.getOrdln().getProdTp2() == null ? 0
						: o1.getOrdln().getProdTp2();
			}
		};
	}
	...
	public class PackageOrderLineTableModel extends AbstractTableAdapter {
		 ...
		 * @param aWindow
		 */
		public PackageOrderLineTableModel(ListModel listModel, OrderLineManager aOrderLineManager,
				WindowInterface aWindow) {
			super(listModel, new String[] { "Artikkel", "#", "Klar", "Spesifikasjon", "Kolli", "Top level",
					"Har artikkel", "Startet", "Produkttype" });
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			...
			if (orderLineSelectionList.getSize() != 0) {
				...
				if (orderLine.getHasArticle() == null || orderLine.getAttributeInfo() == null
				...
				case 8:
					if (orderLine.getOrdln() != null && orderLine.getOrdln().getProdTp() == 35) {
						...
						if ("Takstoler".equalsIgnoreCase(orderLine.getArticlePath())
								|| "Gavl".equalsIgnoreCase(orderLine.getArticlePath())) {
	...
	void setSums() {
		...
		SumOrderReadyV sum = managerRepository.getSumOrderReadyVManager()
				.findByDateAndProductAreaGroupName(Calendar.getInstance().getTime());
		if (sum == null) {
			sum = new SumOrderReadyV(null, BigDecimal.valueOf(0), null, null, null);
		}
	 ...
	 * private WindowInterface window;
	 * 
	 * public EditColliAction(WindowInterface aWindow) { super("Editer kolli..."
	 * ); window = aWindow; }
	 * 
	...
	private class OrderSelectionListener implements PropertyChangeListener {
		...
		public void propertyChange(PropertyChangeEvent event) {
			if (event.getNewValue() != null && event.getPropertyName().equalsIgnoreCase("selectionIndex")
	...
	private void setBean(MainPackageV mainPackageV) {
		...
		if (order != null) {
			...
			orderComments.clear();
			orderComments.addAll(
					(List<OrderComment>) presentationModelPackable.getBufferedValue(OrderModel.PROPERTY_COMMENTS));
		}
	...
	private void checkBuffering(WindowInterface window) {
		if (presentationModelPackable.isBuffering()) {
			if (JOptionPane.showConfirmDialog(window.getComponent(), "Det er gjort endringer, ønsker du å lagre?",
					"Lagre?", JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
				saveComment(window);
			} else {
				...
				if (postShipmentSelectionList.hasSelection()) {
					...
					if (realIndex != -1 && realIndex < postShipmentList.size()) {
						...
						if (postShipment != null) {
							...
							orderComments.addAll(PostShipmentModel
	...
	public JCheckBox getCheckBoxReadyVegg(WindowInterface window) {
		checkBoxReadyVegg = BasicComponentFactory.createCheckBox(
				presentationModelPackable.getModel(OrderModel.PROPERTY_ORDER_READY_VEGG_BOOL), "Ordre klar vegg");
	...
	public JCheckBox getCheckBoxReadyTakstol(WindowInterface window) {
		checkBoxReadyTakstol = BasicComponentFactory.createCheckBox(
				presentationModelPackable.getModel(OrderModel.PROPERTY_ORDER_READY_TAKSTOL_BOOL), "Ordre klar takstol");
	...
	public JCheckBox getCheckBoxReadyPakk(WindowInterface window) {
		checkBoxReadyPakk = BasicComponentFactory.createCheckBox(
				presentationModelPackable.getModel(OrderModel.PROPERTY_ORDER_READY_PAKK_BOOL), "Ordre klar pakk");
			...
			if (order1 != null && order2 != null) {
				if (order1.getTransportDetails() != null && order2.getTransportDetails() != null) {
					return new CompareToBuilder().append(order1.getTransportYear(), order2.getTransportYear())
							.append(order1.getTransportWeek(), order2.getTransportWeek())
							.append(order1.getLoadingDate(), order2.getLoadingDate())
			...
			if (postShipment1 != null && postShipment2 != null) {
				if (postShipment1.getTransport() != null && postShipment2.getTransport() != null) {
					return new CompareToBuilder()
							.append(postShipment1.getTransport().getTransportYear(),
									postShipment2.getTransport().getTransportYear())
							.append(postShipment1.getTransport().getTransportWeek(),
									postShipment2.getTransport().getTransportWeek())
							.toComparison();
					...
					return new CompareToBuilder()
							.append(postShipment1.getPostShipmentId(), postShipment2.getPostShipmentId())
							.toComparison();
				}
			}
			return 0;
	...
	private void setOrderLineRefs(ArticleType articleType, OrderLine orderLineMain) {
		...
		if (articleRefs != null) {
			for (ArticleTypeArticleType articleRef : articleRefs) {
				orderLine = OrderLine
						.getInstance((Order) presentationModelPackable.getBufferedValue(ICostableModel.PROPERTY_ORDER),
								articleRef
										.getArticleTypeRef(),
								orderLineMain, (Deviation) presentationModelPackable
	 ...
	 */
	@SuppressWarnings("unchecked")
	private OrderLine showArticleAttributeView(List<ArticleType> newArticles, AbstractOrderModel abstractOrderModel,
		...
		if (newArticles != null && newArticles.size() == 1) {
			...
			OrderLine orderLineMain = OrderLine.getInstance(abstractOrderModel.getOrder(), newArticleType,
					abstractOrderModel.getDeviation(), abstractOrderModel.getOrderModelPostShipment());
	...
	private class RemoveArticleAction extends AbstractAction {
		...
		@SuppressWarnings("unchecked")
		public void actionPerformed(ActionEvent arg0) {
			if (JOptionPane.showConfirmDialog(window.getComponent(), "Vil du virkelig slette artikkel?", "Slette",
	...
	class CheckBoxReadyVeggActionListener implements ActionListener {
		...
		@SuppressWarnings("unchecked")
		private void handlePackedByAndColliHeight(AbstractOrderModel abstractOrderModel, Date orderReadyWall,
				String packedBy, boolean canceled) {
			if (!canceled) {
				OverviewManager overviewManager = (OverviewManager) ModelUtil
						.getBean(abstractOrderModel.getManagerName());
	...
	class CheckBoxReadyTakstolActionListener implements ActionListener {
		...
		@SuppressWarnings("unchecked")
		private void handlePackedByAndColliHeight(AbstractOrderModel abstractOrderModel, Date orderReadyTross,
				String packedByTross, boolean canceled) {
			if (!canceled) {
				OverviewManager overviewManager = (OverviewManager) ModelUtil
						.getBean(abstractOrderModel.getManagerName());
	...
	class CheckBoxReadyPakkActionListener implements ActionListener {
		...
		@SuppressWarnings("unchecked")
		private void handlePackedByAndColliHeight(AbstractOrderModel abstractOrderModel, Date orderReadyPakk,
				String packedByPack, boolean canceled) {
			if (!canceled) {
				OverviewManager overviewManager = (OverviewManager) ModelUtil
						.getBean(abstractOrderModel.getManagerName());
	...
	private void setOrderReady(AbstractOrderModel abstractOrderModel, Date orderReadyEnhet) {
		if (orderReadyEnhet != null && abstractOrderModel.getOrderReadyTakstolBool()
				&& abstractOrderModel.getOrderReadyVeggBool() && abstractOrderModel.getOrderReadyPakkBool()) {
	...
	class OrderLineSelectionListener implements PropertyChangeListener {
		...
		public void propertyChange(PropertyChangeEvent arg0) {
			...
			if (orderLineSelectionList.hasSelection()) {
				...
				orderLine = (OrderLine) orderLineSelectionList.getElementAt(
						tableOrderLines.convertRowIndexToModel(orderLineSelectionList.getSelectionIndex()));
	...
	public JList getListComments() {
		orderComments.clear();
		orderComments
				.addAll((List<OrderComment>) presentationModelPackable.getBufferedValue(OrderModel.PROPERTY_COMMENTS));
	...
	public JButton getButtonSaveComment(WindowInterface window) {
		...
		PropertyConnector.connect(buttonSaveComment, "enabled", presentationModelPackable,
	...
	private void packOrderLines(List<OrderLine> orderLines, Packable packable, WindowInterface window,
			ArticlePacker articlePacker, boolean useDefaultColli) {
	...
	final class OrderLineDoubleClickHandler extends MouseAdapter {
		...
		private void handleMouseClick(MouseEvent mouseEvent, Packable packable) {
			...
			if (SwingUtilities.isLeftMouseButton(mouseEvent) && mouseEvent.getClickCount() == 2 && hasWriteAccess()) {
				...
				if (orderLines.size() == 1 && orderLines.get(0).getArticleType() != null
						&& orderLines.get(0).getArticleType().isExtra()) {
					buttonRemoveArticle.setEnabled(true);
				}
			}
		}
	...
	private List<OrderLine> getSelectedOrderLines() {
		...
		if (selectedRows.length > 0) {
			for (int i : selectedRows) {
				orderlines.add(
						(OrderLine) orderLineSelectionList.getElementAt(tableOrderLines.convertRowIndexToModel(i)));
			}
		}
		return orderlines;
	...
	void searchOrder(final Transportable transportable, final WindowInterface window) {
		...
		if (transportable.getPostShipment() == null) {
			...
			if (mainPackageV != null) {
				...
				orderSelectionList
						.setSelectionIndex(tableOrders.convertRowIndexToView(orderSelectionList.getSelectionIndex()));
			...
			PostShipment postShipment = postShipmentManager
					.loadById(transportable.getPostShipment().getPostShipmentId());
			if (postShipment != null) {
	...
	public JLabel getLabelDayValue() {
		return BasicComponentFactory
				.createLabel(presentationModelSum.getModel(SumOrderReadyVModel.PROPERTY_PACKAGE_SUM_STRING));
	}
	...
	public JLabel getLabelWeekValue() {
		return BasicComponentFactory
				.createLabel(presentationModelWeekSum.getModel(SumOrderReadyVModel.PROPERTY_PACKAGE_SUM_STRING));
	}
	...
	public JLabel getLabelPackedByTross() {
		return BasicComponentFactory
				.createLabel(presentationModelPackable.getModel(OrderModel.PROPERTY_PACKED_BY_TROSS));
	}

	public JLabel getLabelPackedByPakk() {
		return BasicComponentFactory
				.createLabel(presentationModelPackable.getModel(OrderModel.PROPERTY_PACKED_BY_PACK));
	}
	 ...
	 * @return meny
	 */
	public JMenuBar getMenuBar(ApplicationUser user, GulvsponPackageVManager gulvsponPackageVManager,
			WindowInterface window) {
	...
	private class GulvsponMenuAction extends AbstractAction {
		 ...
		 * @param aWindow
		 */
		public GulvsponMenuAction(ApplicationUser aApplicationUser, GulvsponPackageVManager aGulvsponPackageVManager,
				WindowInterface aWindow) {
			super("Gulvspon...");
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			AbstractProductionPackageViewHandler<no.ugland.utransprod.model.PackableListItem> packageViewHandler = new PackageViewHandler(
					...
					new GulvsponPackageApplyList(login, gulvsponPackageVManager, managerRepository, vismaFileCreator),
	...
	private class MenuItemListenerPacklist implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			PostShipment postShipment = (PostShipment) postShipmentSelectionList.getElementAt(
	...
	private void saveAndAddOrderComment(OrderComment orderComment, WindowInterface window) {
		...
		initializeAndSaveOrderComment(abstractOrderModel.getOrder(), orderComment, abstractOrderModel.getDeviation(),
				window);

	}

	private void initializeAndSaveOrderComment(Order order, OrderComment orderComment, Deviation deviation,
			WindowInterface window) {
		OrderManager orderManager = (OrderManager) ModelUtil.getBean("orderManager");
		orderManager.refreshObject(order);
		if (!Hibernate.isInitialized(order.getOrderComments())) {
			orderManager.lazyLoadOrder(order, new LazyLoadOrderEnum[] { LazyLoadOrderEnum.COLLIES,
					LazyLoadOrderEnum.COMMENTS, LazyLoadOrderEnum.ORDER_LINES });
		}
	...
	protected void handleFilter() {
		...
		FilterPipeline filterPipeline = filtersOrder.size() != 0 ? new FilterPipeline(filtersOrder.toArray(filterArray))
				: null;
		tableOrders.setFilters(filterPipeline);

		filterArray = new Filter[filtersPostShipment.size()];
		filterPipeline = filtersPostShipment.size() != 0 ? new FilterPipeline(filtersPostShipment.toArray(filterArray))
				: null;
	...
	private enum FilterGroupSelector {
		...
		public abstract void addFilter(ProductAreaGroup group, List<Filter> filtersOrder,
				List<Filter> filtersPostShipment);

		private static void addFiltersForGroup(final ProductAreaGroup group, final List<Filter> filtersOrder,
			...
			Filter filterGroupPostShipment = new PatternFilter(group.getProductAreaGroupName(),
