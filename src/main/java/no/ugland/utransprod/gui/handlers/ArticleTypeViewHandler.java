 ...
 * @author atle.brekka
 */
public class ArticleTypeViewHandler extends DefaultAbstractViewHandler<ArticleType, ArticleTypeModel> {
     ...
     */
    @Inject
    public ArticleTypeViewHandler(Login aLogin, ManagerRepository managerRepository, @Assisted List<ArticleType> usedArticles,
    ...
    @Inject
    public ArticleTypeViewHandler(Login aLogin, ManagerRepository aManagerRepository, @Assisted List<ArticleType> usedArticles) {
	super("Artikkelttype", aManagerRepository.getArticleTypeManager(), aLogin.getUserType(), true);

	login = aLogin;
	managerRepository = aManagerRepository;
	attributeUpdate = new AttributeUpdate(managerRepository.getAttributeManager());
	...
	attributeSelection.addPropertyChangeListener(SelectionInList.PROPERTYNAME_SELECTION_INDEX, new SelectionListener());

	metricList = ApplicationParamUtil.findParamListByType("Betegnelse");
    }

    public JXList getListArticles(boolean useSingleSelection) {
	JXList listArticles = new JXList(getObjectSelectionList(new ArticleTypeComparator()));
	listArticles.setName("ListArticles");
	if (useSingleSelection) {
	    listArticles.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
	    listArticles.setSelectionModel(new SingleListSelectionAdapter(objectSelectionList.getSelectionIndexHolder()));
	}
	...
	JButton buttonOk = new CancelButton(window, this, false, "Ok", IconEnum.ICON_OK, null, true);
	...
	JTextField textField = BasicComponentFactory
		.createTextField(presentationModel1.getBufferedModel(ArticleTypeModel.PROPERTY_ARTICLE_TYPE_NAME));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel1.getBufferedModel(ArticleTypeModel.PROPERTY_PROD_CAT_NO));
	textField.setName("TextFieldProdCatNo");
	textField.setEnabled(hasWriteAccess());
	return textField;
    }

    public JTextField getTextFieldProdCatNo2(PresentationModel presentationModel1) {
	JTextField textField = BasicComponentFactory.createTextField(presentationModel1.getBufferedModel(ArticleTypeModel.PROPERTY_PROD_CAT_NO_2));
	...
	JComboBox comboBox = new JComboBox(new ComboBoxAdapter(metricList, presentationModel1.getBufferedModel(ArticleTypeModel.PROPERTY_METRIC)));
     ...
     * @return tekstfelt
     */
    public JTextField getTextFieldDescription(PresentationModel presentationModel1) {
	JTextField textField = BasicComponentFactory.createTextField(presentationModel1.getBufferedModel(ArticleTypeModel.PROPERTY_DESCRIPTION));
	...
	JCheckBox checkBox = BasicComponentFactory.createCheckBox(presentationModel1.getBufferedModel(ArticleTypeModel.PROPERTY_TOP_LEVEL_BOOL),
		"Toppnivå");
	...
	JCheckBox checkBox = BasicComponentFactory.createCheckBox(presentationModel1.getBufferedModel(ArticleTypeModel.PROPERTY_FORCE_IMPORT_BOOL),
		"Må importeres");
	checkBox.setEnabled(hasWriteAccess());
	return checkBox;
    }
     ...
     * @param property
     */
    public void initPresentationModel(PresentationModel aPresentationModel, String property) {
	...
	JButton buttonAddAttribute = new NewButton("attributt", attributeUpdate, window);
	...
	buttonRemoveAttribute = new DeleteButton("attributt", attributeUpdate, window);
	...
	buttonSetAttributeInactive = new JButton(new SetAttributeInactiveAction());
    ...
    @SuppressWarnings("unchecked")
    @Override
    public CheckObject checkSaveObject(ArticleTypeModel object, PresentationModel aPresentationModel, WindowInterface window) {
	String errorString = null;
	ArticleType articleType = (object).getObject();
	List<ArticleTypeArticleType> list = (List) aPresentationModel.getBufferedValue(ArticleTypeModel.PROPERTY_ARTICLE_TYPE_ARTICLE_TYPES);
	if (articleType.getArticleTypeId() == null && objectList.contains(articleType)) {
	...
	if (window != null && updateConstructionType) {
	    if (Util.showConfirmDialog(window.getComponent(), "Oppdaterer garsjetyper?",
	...
	((ArticleTypeManager) overviewManager).lazyLoad(articleType, new LazyLoadArticleTypeEnum[] {
		LazyLoadArticleTypeEnum.CONSTRUCTION_TYPE_ARTICLE, LazyLoadArticleTypeEnum.ATTRIBUTE,
		LazyLoadArticleTypeEnum.ARTICLE_TYPE_ARTICLE_TYPE });
	Set<ConstructionTypeArticle> articles = articleType.getConstructionTypeArticles();
	Set<ArticleTypeAttribute> articleAttributes = articleType.getArticleTypeAttributes();
	...
	for (ConstructionTypeArticle consArticle : articles) {
	    constructionTypeArticleManager.lazyLoad(consArticle, new LazyLoadEnum[][] { { LazyLoadEnum.ATTRIBUTES, LazyLoadEnum.NONE } });
	    attributes = consArticle.getAttributes();
	    for (ArticleTypeAttribute articleTypeAttribute : articleAttributes) {
		if (!constructionTypeHasAttribute(attributes, articleTypeAttribute)) {
		    newAttribute = new ConstructionTypeArticleAttribute(null, consArticle, articleTypeAttribute,
			    Util.convertNumberToBoolean(articleTypeAttribute.getAttribute().getYesNo()) ? "Nei" : "", null);
		    attributes.add(newAttribute);
		}
	    }
	    constructionTypeArticleManager.saveConstructionTypeArticle(consArticle);
	}
    }
     ...
     */
    private boolean constructionTypeHasAttribute(Set<ConstructionTypeArticleAttribute> attributes, ArticleTypeAttribute articleTypeAttribute) {
	boolean hasAttribute = false;
	for (ConstructionTypeArticleAttribute attribute : attributes) {
	    if (attribute.getArticleTypeAttribute().getAttribute().equals(articleTypeAttribute.getAttribute())) {
		hasAttribute = true;
		break;
	    }
	}
     ...
     * @author atle.brekka
     */
    public static final class ArticleTypeTableModel extends AbstractTableAdapter {

	private static final long serialVersionUID = 1L;

	private static final String[] COLUMNS = { "Navn", "Betegnesle", "Beskrivelse" };
	...
	ArticleTypeModel attributeable = (ArticleTypeModel) presentationModel.getBean();

	AttributeViewHandler attributeViewHandler = new AttributeViewHandler(login, managerRepository);
	AttributeView attributeView = new AttributeView(attributeViewHandler, false);
	WindowInterface dialog = new JDialogAdapter(new JDialog(ProTransMain.PRO_TRANS_MAIN, "Attributt", true));
	...
	if (!attributeView.isCanceled()) {
		...
		attributeList.addAll(attributeable.getArticleAttributes(newAttributes));
	...
	public boolean doDelete(WindowInterface window) {
	    ...
	    managerRepository.getArticleTypeAttributeManager().lazyLoad(attribute,
		    new LazyLoadEnum[][] { { LazyLoadEnum.CONSTRUCTION_TYPE_ARTICLE_ATTRIBUTES, LazyLoadEnum.NONE } });

	    if (attribute.getConstructionTypeArticleAttributes() != null && attribute.getConstructionTypeArticleAttributes().size() != 0) {
		deleted = false;
		Util.showErrorDialog(window, "Feil", "Kan ikke slette attributt som er referert til av an garasjetype eller master");
	    } else {
	...
	final ArticleTypeAttribute attribute = (ArticleTypeAttribute) attributeSelection.getSelection();
	return attribute;
    }
	...
	String errorString = null;
	((ArticleTypeManager) overviewManager).lazyLoad(articleType, new LazyLoadArticleTypeEnum[] {
		LazyLoadArticleTypeEnum.ARTICLE_TYPE_ARTICLE_TYPE_REF, LazyLoadArticleTypeEnum.CONSTRUCTION_TYPE_ARTICLE });

	if (articleType.getArticleTypeArticleTypeRefs() != null && articleType.getArticleTypeArticleTypeRefs().size() != 0) {
	    errorString = "Kan ikke slette artikkel som er referert til av andre artikler";
	} else if (articleType.getConstructionTypeArticles() != null && articleType.getConstructionTypeArticles().size() != 0) {
	...
	buttonAddArticle = new NewButton("artikkel", new ArticleUpdate(), window);
	...
	ArticleType clonedType = ArticleTypeModel.cloneArticleType((ArticleType) presentationModel.getBufferedValue(AbstractModel.PROPERTY_OBJECT));

	((ArticleTypeManager) overviewManager).lazyLoad(clonedType, new LazyLoadArticleTypeEnum[] {
		LazyLoadArticleTypeEnum.ARTICLE_TYPE_ARTICLE_TYPE, LazyLoadArticleTypeEnum.ATTRIBUTE });
	...
	buttonRemoveArticle = new DeleteButton("artikkel", new ArticleUpdate(), window);
	...
	public boolean doDelete(WindowInterface window) {
		...
		if (selectedNode.isLeaf()) {
		    article = (ArticleTypeArticleType) selectedNode.getParent().getObject();
		} else {
		    article = (ArticleTypeArticleType) selectedNode.getObject();
		}
		ArticleType articleType = (ArticleType) ((ArticleTypeTreeNode) articleTypeTreeTableModel.getRoot()).getObject();
		articleType.getArticleTypeArticleTypes().remove(article);

		BufferedValueModel bufferedArticles = presentationModel.getBufferedModel(ArticleTypeModel.PROPERTY_ARTICLE_TYPE_ARTICLE_TYPES);
		bufferedArticles.setValue(new ArrayListModel(articleType.getClonedArticleTypeArticleTypes()));
	...
	List<ArticleType> articles = (List<ArticleType>) presentationModel.getValue(ArticleTypeModel.PROPERTY_ARTICLES);

	ArticleType currentArticleType = (ArticleType) presentationModel.getBufferedValue(AbstractModel.PROPERTY_OBJECT);

	ArticleTypeViewHandler articleTypeViewHandler = new ArticleTypeViewHandler(login, managerRepository, articles);
	ArticleTypeView articleTypeView = new ArticleTypeView(articleTypeViewHandler, false, false);
     ...
     */
    private void showArticleAttributeView(List<ArticleType> newArticles, ArticleType currentArticleType) {
	if (newArticles != null && newArticles.size() != 0) {
		...
		((ArticleTypeManager) overviewManager).lazyLoad(article, new LazyLoadArticleTypeEnum[] {
			LazyLoadArticleTypeEnum.ARTICLE_TYPE_ARTICLE_TYPE, LazyLoadArticleTypeEnum.ATTRIBUTE });
		articles.add(new ArticleTypeArticleType(null, currentArticleType, article));
	    }

	    ArticleTypeTreeNode rootNode = (ArticleTypeTreeNode) articleTypeTreeTableModel.getRoot();
	    ...
	    BufferedValueModel bufferedArticles = presentationModel.getBufferedModel(ArticleTypeModel.PROPERTY_ARTICLE_TYPE_ARTICLE_TYPES);
	    bufferedArticles.setValue(new ArrayListModel(articleType.getArticleTypeArticleTypes()));
	...
	if (hasWriteAccess()) {
	    TreePath path = treeTableArticleType.getPathForRow(treeTableArticleType.getSelectedRow());

	    buttonRemoveArticle.setEnabled(false);

	    if (path != null) {
		selectedNode = (ArticleTypeTreeNode) path.getLastPathComponent();
	...
	public void actionPerformed(ActionEvent e) {
	    String articleName = JOptionPane.showInputDialog("Navn på artikkel");
		...
		((ArticleTypeManager) overviewManager).saveArticleType(articleType);
	...
	if (hasWriteAccess()) {
		...
		ArticleTypeAttribute attribute = (ArticleTypeAttribute) attributeSelection.getSelection();
     ...
     */
    @Override
    protected AbstractEditView<ArticleTypeModel, ArticleType> getEditView(AbstractViewHandler<ArticleType, ArticleTypeModel> handler,
	    ArticleType object, boolean searching) {
	((ArticleTypeManager) overviewManager).lazyLoad(object, new LazyLoadArticleTypeEnum[] { LazyLoadArticleTypeEnum.ATTRIBUTE,
		LazyLoadArticleTypeEnum.ARTICLE_TYPE_ARTICLE_TYPE });
     ...
     * @author atle.brekka
     */
    static class ArticleTypeComparator implements Comparator<ArticleType>, Serializable {
	...
	tableAttributes = new JXTable(new AttributeTableModel(attributeSelection));
	tableAttributes.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
	tableAttributes.setSelectionModel(new SingleListSelectionAdapter(attributeSelection.getSelectionIndexHolder()));
	...
	private static final long serialVersionUID = 1L;
	private static final String[] columnNames = new String[] { "Navn", "Inaktiv" };
    ...
    @Override
    public boolean canClose(String actionString, WindowInterface window) {
	super.canClose(actionString, window);
	objectSelectionList.clearSelection();
	return true;
    }
}
