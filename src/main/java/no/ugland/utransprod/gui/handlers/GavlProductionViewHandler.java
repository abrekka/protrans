...
public class GavlProductionViewHandler extends ProductionViewHandler {
	 ...
	 */
	public GavlProductionViewHandler(ApplyListInterface<Produceable> productionInterface, String title, Login login,
		...
		super(productionInterface, title, login, null, "produksjon", TableEnum.TABLEPRODUCTIONGAVL, articleType,
	...
	public static enum GavlColumn {
		TRANSPORT("Transport") {
			...
			public Object getValue(GavlProductionV gavlProductionV,
					StatusCheckerInterface<Transportable> takstolChecker, Map<String, String> statusMap,
		...
		ANTALL("Antall") {
			...
			public Object getValue(GavlProductionV gavlProductionV,
					...
					WindowInterface window, ManagerRepository managerRepository,
					ApplyListInterface<Produceable> applyListInterface) {
				return gavlProductionV.getNumberOfItems() != null ? gavlProductionV.getNumberOfItems().intValue()
						: null;
			}
		},
		...
		TAKSTOL("Takstol") {
			...
			public Object getValue(GavlProductionV gavlProductionV,
					...
					WindowInterface window, ManagerRepository managerRepository,
					ApplyListInterface<Produceable> applyListInterface) {
				return getStatus(takstolChecker, statusMap, gavlProductionV, window, managerRepository,
						applyListInterface);
			}
		},
		OPPLASTING("Opplasting") {
			...
			public Object getValue(GavlProductionV gavlProductionV,
					...
					WindowInterface window, ManagerRepository managerRepository,
					ApplyListInterface<Produceable> applyListInterface) {
				return gavlProductionV.getLoadingDate() != null
						? Util.SHORT_DATE_FORMAT.format(gavlProductionV.getLoadingDate()) : null;
			}
		},
		PRODUSERT("Produsert") {
			@Override
			...
			public Object getValue(GavlProductionV gavlProductionV,
					...
					WindowInterface window, ManagerRepository managerRepository,
					ApplyListInterface<Produceable> applyListInterface) {
				return gavlProductionV.getProduced() != null
						? Util.SHORT_DATE_TIME_FORMAT.format(gavlProductionV.getProduced()) : "---";
			}
		},
		...
		STARTET("Startet") {
			...
			public Object getValue(GavlProductionV gavlProductionV,
					...
					WindowInterface window, ManagerRepository managerRepository,
					ApplyListInterface<Produceable> applyListInterface) {
				return gavlProductionV.getActionStarted() != null
						? Util.SHORT_DATE_TIME_FORMAT.format(gavlProductionV.getActionStarted()) : "---";
			}
		},
		STANDARD("Standard") {
			@Override
			public Object getValue(GavlProductionV gavlProductionV,
					...
					WindowInterface window, ManagerRepository managerRepository,
					ApplyListInterface<Produceable> applyListInterface) {
				String takstolStatus = getStatus(takstolChecker, statusMap, gavlProductionV, window, managerRepository,
						applyListInterface);
		...
		REELL_TIDSFORBRUK("Reell tidsforbruk") {
			...
			public Object getValue(GavlProductionV gavlProductionV,
					...
					WindowInterface window, ManagerRepository managerRepository,
					ApplyListInterface<Produceable> applyListInterface) {
				return gavlProductionV.getRealProductionHours() == null ? Tidsforbruk
						.beregnTidsforbruk(gavlProductionV.getActionStarted(), gavlProductionV.getProduced())
						: gavlProductionV.getRealProductionHours();
			}
		...
		static String getStatus(StatusCheckerInterface<Transportable> checker, Map<String, String> statusMap,
				GavlProductionV gavlProductionV, WindowInterface window, ManagerRepository managerRepository,
				ApplyListInterface<Produceable> applyListInterface) {
...
//					managerRepository.getOrderManager().saveOrder(order);
					managerRepository.getOrderManager().settStatus(order.getOrderId(),order.getStatus());
	...
	final class GavlProductionTableModel extends AbstractTableAdapter {
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			...
			return GavlColumn.valueOf(columnName).getValue(gavlProductionV, takstolChecker, statusMap, window,
		...
		public Class<?> getColumnClass(int columnIndex) {
			return GavlColumn
					.valueOf(StringUtils.replace(
							StringUtils.replace(StringUtils.upperCase(getColumnName(columnIndex)), ".", "_"), " ", "_"))
					.getColumnClass();
		}
	}
	...
	void setAdditionHighlighters() {
		ColorHighlighter redPattern = new ColorHighlighter(new PatternPredicate("nei", GavlColumn.STANDARD.ordinal()),
