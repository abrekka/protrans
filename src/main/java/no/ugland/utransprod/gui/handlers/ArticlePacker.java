...
public class ArticlePacker {
	...
	public ArticlePacker(ColliViewHandlerProvider aColliViewHandlerProvider,
			Map<String, String> aColliSetup,
	...
	public void packOrderLine(final OrderLine orderLine, Packable packable,
			final WindowInterface window, final boolean useDefaultColli) {
		String articleName = orderLine.getArticleName();
		if (canPack(articleName)) {
			createColliAndPackArticle(articleName, packable, orderLine, window,
					useDefaultColli);
		} else {
			Util.showMsgDialog(window.getComponent(),
					"Kan ikke pakke artikkel",
					"Denne artikkelen kan ikke pakkes her!");
		}

	}

	public boolean canPack(String articleName) {
		return ApplicationParamUtil.getNotPackageList().indexOf(articleName) < 0 ? true
				: false;
	}

	private void createColliAndPackArticle(String articleName,
			Packable packable, OrderLine orderLine, WindowInterface window,
		...
		return new Comparator<Colli>() {
			...
			public int compare(Colli colli, Colli annenColli) {
				...
				return colli.getColliName()
						.compareTo(annenColli.getColliName());
			}
		};
	}

	private void addArticleToColliAndSendFileToVisma(OrderLine orderLine,
			Colli useColli, WindowInterface window) {
		try {
	...
	private void addArticleToColli(OrderLine orderLine, Colli useColli)
			throws ProTransException {
		ColliViewHandler colliViewHandler = colliViewHandlerProvider
				.getColliViewHandler(useColli);
		if (colliViewHandler != null) {
			colliViewHandler.addOrderLine(orderLine, 0);
		}
	}

	private void sendFileToVisma(final OrderLine orderLine)
			throws ProTransException {
	...
	private String getColliName(String articleName) {
		return colliSetup.containsValue(articleName) ? getColliNameFromColliSetup(articleName)
				: null;
	}
