...
public class ArticlePacker {
	...
	public void packOrderLines(final List<OrderLine> orderLines, Packable packable, final WindowInterface window,
			final boolean useDefaultColli) {
		...
		if (canPack(articleNames)) {
			...
			Util.showMsgDialog(window.getComponent(), "Kan ikke pakke artikkel",
	...
	private void createColliAndPackArticle(List<String> articleNames, Packable packable, List<OrderLine> orderLines,
			WindowInterface window, boolean useDefaultColli) {
	...
	private String getColliName(List<String> articleNames) {
		...
		for (String articleName : articleNames) {
			String tmpColliName = colliSetup.containsValue(articleName) ? getColliNameFromColliSetup(articleName)
					: null;
			if (colliName == null) {
