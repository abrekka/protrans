...
public class ExcelReportViewHandler implements Closeable {
	...
	PropertyConnector conn = new PropertyConnector(yearChooser1, "year", presentationModel.getModel(ExcelReportSetting.PROPERTY_YEAR), "value");
	...
	JComboBox comboBox = new JComboBox(new ComboBoxAdapter(Util.getWeeks(), presentationModel.getModel(ExcelReportSetting.PROPERTY_WEEK_FROM)));
	...
	JComboBox comboBox;
	comboBox = new JComboBox(new ComboBoxAdapter(Util.getWeeks(), presentationModel.getModel(ExcelReportSetting.PROPERTY_WEEK_TO)));
	if (!excelReportEnum.useTo()) {// dersom til ikke skal brukes kobles
	    // til og fra
	    PropertyConnector conn = new PropertyConnector(presentationModel.getModel(ExcelReportSetting.PROPERTY_WEEK_FROM), "value",
	...
	if (presentationModel.getValue(ExcelReportSetting.PROPERTY_EXCEL_REPORT_TYPE) == ExcelReportEnum.PRODUCTIVITY_PACK) {
	...
	JComboBox comboBox = Util.createComboBoxProductArea(presentationModel.getModel(ExcelReportSetting.PROPERTY_PRODUCT_AREA), addEmpty);
