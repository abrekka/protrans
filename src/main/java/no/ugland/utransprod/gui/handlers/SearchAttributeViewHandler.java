...
public class SearchAttributeViewHandler implements Closeable {
	...
	public JYearChooser getYearChooserFrom() {
		...
		PropertyConnector connYear = new PropertyConnector(yearChooser, "year",
				queryPresentationModel.getModel(QuerySettings.PROPERTY_YEAR_FROM), "value");
	...
	public JYearChooser getYearChooserTo() {
		JYearChooser yearChooser = new JYearChooser();
		PropertyConnector connYear = new PropertyConnector(yearChooser, "year",
				queryPresentationModel.getModel(QuerySettings.PROPERTY_YEAR_TO), "value");
	...
	public JComboBox getComboBoxWeeksFrom() {
		return new JComboBox(new ComboBoxAdapter(Util.getWeeks(),
				queryPresentationModel.getModel(QuerySettings.PROPERTY_WEEK_FROM)));
	}
	...
	public JComboBox getComboBoxWeeksTo() {
		return new JComboBox(
				new ComboBoxAdapter(Util.getWeeks(), queryPresentationModel.getModel(QuerySettings.PROPERTY_WEEK_TO)));
	}
	...
	private void lazyLoadArticles(List<ArticleType> articles) {
		...
		for (ArticleType article : articles) {
			articleTypeManager
					.lazyLoad(article,
							new LazyLoadArticleTypeEnum[] { LazyLoadArticleTypeEnum.ARTICLE_TYPE_ARTICLE_TYPE_REF,
	...
	public JXTreeTable getTreeTableArticles() {
		treeTableArticles = new JXTreeTable(new ArticleTypeTreeTableModel(articleList));
		treeTableArticles.getSelectionModel().addListSelectionListener(new ArticleSelectionListener());
	...
	public JXTreeTable getTreeTableChosen() {
		treeTableModelChosen = new OrderLineTreeTableModel<Order, OrderModel>(null, true);
		treeTableChosen = new JXTreeTable(treeTableModelChosen);
		treeTableChosen.getSelectionModel().addListSelectionListener(new ArticleSelectionListener());
	...
	public JRadioButton getRadioButtonInvoiceDate() {
		JRadioButton radioButtonInvoiceDate = BasicComponentFactory.createRadioButton(
				queryPresentationModel.getModel(QuerySettings.PROPERTY_DATE_ENUM_STRING), "INVOICE_DATE",
				"Fakturadato");
	...
	public JRadioButton getRadioButtonOrderDate() {
		return BasicComponentFactory.createRadioButton(
				queryPresentationModel.getModel(QuerySettings.PROPERTY_DATE_ENUM_STRING), "ORDER_DATE", "Ordredato");
	}
	...
	public JRadioButton getRadioButtonTransportWeek() {
		return BasicComponentFactory.createRadioButton(
				queryPresentationModel.getModel(QuerySettings.PROPERTY_DATE_ENUM_STRING), "TRANSPORT_WEEK",
				"Transportuke");
	}
	...
	public JRadioButton getRadioButtonOr() {
		JRadioButton radioButtonOr = BasicComponentFactory.createRadioButton(
				queryPresentationModel.getModel(QuerySettings.PROPERTY_OPERAND_ENUM_STRING), "OR", "Eller");
	...
	public JRadioButton getRadioButtonAnd() {
		JRadioButton radioButtonAnd = BasicComponentFactory.createRadioButton(
				queryPresentationModel.getModel(QuerySettings.PROPERTY_OPERAND_ENUM_STRING), "AND", "Og");
	...
	public JDateChooser getDateChooserFrom() {
		...
		PropertyConnector connDate = new PropertyConnector(dateChooserFrom, "date",
				queryPresentationModel.getModel(QuerySettings.PROPERTY_DATE_FROM), "value");
	...
	public JDateChooser getDateChooserTo() {
		...
		PropertyConnector connDate = new PropertyConnector(dateChooserTo, "date",
				queryPresentationModel.getModel(QuerySettings.PROPERTY_DATE_TO), "value");
	...
	public void initEventHandling() {
		...
		articleSelectionList.addPropertyChangeListener(SelectionInList.PROPERTYNAME_SELECTION_EMPTY,
	...
	void setComponentEnablement() {
		TreePath path = treeTableArticles.getPathForRow(treeTableArticles.getSelectedRow());
		buttonSelect.setEnabled(false);

		if (path != null) {
			TreeNode selectedNode = (ArticleTypeTreeNode) path.getLastPathComponent();
		...
		if (path != null) {
			TreeNode selectedNode = (OrderLineTreeNode) path.getLastPathComponent();
			if (selectedNode.isLeaf()) {
	 ...
	 * @return ordrelinje
	 */
	private OrderLine setOrderLineRefs(ArticleType articleType, OrderLine orderLineRef, List<OrderLine> orderLines,
		...
		if (articleType != null && usedArticles.contains(articleType)) {
			...
			Set<ArticleTypeArticleType> articleTypeArticleTypes = articleType.getArticleTypeArticleTypes();

			if (articleTypeArticleTypes != null) {

				for (ArticleTypeArticleType articleTypeArticleType : articleTypeArticleTypes) {
					orderLine = setOrderLineRefs(articleTypeArticleType.getArticleTypeRef(), orderLine, orderLines,
							usedArticles);
				}
			}
	 ...
	 * @return ordrelinje
	 */
	private OrderLine getOrderLine(ArticleTypeTreeNode leaf, List<OrderLine> orderLines1,
			List<ArticleType> usedArticles) {
		...
		if (parent.getParent() != null && parent.getParent().getObject() instanceof ArrayListModel) {
			return setOrderLineRefs(articleType, null, orderLines1, usedArticles);
		}
	...
	void doSelect() {
		...
		if (path != null) {
			ArticleTypeTreeNode selectedNode = (ArticleTypeTreeNode) path.getLastPathComponent();
			if (selectedNode.isLeaf()) {
				ArticleTypeAttribute attribute = (ArticleTypeAttribute) selectedNode.getObject();
				...
				if (object != null && ((OrderLineTreeNode) object).getObject() != null) {
				...
				OrderLine orderLine = getOrderLine(selectedNode, orderLines1, null);
				...
				if (orderLine.getOrderLineAttributes().contains(orderLineAttribute)) {
					List<OrderLineAttribute> atts = new ArrayList<OrderLineAttribute>(
							orderLine.getOrderLineAttributes());
					orderLineAttribute = (OrderLineAttributeCriteria) atts.get(atts.indexOf(orderLineAttribute));
				} else {
				...
				AttributeCriteriaView attributeCriteriaView = new AttributeCriteriaView(orderLineAttribute,
						attributeCriteriaViewHandler);
				...
				dialogAttributes.add(attributeCriteriaView.buildPanel(dialogAttributes));
				...
				if (object == null || ((OrderLineTreeNode) object).getObject() == null) {
					...
					treeTableModelChosen = new OrderLineTreeTableModel(orderWrapper, true);
					treeTableChosen.setTreeTableModel(treeTableModelChosen);
				}
				treeTableModelChosen.fireChanged(true);
	...
	void doSearch() {
		...
		orderLinesConstrcutionTypeArticleAttribute = orderLineManager.findByConstructionTypeArticleAttributes(criterias,
				querySettings);
	...
	@SuppressWarnings("unchecked")
	void doDeselect() {
		TreePath path = treeTableChosen.getPathForRow(treeTableChosen.getSelectedRow());

		if (path != null) {
			OrderLineTreeNode selectedNode = (OrderLineTreeNode) path.getLastPathComponent();
			if (selectedNode.isLeaf()) {
				OrderLineAttribute attribute = (OrderLineAttribute) selectedNode.getObject();
				OrderLine orderLine = (OrderLine) selectedNode.getParent().getObject();
	...
	private String criteriasToString() {
		...
		if (criterias != null) {
			for (OrderLine orderLine : criterias) {
				toStringBuffer.append("[").append(orderLine.getGeneratedArticlePath()).append(";");
				attributes = new ArrayList(orderLine.getOrderLineAttributes());

				for (OrderLineAttributeCriteria attribute : attributes) {
					toStringBuffer.append("(").append(attribute.getAttributeName()).append(":")
							.append(attribute.getAttributeValueFrom()).append("-")
							.append(Util.nullToString(attribute.getAttributeValueTo()));
