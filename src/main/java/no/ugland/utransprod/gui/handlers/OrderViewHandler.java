 ...
 * @author atle.brekka
 */
public class OrderViewHandler extends DefaultAbstractViewHandler<Order, OrderModel> implements ProductAreaGroupProvider,
	ViewHandlerExt<Order, OrderModel> {
	...
	if (presentationModel.getBufferedValue(OrderModel.PROPERTY_PRODUCT_AREA) != null) {
	    ...
	    List<ConstructionType> types = constructionTypeManager.findByProductArea((ProductArea) presentationModel
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_GAVL_DONE));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_TAKSTOL_DONE));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_FRONT_DONE));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_VEGG_DONE));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_ASSEMBLY_DONE_STRING));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_TAKSTOL_PACKAGED));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_LOADING_DATE_STRING));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_PROJECT_NR));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_PROJECT_NAME));
	...
	JComboBox comboBox;
	if (searching) {
	    comboBox = Util.createComboBoxProductArea(presentationModel.getBufferedModel(OrderModel.PROPERTY_PRODUCT_AREA), true);

	} else {
	    comboBox = Util.createComboBoxProductArea(presentationModel.getBufferedModel(OrderModel.PROPERTY_PRODUCT_AREA), false);
	}
	comboBox.setName("ComboBoxProductArea");
	addEditComponent(comboBox);
	presentationModel.getBufferedModel(OrderModel.PROPERTY_PRODUCT_AREA).addPropertyChangeListener(
		new ProductAreaChangeListener(presentationModel));
	return comboBox;

    }

    public boolean openOrderView(Order order, boolean isSearching, WindowInterface window) {
    ...
    private void setCustomerSearchData(Order order, WindowInterface dialog, OrderModel orderModel) throws ProTransException {
	if (orderModel.getCustomerNr() != null || orderModel.getCustomerFirstName() != null || orderModel.getCustomerLastName() != null) {
	...
	Project project;
	lazyLoadOrder(order, new LazyLoadOrderEnum[] { LazyLoadOrderEnum.ORDER_COSTS, LazyLoadOrderEnum.ORDER_LINES,
		LazyLoadOrderEnum.ORDER_LINE_ORDER_LINES, LazyLoadOrderEnum.COMMENTS, LazyLoadOrderEnum.COLLIES, LazyLoadOrderEnum.PROCENT_DONE });
    ...
    public boolean openEditViewExt(Order object, boolean isSearching, WindowInterface parentWindow) {
	...
	if (object.getCustomer() == null || object.getCustomer().getCustomerId() == null) {
	    ...
	    Customer customer = new Customer(null, object.getCustomerNr() != null ? Integer.valueOf(object.getCustomerNr()) : null,
    ...
    @SuppressWarnings("unchecked")
    @Override
    public CheckObject checkSaveObject(OrderModel object, PresentationModel presentationModel1, WindowInterface window) {
	...
	if (object.getCustomer() == null) {
	    ...
	    if (customer != null) {
		if (JOptionPane.showConfirmDialog(window.getComponent(), "Kunde med dette kundenr finnes fra før med navn " + customer.getFirstName()
			+ " " + customer.getLastName() + " ønskes denne kunden å benyttes?", "Kunde finnes fra før", JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
     ...
     * @param window
     */
    private void initStatus(List<Order> orders, StatusCheckerInterface<Transportable> steinChecker, WindowInterface window) {
	...
	orderPanelTableModel = new OrderTableModel(orderPanelSelectionList, orderPanelList, orderPanelTypeEnum, steinChecker);
	tableOrders.setModel(orderPanelTableModel);
	tableOrders.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
	tableOrders.setSelectionModel(new SingleListSelectionAdapter(orderPanelSelectionList.getSelectionIndexHolder()));
	...
	((OrderManager) overviewManager).lazyLoadOrder(object, new LazyLoadOrderEnum[] { LazyLoadOrderEnum.DEVIATIONS });
	...
	((OrderManager) overviewManager).lazyLoadOrder(object.getObject(), new LazyLoadOrderEnum[] { LazyLoadOrderEnum.ORDER_COSTS,
		LazyLoadOrderEnum.ORDER_LINES, LazyLoadOrderEnum.ORDER_LINE_ORDER_LINES, LazyLoadOrderEnum.COLLIES });
	...
	orderArticleViewHandler = new OrderArticleViewHandler<Order, OrderModel>(aPresentationModel, searching, login, managerRepository);
     ...
     * @return hjelpeklasse
     */
    public OrderCostsViewHandler getOrderCostsViewHandler(PresentationModel aPresentationModel) {
	orderCostsViewHandler = new OrderCostsViewHandler(aPresentationModel, login, false, false, false, managerRepository);
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (orderPanelSelectionList.getSelection() != null) {
		    ...
		    openEditView((Order) orderPanelSelectionList.getElementAt(index), false, window);
		}
	    }
	...
	JComboBox comboBox = Util.createComboBoxProductAreaGroup(presentationModelProductAreaGroup
	...
	if (order.getOfferAddresses() != null) {
	    OfferAddress offerAddress = (OfferAddress) JOptionPane.showInputDialog(window.getComponent(), "Ordre har flere adresser", "Velg adresse",
		    JOptionPane.INFORMATION_MESSAGE, null, order.getOfferAddresses().toArray(), null);
	...
	DeviationViewHandler deviationViewHandler = deviationViewHandlerFactory.create(order, true, true, false, null, false);

	deviationOverviewView = deviationOverviewViewFactory.create(deviationViewHandler, false, order, true, true, false, null, false);
     ...
     */
    @Override
    protected AbstractEditView<OrderModel, Order> getEditView(AbstractViewHandler<Order, OrderModel> handler, Order object, boolean searching1) {
	return null;
    }
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (oldValue != null && newValue != null && !oldValue.getProductArea().equalsIgnoreCase(newValue.getProductArea())) {
		    initTypes = true;
	...
	if (!isCanceled) {
		...
		WindowInterface orderDialog = new JDialogAdapter(new JDialog(ProTransMain.PRO_TRANS_MAIN, "Velg ordre", true));
	...
	if (orders != null) {
	    ...
	    for (Order order : orders) {
		((OrderManager) overviewManager).lazyLoadOrder(order, new LazyLoadOrderEnum[] { LazyLoadOrderEnum.POST_SHIPMENTS });
	...
	public void propertyChange(PropertyChangeEvent evt) {
	    ProductAreaGroup group = (ProductAreaGroup) presentationModelProductAreaGroup.getValue(ProductAreaGroupModel.PROPERTY_PRODUCT_AREA_GROUP);
	    if (group != null && !group.getProductAreaGroupName().equalsIgnoreCase("Alle")) {

		Filter filter = new PatternFilter(group.getProductAreaGroupName(), Pattern.CASE_INSENSITIVE, table.getColumnExt("Produktområde")
			.getModelIndex());
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_CUTTING_FILE_NAME));
	...
	public void actionPerformed(ActionEvent e) {
		...
		Util.runInThreadWheel(window.getRootPane(), new CuttingImporter(window, overwrite, presentationModel), null);
	    }
	...
	private boolean continueImport() {
		...
		doImport = Util.showConfirmDialog(window, "Ordre har kappfil", "Order har kappfil, vil du overskrive denne?");
		overwrite = true;
	    }
	    return doImport;
	}
	...
	public void actionPerformed(ActionEvent e) {
	    if (presentationModel.getBufferedValue(OrderModel.PROPERTY_CUTTING_FILE_NAME) == null) {
		Util.showMsgDialog(window != null ? window.getComponent() : null, "Har ikke fil", "Order har ikke kappfil");
	    } else {
	...
	overviewManager.lazyLoad(order, new LazyLoadEnum[][] { { LazyLoadEnum.ORDER_COSTS, LazyLoadEnum.NONE },
		{ LazyLoadEnum.ORDER_COMMENTS, LazyLoadEnum.NONE }, { LazyLoadEnum.ORDER_LINES, LazyLoadEnum.NONE },
		{ LazyLoadEnum.COLLIES, LazyLoadEnum.NONE }, { LazyLoadEnum.PROCENT_DONES, LazyLoadEnum.NONE } });
	...
	public CuttingImporter(WindowInterface aWindow, boolean doOverwrite, final PresentationModel aPresentationModel) {
	    window = aWindow;
	...
	public void doWhenFinished(Object object) {
		...
		if (imported) {
		    Util.showMsgDialog(window != null ? window.getComponent() : null, "Importert", "Kappfil er importert");
		}
	    }
	...
	public Object doWork(Object[] params, JLabel labelInfo) {
		...
		String proId = importAndSaveCutting(((OrderModel) presentationModel.getBean()).getObject(), overwriteExistingCutting);
	...
	return Util.getFileName(aWindow != null ? aWindow.getComponent() : null, new FileExtensionFilter("boq", "BOQ-fil"), null);
    }
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_TELEPHONE_NR_SITE));
	...
	JTextField textField = BasicComponentFactory.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_MAX_TROSS_HEIGHT));
