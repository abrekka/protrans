	...
	public boolean saveObjectExt(AbstractModel<Order, OrderModel> objectModel,
		...
		try {
			...
			Util.showErrorDialog(window, "Feil",
					"Ordre er oppdatert av noen andre, oppdater før lagring");
			e.printStackTrace();
		}
		...
		if (object.getCustomer() == null) {
			...
			if (customer != null) {
				if (JOptionPane.showConfirmDialog(window.getComponent(),
						"Kunde med dette kundenr finnes fra før med navn "
								+ customer.getFirstName() + " "
								+ customer.getLastName()
								+ " ønskes denne kunden å benyttes?",
			...
			if (orderNr != null) {
				...
				if (order != null) {
					Util.showMsgDialog(window.getComponent(),
							"Ordre finnes fra før", "Ordre finnes fra før");
				}
			}
