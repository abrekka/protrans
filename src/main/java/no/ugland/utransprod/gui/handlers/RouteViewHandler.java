...
public class RouteViewHandler
	 ...
	 */
	@Inject
	public RouteViewHandler(final OrderViewHandlerFactory aOrderViewHandlerFactory, final Login aLogin,
		...
		presentationModelTransportSum = new PresentationModel(
				new TransportSumVModel(new TransportSumV(Integer.valueOf(0), BigDecimal.valueOf(0), null)));
	...
	void setTransportSum() {
		...
		TransportSumV sum = managerRepository.getTransportSumVManager()
				.findYearAndWeekByProductAreaGroup(yearWeek.getYear(), yearWeek.getWeek(), ProductAreaGroup.UNKNOWN);
		...
		Budget productionBudget = managerRepository.getBudgetManager().findByYearAndWeekPrProductAreaGroup(
				yearWeekMinusOne.getYear(), yearWeekMinusOne.getWeek(), BudgetType.PRODUCTION);
	...
	public JButton getButtonDeletePostShipment(WindowInterface window) {
		...
		postShipmentSelectionList.addPropertyChangeListener(SelectionInList.PROPERTYNAME_SELECTION_EMPTY,
	...
	public JLabel getLabelOrderCount() {
		return BasicComponentFactory
				.createLabel(presentationModelTransportSum.getModel(TransportSumVModel.PROPERTY_ORDER_COUNT_STRING));
	}
	...
	public JLabel getLabelGarageCost() {
		return BasicComponentFactory
				.createLabel(presentationModelTransportSum.getModel(TransportSumVModel.PROPERTY_GARAGE_COST_STRING));
	}
	 ...
	 * @return sjekkboks
	 */
	public JCheckBox getCheckBoxFilterSent(WindowInterface window) {
		checkBoxFilter = new JCheckBox("Vis opplastet");
		checkBoxFilter.setSelected(true);
		checkBoxFilter.setName("CheckBoxFilter");
		checkBoxFilter.addActionListener(new FilterActionListener(window));
	...
	public JCheckBox getCheckBoxListView() {
		...
		checkBoxListView
				.setSelected(transportWeekViewHandler != null ? transportWeekViewHandler.getUseListView() : false);
	...
	public TransportWeekView getTransportWeekView() {
		if (transportWeekView == null) {
			if (transportWeekViewHandler == null) {
				TransportViewHandler transportViewHandler = new TransportViewHandler(orderViewHandlerFactory, login,
						managerRepository, deviationViewHandlerFactory, showTakstolInfoActionFactory, vismaFileCreator);
				/* applicationUser, userType,transportManager); */
				transportWeekViewHandler = new TransportWeekViewHandler(login, managerRepository, transportViewHandler,
						orderViewHandlerFactory, deviationViewHandlerFactory, showTakstolInfoActionFactory, yearWeek,
						vismaFileCreator);
	...
	public JXTable getTablePostShipment(WindowInterface window) {
		...
		ColorHighlighter pattern = new ColorHighlighter(new PatternPredicate("Ja", 6), ColorEnum.GREEN.getColor(),
				null);
	...
	private class ReportAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			WindowInterface window = new JDialogAdapter(
					new JDialog(ProTransMain.PRO_TRANS_MAIN, "Transport statistikk"));
	...
	public class PostShipmentTableModel extends AbstractTableAdapter {
		...
		public PostShipmentTableModel(ListModel listModel) {
			super(listModel,
					new String[] { "Kunde", "Ordrenr", "Adresse", "Postnummer", "Poststed", "Type",
							// "Kommentar",
							"Klar", "Produktområde" });
		}
	...
	void setTransport(final boolean setPostShipment, final WindowInterface window) {
		...
		if (transport != Transport.UNKNOWN && !transportIsSent(transport, window) && isWeekValid(transport, window)
			...
			setTransportForTransportable(transportable, transport, window, getTransportableList(setPostShipment),
	...
	private int getSelectionIndex(final boolean setPostShipment) {
		return setPostShipment ? tablePostShipment.convertRowIndexToModel(postShipmentSelectionList.getSelectionIndex())
				: orderViewHandler.getOrderPanelSelectedOrderIndex();
	}
	...
	private void setTransportForTransportable(final Transportable transportable, final Transport transport,
			final WindowInterface window, final List<Transportable> transportableList, final int index) {
		try {
			...
			try {
				...
				if (postShipment != null) {
					if (Util.showConfirmDialog(window.getComponent(), "Slette?",
							"Vil du virkelig slette ettersending?")) {
	...
	private class ExcelAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent arg0) {
			if (transportWeekViewHandler.getUseListView()) {
				...
				ExcelUtil.showTableDataInExcel(excelDirectory, fileName, null, "Transportliste",
						transportWeekViewHandler.getTableOrdersList(), null, null, 16, false);
			} else {
	...
	private class ExcelGenerator implements Threadable {
		...
		public void doWhenFinished(Object object) {
			if (object != null) {
				...
				Util.showMsgDialog(window.getComponent(), "Excel",
	...
	class FilterActionListener implements ActionListener {
		...
		public FilterActionListener(WindowInterface window) {
			this.window = window;
		}
	...
	private void showDeviation(WindowInterface aWindow) {
		PostShipment postShipment = (PostShipment) postShipmentSelectionList
				.getElementAt(tablePostShipment.convertRowIndexToModel(postShipmentSelectionList.getSelectionIndex()));
