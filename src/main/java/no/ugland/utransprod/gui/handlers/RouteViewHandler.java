...
public class RouteViewHandler implements Closeable, Updateable, ListDataListener, ProductAreaGroupProvider, OrderNrProvider {
     ...
     */
    @Inject
    public RouteViewHandler(final OrderViewHandlerFactory aOrderViewHandlerFactory, final Login aLogin, final ManagerRepository aManagerRepository,
	    final DeviationViewHandlerFactory aDeviationViewHandlerFactory, ShowTakstolInfoActionFactory aShowTakstolInfoActionFactory,
	...
	postShipmentSelectionList = new SelectionInList((ListModel) postShipmentList);

	initMenus();

	presentationModelTransportSum = new PresentationModel(new TransportSumVModel(new TransportSumV(Integer.valueOf(0), BigDecimal.valueOf(0),
		null)));
	...
	buttonDeletePostShipment = new JButton(new DeletePostShipmentAction(window));
	...
	postShipmentSelectionList.addPropertyChangeListener(SelectionInList.PROPERTYNAME_SELECTION_EMPTY, selectionEmpyHandler);
	...
	return BasicComponentFactory.createLabel(presentationModelTransportSum.getModel(TransportSumVModel.PROPERTY_ORDER_COUNT_STRING));
    }
	...
	return BasicComponentFactory.createLabel(presentationModelTransportSum.getModel(TransportSumVModel.PROPERTY_GARAGE_COST_STRING));
    }
	...
	checkBoxListView.setSelected(transportWeekViewHandler != null ? transportWeekViewHandler.getUseListView() : false);
	...
	if (transportWeekViewHandler != null) {
		...
		if (Util.showConfirmDialog(window.getComponent(), "Lagre?", "Det er gjort endringer, ønsker du å lagre?")) {
		    canClose = false;
		} else {
	...
	if (transportWeekView == null) {
		...
		transportWeekViewHandler = new TransportWeekViewHandler(login, managerRepository, transportViewHandler, orderViewHandlerFactory,
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (event.getPropertyName().equalsIgnoreCase("year")) {
			...
			if (canClose(null, window)) {
			    internalWeekChange = true;
			    transportWeekView.changeWeek((Integer) event.getNewValue());
			    buttonSave.setEnabled(false);
			    internalWeekChange = false;
			    setTransportSum();
			    checkBoxFilter.setSelected(true);
	...
	ColorHighlighter pattern = new ColorHighlighter(new PatternPredicate("Ja", 6), ColorEnum.GREEN.getColor(), null);
	...
	tablePostShipment.setModel(new PostShipmentTableModel(postShipmentSelectionList));
	...
	tablePostShipment.setColumnControl(new UBColumnControlPopup(tablePostShipment, this));
	...
	for (PostShipment postShipment : postShipmentLines) {
		...
		orderViewHandler.lazyLoadOrder(postShipment.getOrder(), new LazyLoadOrderEnum[] { LazyLoadOrderEnum.COMMENTS });
	...
	public void actionPerformed(ActionEvent arg0) {
	    ...
	    ReportConstraintView reportConstraintView = new ReportConstraintView(reportConstraintViewHandler);

	    WindowInterface window = new JDialogAdapter(new JDialog(ProTransMain.PRO_TRANS_MAIN, "Transport statistikk"));
	...
	public PostShipmentTableModel(ListModel listModel) {
	    super(listModel, new String[] { "Kunde", "Ordrenr", "Adresse", "Postnummer", "Poststed", "Type",
		    // "Kommentar",
		    "Klar", "Produktområde" });
	}
	...
	return setPostShipment ? new RightClickListener(popupMenuPostShipment) : new RightClickListener(popupMenuSetTransportOrder);
    }
	...
	public void mouseClicked(MouseEvent e) {
	    ...
	    if (SwingUtilities.isRightMouseButton(e) && !transportWeekViewHandler.useListView()) {
	...
	if (transport != Transport.UNKNOWN && !transportIsSent(transport, window) && isWeekValid(transport, window)
	    ...
	    setTransportForTransportable(transportable, transport, window, getTransportableList(setPostShipment), getSelectionIndex(setPostShipment));
	}
    ...
    @SuppressWarnings("unchecked")
    private List<Transportable> getTransportableList(final boolean setPostShipment) {
	return setPostShipment ? postShipmentList : orderViewHandler.getOrderPanelList();
    }

    private int getSelectionIndex(final boolean setPostShipment) {
	return setPostShipment ? tablePostShipment.convertRowIndexToModel(postShipmentSelectionList.getSelectionIndex()) : orderViewHandler
	...
	SelectionInList selectionInList = setPostShipment ? postShipmentSelectionList : orderViewHandler.getOrderPanelSelectionList();
	 ...
	 */
	public void actionPerformed(ActionEvent actionEvent) {
	    if (actionEvent.getActionCommand().equalsIgnoreCase(menuItemSetTransportPostShimpment.getText())) {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (postShipment != null) {
		    if (Util.showConfirmDialog(window.getComponent(), "Slette?", "Vil du virkelig slette ettersending?")) {
			...
			if (deviation != null) {
			    ...
			    managerRepository.getPostShipmentManager().removeObject(postShipment);
			}
	...
	String orderNr = JOptionPane.showInputDialog(parentWindow.getComponent(), "Ordrenummer:");
	Order order = null;
	...
	if (orderNr != null) {
	    ...
	    if (order == null) {
		if (!Util.showConfirmFrame(parentWindow, "Ordre ikke funnet", "Ordre ble ikke funnet, vil du fortsette?")) {
		    return;
		}
	    } else {
	    ...
	    if (!Util.showConfirmFrame(parentWindow, "Ordrenr ikke satt", "Ordrenr er ikke satt, vil du fortsette?")) {
	...
	EditDeviationView editDeviationView = new EditDeviationView(false, deviationModel, deviationViewHandler, true, true);
	...
	List<Transport> transportList = managerRepository.getTransportManager().findByObject(transport);
	...
	public void propertyChange(PropertyChangeEvent arg0) {
	    if (hasWriteAccess()) {
		if (buttonDeletePostShipment != null && postShipmentSelectionList != null) {
		    buttonDeletePostShipment.setEnabled(postShipmentSelectionList.hasSelection());
		}
	    }
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		Map<Transport, TransportViewHandler> transportHandlers = transportWeekViewHandler.getTransportViewHandlers();
		...
		Collection<ListTransport> selectedTransports = (Collection<ListTransport>) Util.showOptionsDialog(window, listTransports, "Velg",
			true, true);
		if (selectedTransports != null && selectedTransports.size() != 0) {
			 ...
			 */
			models.put(listTransport.getTransport(), transportHandlers.get(listTransport.getTransport())
	 ...
	 * @param aWindow
	 */
	public ExcelGenerator(Map<Transport, TransportOrderTableModel> someModels, WindowInterface aWindow) {
	    models = someModels;
	    window = aWindow;
	}
	...
	public void doWhenFinished(Object object) {
		...
		Util.showMsgDialog(window.getComponent(), "Excel", "Dersom excel ikke starter opp automatisk, ligger fil under katalog excel");
	    }
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (!Hibernate.isInitialized(order.getOrderComments())) {
		    orderViewHandler.lazyLoadOrder(order, new LazyLoadOrderEnum[] { LazyLoadOrderEnum.COMMENTS });
		}
     ...
     * @param window
     */
    public static void showContentForPostShipment(PostShipment postShipment, WindowInterface window) {
	PostShipmentManager postShipmentManager = (PostShipmentManager) ModelUtil.getBean("postShipmentManager");
	if (postShipment != null) {
	...
	public String toString() {
		...
		return transport.toString() + " " + Util.SHORT_DATE_FORMAT.format(transport.getLoadingDate());
	    }
