 ...
 * @author atle.brekka
 */
public class ProductionOverviewViewHandler extends DefaultAbstractViewHandler<Order, OrderModel>
		implements ProductAreaGroupProvider, OrderNrProvider, ProduceableProvider {
	...
	@Inject
	public ProductionOverviewViewHandler(final VismaFileCreator aVismaFileCreator,
	...
	protected AbstractEditView<OrderModel, Order> getEditView(AbstractViewHandler<Order, OrderModel> handler,
			Order object, boolean searching) {
		return null;
	}
		...
		TID_VEGG("Tid vegg") {
			...
			public Object getValue(Transportable transportable, Map<String, String> statusMap,
				...
				if (Order.class.isInstance(transportable)) {
					...
					return vegg.getRealProductionHours() == null
							? Tidsforbruk.beregnTidsforbruk(vegg.getActionStarted(), vegg.getProduced())
							: vegg.getRealProductionHours();
				}
				return null;
			}
		...
		TID_GAVL("Tid gavl") {
			...
			public Object getValue(Transportable transportable, Map<String, String> statusMap,
				...
				if (Order.class.isInstance(transportable)) {
					...
					return gavl.getRealProductionHours() == null
							? Tidsforbruk.beregnTidsforbruk(gavl.getActionStarted(), gavl.getProduced())
							: gavl.getRealProductionHours();
				}
				return null;
			}
		...
		MONTERING("Montering") {
			...
			public Object getValue(Transportable transportable, Map<String, String> statusMap,
				...
				if (transportable.getDoAssembly() != null && transportable.getDoAssembly() == 1) {
					returnString = "M" + (transportable.getAssembly() == null ? ""
							: transportable.getAssembly().getAssemblyWeek() == null ? ""
		...
		ØNSKET_LEVERING("Ønsket levering") {
			...
			public Object getValue(Transportable transportable, Map<String, String> statusMap,
				...
				if (transportable.getTakstolKjopOrd() != null) {
					Integer forventetLevering = transportable.getTakstolKjopOrd().getDeldt() == null ? 0
							: transportable.getTakstolKjopOrd().getDeldt();
					Integer bekreftetLevering = transportable.getTakstolKjopOrd().getCfdeldt() == null ? 0
							: transportable.getTakstolKjopOrd().getCfdeldt();
	...
	public JXTable getTable(WindowInterface window) {
		...
		table.getColumnModel().getColumn(ProductionColumn.ORDRE.ordinal())
		...
		table.getColumnModel().getColumn(ProductionColumn.PROCENT.ordinal())
				.setCellRenderer(new TextPaneRendererProcentDone());
	...
	protected void handleFilter() {
		...
		if (group != ProductAreaGroup.UNKNOWN) {
			if (!group.getProductAreaGroupName().equalsIgnoreCase("Takstol")) {
				filterList.add(new PatternFilter(group.getProductAreaGroupName(), Pattern.CASE_INSENSITIVE,
						ProductionColumn.PRODUKTOMRÅDE.ordinal()));
			} else {
				filterList
						.add(new PatternFilter(".*e.*", Pattern.CASE_INSENSITIVE, ProductionColumn.TAKSTOL.ordinal()));
			}
