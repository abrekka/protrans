 ...
 * @author atle.brekka
 */
public class PackageViewHandler extends
		AbstractProductionPackageViewHandler<PackableListItem> {
	 ...
	 */
	@Inject
	public PackageViewHandler(Login login, ManagerRepository managerRepository,
			DeviationViewHandlerFactory deviationViewHandlerFactory,
			@Assisted ApplyListInterface<PackableListItem> packageInterface,
			@Assisted String title,
			@Assisted TableEnum tableEnum, @Assisted final String aMainArticleName) {
		super(login, managerRepository, deviationViewHandlerFactory,
				packageInterface, title, tableEnum);
	...
	protected void searchOrder(String orderNr, String customerNr,
		...
		try {
			...
			if (packable != null) {
				...
				table.getSelectionModel().setAnchorSelectionIndex(
	...
	protected TableModel getTableModelReport() {
		return applyListInterface.getTableModelReport(objectSelectionList,
				table, getObjectSelectionList());
	}
	...
	public PackableListItem getApplyObject(final Transportable transportable,
		...
		applyable = applyable != null ? applyable : applyListInterface
