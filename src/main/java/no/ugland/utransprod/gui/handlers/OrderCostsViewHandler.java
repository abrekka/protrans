...
public class OrderCostsViewHandler implements CostChangeListener {
	 ...
	 */
	public OrderCostsViewHandler(PresentationModel aPresentationModel,
			Login aLogin, boolean doAddInternalCost, boolean isTableEditable,
	...
	public void openCostView(WindowInterface window, OrderCost selectedOrderCost) {
		OrderCostViewHandler orderCostViewHandler = new OrderCostViewHandler(
				login, managerRepository);
		OrderCostView orderCostView = new OrderCostView(orderCostViewHandler,
				selectedOrderCost);
	...
	private static final class CostTableModel extends AbstractTableAdapter {
		...
		private static final String[] COLUMNS = { "Kostnad", "Betales av",
				"Beløp", "Mva", "Av." };
		 ...
		 * @param aPresentationModel
		 */
		CostTableModel(ListModel listModel, boolean isEditable,
				PresentationModel aPresentationModel) {
	...
	public void initEventHandling(WindowInterface window) {
		getCostSelectionList().addPropertyChangeListener(
