 ...
 * @param <T>
 */
public abstract class AbstractProductionPackageViewHandler<T extends Applyable> implements Closeable, Updateable, ProductAreaGroupProvider {
    ...
    private JButton buttonRealProductionHours;
    ...
    @Inject
    public AbstractProductionPackageViewHandler(final Login aLogin, ManagerRepository aManagerRepository,
	    DeviationViewHandlerFactory aDeviationViewHandlerFactory, @Assisted final ApplyListInterface<T> productionInterface,
	...
	objectSelectionList.addPropertyChangeListener(SelectionInList.PROPERTYNAME_SELECTION_INDEX, new SelectionListener());
	...
	List<ProductAreaGroup> groups = managerRepository.getProductAreaGroupManager().findAll();
	if (groups != null) {
     ...
     * @param window
     */
    protected abstract void setApplied(T object, boolean applied, WindowInterface window);
     ...
     * @param window
     */
    protected abstract void searchOrder(String orderNr, String customerNr, WindowInterface window);
     ...
     */
    public abstract T getApplyObject(Transportable transportable, WindowInterface window);
	...
	buttonUnapply = new JButton(new ApplyAction(getUnapplyText(), false, window));
    ...
    public final JButton getButtonRealProductionHours() {
	buttonRealProductionHours = new JButton(new RealProductionHoursAction());
	buttonRealProductionHours.setEnabled(false);
	buttonRealProductionHours.setName("ButtonAdjustTime");
	return buttonRealProductionHours;
    }
	...
	table.getColumnModel().getColumn(getOrderInfoCell()).setCellRenderer(getOrderCellRenderer());
	int specCell = getSpecificationCell();
	if (specCell != -1) {
	    table.getColumnModel().getColumn(specCell).setCellRenderer(getSpecificationCellRenderer());
	}
	...
	if (startColumn != null) {
	    ColorHighlighter patternStart = new ColorHighlighter(new PatternPredicate("[^---]", startColumn), ColorEnum.YELLOW.getColor(),
		    Color.BLACK);
	    table.addHighlighter(patternStart);
	}

	ColorHighlighter pattern = new ColorHighlighter(new PatternPredicate("[^---]", getApplyColumn()), ColorEnum.GREEN.getColor(), Color.BLACK);
	...
	BigDecimal overstyrtTidsforbruk = StringUtils.isBlank(overstyrtTidsforbrukString) ? null : new BigDecimal(overstyrtTidsforbrukString);
	 ...
	 * @param aWindow
	 */
	public ApplyAction(final String buttonText, final boolean applied, final WindowInterface aWindow) {
	    super(buttonText);
	    window = aWindow;
	    isApplied = applied;
	}
    ...
    private class RealProductionHoursAction extends AbstractAction {
	private static final long serialVersionUID = 1L;

	public RealProductionHoursAction() {
	    super("Sett reell tidsforbruk");
	}
	...
	if (hasWriteAccess()) {
	    ...
	    buttonRealProductionHours.setEnabled(hasSelection);

	    if (hasSelection) {
		int selectionIndex = objectSelectionList.getSelectionIndex();
		if (selectionIndex < table.getRowCount()) {
		    object = (T) objectSelectionList.getElementAt(table.convertRowIndexToModel(selectionIndex));
	...
	public void actionPerformed(final ActionEvent arg0) {
	    ...
	    Util.runInThreadWheel(window.getRootPane(), new Printer(window, useDialog), null);
	...
	public Object doWork(final Object[] params, final JLabel labelInfo) {
		...
		window = new JDialogAdapter(new JDialog((JDialog) owner.getComponent(), getWindowTitle()));
	    } else if (owner instanceof JFrameAdapter) {
		window = new JDialogAdapter(new JDialog((JFrame) owner.getComponent(), getWindowTitle()));
	    } else {
		window = new JInternalFrameAdapter(new JInternalFrame(getWindowTitle(), true, true, true, true));
	    ...
	    try {
		reportViewer.generateProtransReport(getTableModelReport(), getWindowTitle(), getReportEnum(), null);
		window.pack();
	...
	if (objects != null && objects.size() != 0) {
	    ...
	    if (objects.size() > 1) {
		object = (T) JOptionPane.showInputDialog(window.getComponent(), "Velg ordre", "Velg ordre", JOptionPane.QUESTION_MESSAGE, null,
	    ...
	    JOptionPane.showMessageDialog(window.getComponent(), "Ordre ble ikke funnet");
	}
	return object;
    }
	 ...
	 */
	public void actionPerformed(final ActionEvent arg0) {
	    Applyable applyable = (Applyable) objectSelectionList.getElementAt(table.convertRowIndexToModel(objectSelectionList.getSelectionIndex()));
	    ...
	    if (order != null) {
		DeviationViewHandler deviationViewHandler = deviationViewHandlerFactory.create(order, true, false, true, null, false);
	...
	public void actionPerformed(final ActionEvent arg0) {
	    ...
	    Order order = managerRepository.getOrderManager().findByOrderNr(applyable.getOrderNr());
	    if (order != null) {
	...
	return (ProductAreaGroup) productAreaGroupModel.getValue(ProductAreaGroupModel.PROPERTY_PRODUCT_AREA_GROUP);
    }
