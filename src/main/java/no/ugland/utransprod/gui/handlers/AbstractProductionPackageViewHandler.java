...
public abstract class AbstractProductionPackageViewHandler<T extends Applyable>
	...
	public AbstractProductionPackageViewHandler(final Login aLogin, ManagerRepository aManagerRepository,
			...
			@Assisted final ApplyListInterface<T> productionInterface, @Assisted final String title,
			@Assisted final TableEnum aTableEnum) {
		...
		objectSelectionList.addPropertyChangeListener(SelectionInList.PROPERTYNAME_SELECTION_INDEX,
	...
	public final JXTable getTable(final WindowInterface window) {
		...
		if (startColumn != null) {
			ColorHighlighter patternStart = new ColorHighlighter(new PatternPredicate("[^---]", startColumn),
					ColorEnum.YELLOW.getColor(), Color.BLACK);
			table.addHighlighter(patternStart);
		}

		ColorHighlighter pattern = new ColorHighlighter(new PatternPredicate("[^---]", getApplyColumn()),
				ColorEnum.GREEN.getColor(), Color.BLACK);
	...
	final void setRealProductionHours() {
		...
		String overstyrtTidsforbrukString = Util.showInputDialogWithdefaultValue(null, "Sett reell tidsforbruk",
				"Tidsforbruk:", realProductionHours == null ? "" : String.valueOf(realProductionHours));

		BigDecimal overstyrtTidsforbruk = StringUtils.isBlank(overstyrtTidsforbrukString) ? null
	...
	protected void handleFilter() {
		...
		if (group != null) {
			PatternFilter filterProductAreaGroup = new PatternFilter(group.getProductAreaGroupName(),
					Pattern.CASE_INSENSITIVE, getProductAreaColumn());
	...
	private class AddAccidentAction extends AbstractAction {
		...
		public void actionPerformed(final ActionEvent arg0) {
			...
			accidentViewHandler.openEditView(new Accident(), false, window, false);
			Util.setDefaultCursor(window.getComponent());
		}
	}
	...
	final T getSearchObject(final WindowInterface window, final List<T> objects) {
		...
		if (objects != null && objects.size() != 0) {
			...
			if (objects.size() > 1) {
				object = (T) JOptionPane.showInputDialog(window.getComponent(), "Velg ordre", "Velg ordre",
