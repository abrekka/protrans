...
public class TransportView {
     ...
     * @param transport
     */
    public TransportView(final TransportViewHandler handler, final Transport transport) {
	viewHandler = handler;
	viewHandler.setPresentationModel(new PresentationModel(new TransportModel(transport)));
    }
     ...
     * @param number
     */
    private void initComponents(final WindowInterface window, final int number, TransportWeekViewHandler transportWeekViewHandler,
	...
	tableOrders = viewHandler.getTableOrders(window, number, transportWeekViewHandler, transportWeekView);
     ...
     * @return panel
     */
    public final Component buildPanel(final WindowInterface window, final int number, TransportWeekViewHandler transportWeekViewHandler,
	    TransportWeekView transportWeekView) {// ,
	// ProductAreaGroup
	// productAreaGroup)
	// {
	initComponents(window, number, transportWeekViewHandler, transportWeekView);
	viewHandler.initEventHandling();

	int numberOfOrders = tableOrders.getRowCount();
	int tableSize = numberOfOrders * 25 + 25;

	FormLayout layout = new FormLayout("fill:100dlu:grow", "10dlu,p,3dlu,fill:" + tableSize + "dlu");
