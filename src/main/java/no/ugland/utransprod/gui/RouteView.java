...
public class RouteView implements Viewer {
	...
	private JComboBox<String> comboBoxTransportfirma;
	...
	private void initComponents(WindowInterface window) {
		...
		textFieldRed = new JTextField("ikke opplastet");
		...
		buttonUpdateStatus = viewHandler.getUpdateStatusButton(window);
		...
		buttonDeletePostShipment = viewHandler.getButtonDeletePostShipment(window);
		...
		comboBoxTransportfirma = viewHandler.getComboBoxTransportfirma(window);
	...
	private void initEventHandling(WindowInterface window) {
		tablePostShipment.addMouseListener(viewHandler.getRightClickListener(true));
	}

	private JPanel buildFilterPanel() {
		FormLayout layout = new FormLayout("p,3dlu,30dlu,3dlu,p,3dlu,p,3dlu,25dlu,3dlu,p", "p,3dlu,p,3dlu,10dlu");
		...
		builder.add(ButtonBarFactory.buildLeftAlignedBar(buttonSearchOrder, buttonReport), cc.xyw(1, 3, 10));
	...
	private JPanel buildLeftPane(WindowInterface window) {
		FormLayout layout = new FormLayout("p:grow", "p,3dlu,fill:p:grow,3dlu,fill:p:grow");
		PanelBuilder builder = new PanelBuilder(layout);
		// PanelBuilder builder = new PanelBuilder(new FormDebugPanel(),layout);
		CellConstraints cc = new CellConstraints();
	...
	private JPanel buildInfoPanel() {
		FormLayout layout = new FormLayout("p,3dlu,p,3dlu,15dlu,3dlu,p,3dlu,35dlu,3dlu,28dlu,3dlu,35dlu,p,p,p,p,3dlu,p",
				"p,3dlu,p");
		PanelBuilder builder = new PanelBuilder(layout);
		// PanelBuilder builder = new PanelBuilder(new FormDebugPanel(),layout);
		CellConstraints cc = new CellConstraints();
		...
		builder.add(buttonUpdateStatus, cc.xy(17, 1));
		builder.add(comboBoxTransportfirma, cc.xy(19, 1));
		builder.add(buildColorInfoPanel(), cc.xyw(1, 3, 17));

		return builder.getPanel();
	}

	private JPanel buildColorInfoPanel() {
		FormLayout layout = new FormLayout("55dlu,3dlu,55dlu,3dlu,55dlu,3dlu,55dlu", "p");
	...
	public Component buildPanel(WindowInterface window) {
		...
		FormLayout layout = new FormLayout("10dlu,fill:p:grow", "10dlu,fill:p:grow,3dlu,p,3dlu");
		PanelBuilder builder = new PanelBuilder(layout);
		// PanelBuilder builder = new PanelBuilder(new FormDebugPanel(),layout);
		CellConstraints cc = new CellConstraints();
		builder.add(splitPane, cc.xy(2, 2));
		builder.add(ButtonBarFactory.buildCenteredBar(buttonExcel, buttonSave, buttonRefresh, buttonCancel),
				cc.xy(2, 4));
	...
	private Component buildOrderPanel(WindowInterface window) {
		return viewHandler.getOrderPanelView().buildPanel(window, "200", viewHandler.getRightClickListener(false),
				"240", true);
	}
	...
	private Component buildPostShipmentPanel(WindowInterface window) {
		FormLayout layout = new FormLayout("240dlu:grow", "p,3dlu,fill:120dlu:grow,3dlu,p");
		PanelBuilder builder = new PanelBuilder(layout);
		CellConstraints cc = new CellConstraints();

		builder.addLabel("Ettersendinger:", cc.xy(1, 1));
		builder.add(new JScrollPane(tablePostShipment), cc.xy(1, 3));
		builder.add(ButtonBarFactory.buildCenteredBar(buttonAddPostShipment, buttonDeletePostShipment), cc.xy(1, 5));
