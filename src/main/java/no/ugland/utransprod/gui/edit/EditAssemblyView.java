...
public class EditAssemblyView extends AbstractEditView<AssemblyModel, Assembly> {
	 ...
	 * @param aViewHandler
	 */
	public EditAssemblyView(final boolean searchDialog,
	...
	protected final JComponent buildEditPanel(final WindowInterface window) {
		FormLayout layout = new FormLayout(
				"10dlu,p,3dlu,p,3dlu,p,3dlu,55dlu,10dlu",
	 ...
	 */
	@Override
	protected final Validator getValidator(final AssemblyModel object,
			boolean search) {
		return null;
	}
	...
	protected final void initEditComponents(final WindowInterface window1) {
		comboBoxPlanned = ((SupplierOrderViewHandler) viewHandler)
				.getComboBoxPlanned(presentationModel);
		checkBoxAssemblied = ((SupplierOrderViewHandler) viewHandler)
				.getCheckBoxAssemblied(presentationModel);
		listAreaComment = ((SupplierOrderViewHandler) viewHandler)
				.getListAreaComment(presentationModel);
		dateChooser = ((SupplierOrderViewHandler) viewHandler)
				.getDateChooser(presentationModel);
		buttonAddComment = ((SupplierOrderViewHandler) viewHandler)
				.getButtonAddComment(presentationModel);
	}
