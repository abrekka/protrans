...
public class EditUserTypeView extends AbstractEditView<UserTypeModel, UserType> {
	 ...
	 * @param aViewHandler
	 */
	public EditUserTypeView(boolean searchDialog, UserType userType,
			AbstractViewHandler<UserType, UserTypeModel> aViewHandler) {
	...
	protected JComponent buildEditPanel(WindowInterface window) {
		FormLayout layout = new FormLayout("10dlu,p,3dlu,170dlu,10dlu",
				"10dlu,p,3dlu,p,3dlu,p,3dlu,p,3dlu,fill:200dlu:grow,3dlu,p,3dlu");
	...
	protected void initComponentAnnotations() {
		...
		ValidationComponentUtils.setMessageKey(textFieldDescription,
				"Brukertype.beskrivelse");

		ValidationComponentUtils.setMandatory(comboBoxStartupWindow, true);
		ValidationComponentUtils.setMessageKey(comboBoxStartupWindow,
				"Brukertype.oppstartsvindu");
	...
	protected void initEditComponents(WindowInterface aWindow) {
		textFieldDescription = ((UserTypeViewHandler) viewHandler)
				.getTextFieldDescription(presentationModel);
		comboBoxStartupWindow = ((UserTypeViewHandler) viewHandler)
				.getComboBoxStartupWindow(presentationModel);
		checkBoxAdmin = ((UserTypeViewHandler) viewHandler)
				.getCheckBoxAdmin(presentationModel);
		tableWindows = ((UserTypeViewHandler) viewHandler)
				.getTableWindows(presentationModel);
