...
public class EditDeviationView extends AbstractEditView<DeviationModel, Deviation> implements Viewer {
    ...
    public EditDeviationView(final boolean searchDialog, final AbstractModel<Deviation, DeviationModel> object,
	    final AbstractViewHandler<Deviation, DeviationModel> aViewHandler, final boolean onlyOk, final boolean doAddInternalCost) {
	...
	OrderArticleView<Deviation, DeviationModel> orderArticleView = new OrderArticleView<Deviation, DeviationModel>(
		((DeviationViewHandler) viewHandler).getOrderArticleViewHandler(presentationModel, search, window), true, false);
	...
	// PanelBuilder builder = new PanelBuilder(new FormDebugPanel(),layout);
	CellConstraints cc = new CellConstraints();
	OrderCostsViewHandler orderCostsViewHandler = ((DeviationViewHandler) viewHandler).getOrderCostsViewHandler(presentationModel,
		addInternalCost, true);
	...
	tabbedPane.setTitleAt(1, "Andre avvik(" + ((DeviationViewHandler) viewHandler).getNumberOfOtherDeviations() + ")");
	...
	comboBoxDeviationFunction = ((DeviationViewHandler) viewHandler).getComboBoxDeviationFunction(presentationModel);
	textFieldCustomerNr = ((DeviationViewHandler) viewHandler).getTextFieldCustomerNr(presentationModel);
	textFieldCustomerName = ((DeviationViewHandler) viewHandler).getTextFieldCustomerName(presentationModel, search);
	textFieldOrderNr = ((DeviationViewHandler) viewHandler).getTextFieldOrderNr(presentationModel, search);
	textFieldOrderComplete = ((DeviationViewHandler) viewHandler).getTextFieldOrderComplete(presentationModel, search);
	textFieldTransportDate = ((DeviationViewHandler) viewHandler).getTextFieldTransportDate(presentationModel, search);
	comboBoxStatus = ((DeviationViewHandler) viewHandler).getComboBoxStatus(presentationModel);
	dateChooserProcedureCheck = ((DeviationViewHandler) viewHandler).getDateChooserProcedureCheck(presentationModel);
	...
	buttonDeviationOk = ((DeviationViewHandler) viewHandler).getButtonOk(window1, validationResultModel, presentationModel);
	...
	buttonAddPreventiveAction = ((DeviationViewHandler) viewHandler).getButtonAddPreventiveAction(presentationModel, window1);
