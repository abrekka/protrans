...
public class EditOrderView extends AbstractEditView<OrderModel, Order> {
	private boolean lettvekt = false;
	...
	public EditOrderView(final OrderViewHandler handler, final Order order, final boolean searchDialog,
		...
		super(searchDialog, new OrderModel(order, searchDialog, true, true, project.getProjectNumber(),
				project.getProjectName(), erLettvekt), handler);
		this.lettvekt = erLettvekt;
	...
	protected final void initEditComponents(final WindowInterface window1) {
		...
		textFieldTelephoneNr = BasicComponentFactory
				.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_TELEPHONE_NR), !search);
		...
		textFieldSalesman = BasicComponentFactory
				.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_SALESMAN));
	...
	private void createOrderFields(final WindowInterface window1) {
		textFieldOrderNr = BasicComponentFactory
				.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_ORDER_NR), !search);
		...
		textFieldAddress = BasicComponentFactory
				.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_DELIVERY_ADDRESS), !search);
		textFieldAddress.setName("Address");
		orderViewHandler.addEditComponent(textFieldAddress);

		textFieldPostalCode = BasicComponentFactory
				.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_POSTAL_CODE), !search);
		textFieldPostalCode.setName("PostalCode");
		orderViewHandler.addEditComponent(textFieldPostalCode);

		textFieldPostoffice = BasicComponentFactory
				.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_POST_OFFICE), !search);
	...
	private void createCustomerFields() {
		textFieldCustomerNr = BasicComponentFactory
				.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_CUSTOMER_NR), !search);
		...
		textFieldCustomerFirstName = BasicComponentFactory
				.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_CUSTOMER_FIRST_NAME), !search);
		...
		textFieldCustomerLastName = BasicComponentFactory
				.createTextField(presentationModel.getBufferedModel(OrderModel.PROPERTY_CUSTOMER_LAST_NAME), !search);
	...
	private void addOrderNrChangeListener(final WindowInterface window1) {
		...
		if (((OrderModel) presentationModel.getBean()).getOrderId() == null && !search) {
			presentationModel.getBufferedModel(OrderModel.PROPERTY_ORDER_NR)
					.addValueChangeListener(((OrderViewHandler) viewHandler).getOrderNrChangeListener(window1));
		}
	}
	...
	protected final JComponent buildEditPanel(final WindowInterface window) {
		...
		layout = new FormLayout("10dlu,145dlu,3dlu,150dlu,3dlu," + tabbedPaneSize + ",10dlu",
				"10dlu,p,3dlu,fill:220dlu:grow,3dlu,15dlu,3dlu," + commentPaneSize + ",5dlu:grow,p:grow,5dlu");
		PanelBuilder builder = new PanelBuilder(layout);
	...
	private JPanel buildProductionPanel() {
		...
		builder.add(ButtonBarFactory.buildCenteredBar(buttonImportCuttingFile, buttonOpenCuttingFile),
				cc.xyw(1, 22, 7));
