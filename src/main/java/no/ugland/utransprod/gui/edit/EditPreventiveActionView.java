 ...
 * 
 */
public class EditPreventiveActionView extends AbstractEditView<PreventiveActionModel, PreventiveAction> {
     ...
     * @param aViewHandler
     */
    public EditPreventiveActionView(boolean searchDialog, PreventiveAction object, PreventiveActionViewHandler aViewHandler) {
	...
	textFieldProjectNr = ((PreventiveActionViewHandler) viewHandler).getTextFieldProjectNr(presentationModel);
	textFieldManager = ((PreventiveActionViewHandler) viewHandler).getTextFieldManager(presentationModel);
	comboBoxFunction = ((PreventiveActionViewHandler) viewHandler).getComboBoxFunction(presentationModel);
	comboBoxCategory = ((PreventiveActionViewHandler) viewHandler).getComboBoxCategory(presentationModel);
	textAreaDescription = ((PreventiveActionViewHandler) viewHandler).getTextAreaDescription(presentationModel);
	textAreaExpectedOutcome = ((PreventiveActionViewHandler) viewHandler).getTextAreaExpectedOutcome(presentationModel);
	listComments = ((PreventiveActionViewHandler) viewHandler).getListComments(presentationModel);
	buttonAddComment = ((PreventiveActionViewHandler) viewHandler).getButtonAddComment(aWindow, presentationModel);
	checkBoxClosed = ((PreventiveActionViewHandler) viewHandler).getCheckBoxClosed(presentationModel, aWindow);
	textFieldName = ((PreventiveActionViewHandler) viewHandler).getTextFieldName(presentationModel);
	buttonPrint = ((PreventiveActionViewHandler) viewHandler).getButtonPrint(presentationModel);
    }
	...
	FormLayout layout = new FormLayout("10dlu,50dlu,center:3dlu,center:220dlu,10dlu",
	...
	builder.addLabel("Forventet resultat:", cc.xyw(2, 8, 3));
	builder.add(new JScrollPane(textAreaExpectedOutcome), cc.xywh(2, 10, 3, 1));
	builder.addLabel("Historikk:", cc.xy(2, 12));
	builder.add(new JScrollPane(listComments), cc.xywh(2, 14, 3, 1));
	builder.add(ButtonBarFactory.buildLeftAlignedBar(buttonAddComment), cc.xyw(2, 16, 3));
	builder.add(ButtonBarFactory.buildCenteredBar(buttonPrint, buttonSave, buttonCancel), cc.xyw(1, 18, 5));
	...
	FormLayout layout = new FormLayout("p,3dlu,p", "p,3dlu,p,3dlu,p,3dlu,p,3dlu,p,3dlu,p");
	PanelBuilder builder = new PanelBuilder(layout);
     ...
     */
    @Override
    protected Validator getValidator(PreventiveActionModel object, boolean search) {
	...
	ValidationComponentUtils.setMessageKey(textFieldName, "Prevantivt tiltak.prosjektnavn");
