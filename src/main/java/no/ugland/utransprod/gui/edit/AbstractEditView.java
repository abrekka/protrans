 ...
 * @param <T>
 */
public abstract class AbstractEditView<E, T> implements Closeable, Updateable, CancelListener, EditViewable {
    /**
     * Lagreknapp
     */
    JButton buttonSave;
     ...
     * @param aViewHandler
     */
    public AbstractEditView(final boolean searchDialog, final AbstractModel<T, E> object, final AbstractViewHandler<T, E> aViewHandler) {
	...
	if (search) {
	    ...
	    buttonCancel = new CancelButton(window, this, false, Util.getGuiProperty("button.cancel.text"), IconEnum.ICON_CANCEL, this, true);
	} else {
	...
	buttonOk = new CancelButton(window, new OkAction(), true, "Ok", IconEnum.ICON_OK, null, true);
	...
	Validator validator = getValidator(currentObject.getBufferedObjectModel(presentationModel), search);
	if (validator != null) {
	...
	// if (!search) {
	PropertyConnector connector = new PropertyConnector(presentationModel, PresentationModel.PROPERTYNAME_BUFFERING, buttonSave, "enabled");
     ...
     */
    public final boolean canClose(final String actionString, final WindowInterface window1) {
	boolean canClose = true;
	if (presentationModel.isBuffering()) {
	    if (Util.showConfirmDialog(window1.getComponent(), "Lagre?", "Det er gjort endringer, skal det lagres?")) {
		canClose = false;
	    }
	}
	if (canClose) {
	...
	if (validationResultModel.hasErrors()) {
	    Util.showErrorDialog((Component) null, "Rett feil", "Rett alle feil før lagring!");
	} else {
	    E object = currentObject.getBufferedObjectModel(presentationModel);
	    CheckObject checkObject = viewHandler.checkSaveObject(object, presentationModel, window1);
		...
		viewHandler.saveObject((AbstractModel) presentationModel.getBean(), window1);
	    }
	}

    }

    private boolean handleCheckObject(WindowInterface window, CheckObject checkObject, String msg) {
	boolean returnValue;
	...
	boolean doSave = Util.showConfirmDialog(window.getComponent(), "Slette?", msg + " Vil du slette?");
	return doSave;
    }
	...
	public void actionPerformed(final ActionEvent arg0) {
	    if (validationResultModel.hasErrors()) {
		Util.showErrorDialog((Component) null, "Mangler søkekriteria", "Legg inn søkekriteria for å søke");
	    } else {
		trigger.triggerCommit();
		searchPressed = true;
		window.dispose();
	...
	public boolean canClose(final String actionString, final WindowInterface window) {
	    if (validationResultModel.hasErrors()) {
		Util.showErrorDialog(window.getComponent(), "Rett feil", "Rett alle feil før lagring!");
		return false;
	    }
	    trigger.triggerCommit();
	    currentObject.viewToModel();
