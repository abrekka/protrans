...
public class TaksteinConverter extends DefaultConverter {
	...
	private String getSendesFraGG(String inf) {
		return inf != null && inf.equalsIgnoreCase(SKARPNES) ? JA : NEI;
	}

	private void setTaksteintype(Ordln ordln, OrderLine orderLine) {
		if (ordln.getDescription() != null
				&& ordln.getDescription().length() != 0) {
			orderLine.setAttributeValue(ATTRIBUTE_TAKSTEINTYPE, ordln
					.getDescription().replaceAll("Takstein:", ""));
		}
	}
