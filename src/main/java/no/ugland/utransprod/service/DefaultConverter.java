...
public class DefaultConverter implements ArticleTypeToOrderLineConverter {
	...
	private void setOrderLineAttributes(final ArticleType articleType, OrderLine orderLine) {
		if (articleType != null) {
			orderLine.setOrderLineAttributes(
					OrderModel.getOrderLinesAttributes(articleType.getArticleTypeAttributes(), orderLine));
		}
	}

	private void setNumberOfItems(final Ordln ordln, OrderLine orderLine) {
		orderLine.setNumberOfItems(
				Util.convertBigDecimalToInteger(ordln != null ? ordln.getNoinvoab() : BigDecimal.ZERO));
	}

	public static void setAttributes(final OrderLine orderLine, OrdlnManager ordlnManager) {
		Set<OrderLineAttribute> attributes = orderLine.getOrderLineAttributes() != null
	...
	private static void setAttributeBoolValue(final OrderLine orderLine, OrdlnManager ordlnManager,
			OrderLineAttribute orderLineAttribute, Attribute attribute) {
		Ordln ordln = attribute.hasProdCatNo()
				? ordlnManager.findByOrderNrProdCatNo(orderLine.getOrderNr(), attribute.getProdCatNo(),
						attribute.getProdCatNo2())
				: Ordln.UNKNOWN;
	...
	private static boolean setChoices(final OrderLine orderLine, final Attribute attribute,
		...
		Set<AttributeChoice> choices = attribute.getAttributeChoices() != null ? attribute.getAttributeChoices()
		...
		while (choiceIt.hasNext() && notFound) {
			...
			hasChoices = true;
			Ordln ordln = choice.hasProdCatNo()
					? ordlnManager.findByOrderNrProdCatNo(orderLine.getOrderNr(), choice.getProdCatNo(),
							choice.getProdCatNo2())
					: Ordln.UNKNOWN;
