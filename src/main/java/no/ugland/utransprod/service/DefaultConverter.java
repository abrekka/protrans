...
public class DefaultConverter implements ArticleTypeToOrderLineConverter {
	...
	public OrderLine convert(final ArticleType articleType, final Ordln ordln,
			final Order order, final Ord ord) {
	...
	private void setAttributeHasValue(final ArticleType articleType,
			OrderLine orderLine) {
		if (articleType != null) {
			orderLine.setOrderLineAttributeValue("Har "
					+ articleType.getArticleTypeName(), "Ja");
		}
	}

	private void setOrderLineAttributes(final ArticleType articleType,
			OrderLine orderLine) {
		if (articleType != null) {
			orderLine.setOrderLineAttributes(OrderModel
					.getOrderLinesAttributes(articleType
							.getArticleTypeAttributes(), orderLine));
		}
	}

	private void setNumberOfItems(final Ordln ordln, OrderLine orderLine) {
		orderLine.setNumberOfItems(Util
				.convertBigDecimalToInteger(ordln != null ? ordln.getNoOrg()
						: BigDecimal.ZERO));
	}

	public static void setAttributes(final OrderLine orderLine,
	...
	private static boolean setChoices(final OrderLine orderLine,
		...
		Set<AttributeChoice> choices = attribute.getAttributeChoices() != null ? attribute
				.getAttributeChoices()
