 ...
 * @author atle.brekka
 */
public class TakstolProductionVManagerImpl extends AbstractApplyListManager<Produceable> implements TakstolProductionVManager {
    ...
    public final List<Produceable> findApplyableByOrderNrAndArticleName(final String orderNr, final String articleName) {
	List<Produceable> items = dao.findByOrderNrAndArticleName(orderNr, articleName);
	if (items.isEmpty()) {
	...
	for (Produceable item : items) {
		...
		// ny
		// ordre
		handleCurrentTakstol(defaultTakstolMap, currentTakstol, relatedArticles, producables);
    ...
    private void handleCurrentTakstol(Map<String, TakstolProductionV> defaultTakstolMap, Produceable currentTakstol, List<Applyable> relatedArticles,
    ...
    private Map<String, TakstolProductionV> addArticleToDefaultTakstol(Produceable item, Map<String, TakstolProductionV> defaultTakstolMap) {
	TakstolProductionV defaultTakstol = defaultTakstolMap.get(item.getOrderNr());
    ...
    private void addRelatedAttributeInfo(TakstolProductionV defaultTakstol, Produceable item) {
    ...
    private List<Applyable> getRelatedArticles(final Produceable produceable, final List<Produceable> items) {
	List<Applyable> relatedArticles = new ArrayList<Applyable>();

	for (Produceable item : items) {
	    if (!item.getArticleName().equalsIgnoreCase(MAIN_ARTICLE_NAME) && item.getOrderNr().equalsIgnoreCase(produceable.getOrderNr())
    ...
    public TakstolProductionV findByOrderNrAndOrderLineId(String orderNr, Integer orderLineId) {
	List<Produceable> list = dao.findByOrderNr(orderNr);
	TakstolProductionV takstolProductionV = findByOrderLineId(orderLineId, list);
	if (takstolProductionV != null) {
	    takstolProductionV
		    .setRelatedArticles(takstolProductionV.getNumberOfItems() != null && takstolProductionV.getNumberOfItems() > 3 ? getRelatedArticles(
			    takstolProductionV, list) : new ArrayList<Applyable>());
	}
	return takstolProductionV;
    }

    private TakstolProductionV findByOrderLineId(Integer orderLineId, List<Produceable> list) {
	...
	List<Produceable> items = dao.findByCustomerNrAndProductAreaGroup(customerNr, productAreaGroup);
	...
	List<Produceable> items = dao.findByOrderNrAndProductAreaGroup(orderNr, productAreaGroup);
	if (items != null && items.size() != 0) {
	    return setRelated(items);
	}
	return items;
	...
	return Lists.newArrayList(transform(dao.findByOrderNr(orderNr), prosucable2TakstolProductionV()));
    }
