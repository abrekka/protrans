...
public class NokkelProduksjonVManagerImpl implements NokkelProduksjonVManager {
     ...
     */
    public final List<NokkelProduksjonV> findByParams(final ExcelReportSetting params) {
	if (params.getExcelReportType() == ExcelReportEnum.PRODUCTIVITY_PACK) {
	    return dao.findByYearWeekProductArea(params.getYear(), params.getWeekFrom(), params.getWeekTo(), params.getProductAreaName());
	}
	return dao.findByYearWeekProductAreaGroup(params.getYear(), params.getWeekFrom(), params.getWeekTo(),
		((ExcelReportSettingOwnProduction) params).getProductAreaGroupName());
	...
	return String.format("For å klare budsjett må det pakkes for %1$.0f hver dag", budget);
    }
