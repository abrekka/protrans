...
public class SalesVManagerImpl implements SalesVManager {
	...
	public final List<SaleReportSum> groupSumCountyByProbabilityProductAreaPeriode(ProbabilityEnum probabilityEnum,
		...
		return salesDataSnapshotDAO.groupSumCountyByProbabilityProductAreaPeriode(probabilityEnum, productArea,
				periode);

	}

	public final List<SaleReportSum> groupSumSalesmanByProbabilityProductAreaPeriode(ProbabilityEnum probabilityEnum,
			ProductArea productArea, Periode periode) throws ProTransException {
		int currentWeek = Util.getCurrentWeek();
		int currentYear = Util.getCurrentYear();
		...
		return salesDataSnapshotDAO.groupSumSalesmanByProbabilityProductAreaPeriode(probabilityEnum, productArea,
				periode);
	...
	public List<?> createReportMapAndGetData(SalesReportType reportType, Periode periode, ProductArea productArea)
	...
	public Multimap<ProbabilityEnum, SaleReportSum> getAllReportData(ProductArea productArea, Periode periode)
			throws ProTransException {
		...
		salesMap.put(ProbabilityEnum.PROBABILITY_CONFRIM_ORDER,
				orderManager.groupSumByProductAreaConfirmPeriode(productArea, periode));
	...
	public Map<ProbabilityEnum, Set<SaleReportData>> getSalesMap(ProductArea productArea, Periode periode)
			throws ProTransException {
		reportMap.clear();
	...
	public Integer countByProbabilityProductAreaPeriode(ProbabilityEnum probabilityEnum, ProductArea productArea,
	...
	private void checkPeriode(Periode periode, int currentWeek, int currentYear) throws ProTransException {
		if (periode.getYear() == currentYear
				&& ((periode.getWeek() == currentWeek && periode.getToWeek() != currentWeek)
						|| (periode.getWeek() != currentWeek && periode.getToWeek() == currentWeek))) {
			throw new ProTransException("Salgsdata for gjeldende uke må genereres for seg selv");
		}
	}

	public List<SaleReportData> getSaleReportByProbabilityProductAreaPeriode(ProbabilityEnum probabilityEnum,
			ProductArea productArea, Periode periode, String typeName) throws ProTransException {
		int currentWeek = Util.getCurrentWeek();
		int currentYear = Util.getCurrentYear();
		checkPeriode(periode, currentWeek, currentYear);
		...
		return salesDataSnapshotDAO.getSaleReportByProbabilityProductAreaPeriode(probabilityEnum, productArea, periode,
				typeName);
	}
	...
	private void setBudgetValues(Map<String, SalesmanGoal> salesmanGoals, Periode periode) {
		for (SalesmanGoal goal : salesmanGoals.values()) {
			Budget budget = budgetManager.findByYearAndSalesman(periode.getYear(), goal.getSalesman(),
					goal.getProductArea(), BudgetType.SALESMAN);
	...
	private void generateForProductAreaAndReportType(Periode periode, Map<String, SalesmanGoal> salesmanGoals,
	...
	private void generateForEachReportType(Periode periode, ProductArea productArea,
			Map<String, SalesmanGoal> salesmanGoals, PeriodeEnum periodeEnum) throws ProTransException {
		for (SalesReportType reportType : SalesReportType.getSalesGoalReportTypes()) {
			...
			if (periode.getToWeek() == 0) {
				...
				List<SaleReportSum> saleReportSumList = (List<SaleReportSum>) createReportMapAndGetData(reportType,
						periode, productArea);
				for (SaleReportSum saleReportSum : saleReportSumList) {
					...
					if (salesmanGoal != null) {
						...
						salesmanGoal.setValue(reportType.getInfoTop(), saleReportSum.getSumOwnProduction(),
								periodeEnum);
	...
	private SalesmanGoal getSalesmanGoal(ProductArea productArea, Map<String, SalesmanGoal> salesmanGoals,
		...
		salesmanGoal = salesmanGoal != null ? salesmanGoal
				: periodeEnum.salesmanShouldExist() ? null : new SalesmanGoal();
		return salesmanGoal;
	}
	...
	public SaleReportSum getGroupSumByProbabilityProductAreaPeriod(ProbabilityEnum probability, ProductArea productArea,
			Periode periode) throws ProTransException {
		int currentWeek = Util.getCurrentWeek();
		int currentYear = Util.getCurrentYear();
		checkPeriode(periode, currentWeek, currentYear);
