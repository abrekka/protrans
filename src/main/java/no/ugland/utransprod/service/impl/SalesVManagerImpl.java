...
public class SalesVManagerImpl implements SalesVManager {
	...
	private static Map<ProbabilityEnum, Set<SaleReportData>> reportMap = com.google.common.collect.Maps
			.newHashMap();
	...
	public final List<SaleReportSum> groupSumCountyByProbabilityProductAreaPeriode(
			ProbabilityEnum probabilityEnum, ProductArea productArea,
		...
		if (currentYear == periode.getYear()
				&& currentWeek == periode.getWeek()) {
			return dao.groupSumCountyByProbabilityProductAreaPeriode(
					probabilityEnum, productArea, periode);
		}
		return salesDataSnapshotDAO
				.groupSumCountyByProbabilityProductAreaPeriode(probabilityEnum,
						productArea, periode);
	...
	public final List<SaleReportSum> groupSumSalesmanByProbabilityProductAreaPeriode(
		...
		if (currentYear == periode.getYear()
				&& currentWeek == periode.getWeek()) {
			return dao.groupSumSalesmanByProbabilityProductAreaPeriode(
					probabilityEnum, productArea, periode);
		}
		return salesDataSnapshotDAO
				.groupSumSalesmanByProbabilityProductAreaPeriode(
						probabilityEnum, productArea, periode);
	...
	public List<?> findByParams(ExcelReportSetting params)
		...
		SalesReportType reportType = SalesReportType.valueOf(params
				.getExcelReportType().name());
		return createReportMapAndGetData(reportType, params.getPeriode(),
				params.getProductArea());
	}

	public List<?> createReportMapAndGetData(SalesReportType reportType,
			Periode periode, ProductArea productArea) throws ProTransException {

		if (reportType == SalesReportType.SALES_REPORT
				|| reportType == SalesReportType.SALES_REPORT_SALESMAN) {
			createReportMap(productArea, periode);
		}
		return reportType
				.getData(reportMap, orderManager, productArea, periode);
	}

	public Multimap<ProbabilityEnum, SaleReportSum> getAllReportData(
			ProductArea productArea, Periode periode) throws ProTransException {
		
		Multimap<ProbabilityEnum, SaleReportSum> salesMap = ArrayListMultimap
		.create();
		...
		for (SalesReportType reportType : SalesReportType.values()) {
			if (reportType.isPartOfSalesStatistic()) {
				salesMap.putAll(reportType.getProbability(),
						(List<SaleReportSum>) reportType.getData(reportMap,
								orderManager, productArea, periode));
			}
		}*/
	...
	private void createReportMap(ProductArea productArea, Periode periode)
			throws ProTransException {
	...
	public Map<ProbabilityEnum, Set<SaleReportData>> getSalesMap(
			ProductArea productArea, Periode periode) throws ProTransException {
		reportMap.clear();
		...
		List<SaleReportData> salesDataList = getSaleReportByProductAreaPeriode(
				productArea, periode);
		...
		for (SaleReportData sale : salesDataList) {
			...
			sale.getProbabilityEnum().addToSalesReportMap(reportMap, sale,
					periode);
		}
		return reportMap;
	}
	...
	public String getInfoButtom(ExcelReportSetting params)
		...
		SalesReportType reportType = SalesReportType.valueOf(params
				.getExcelReportType().name());
		return reportType.getInfoButtom(params.getProductArea(), params
				.getPeriode(), this, orderManager);
	}
	...
	public Integer countByProbabilityProductAreaPeriode(
			ProbabilityEnum probabilityEnum, ProductArea productArea,
			Periode periode) throws ProTransException {
		int currentWeek = Util.getCurrentWeek();
		int currentYear = Util.getCurrentYear();
		...
		if (currentYear == periode.getYear()
				&& currentWeek == periode.getWeek()) {
			return dao.countByProbabilityProductAreaPeriode(probabilityEnum,
					productArea, periode);
		}
		return salesDataSnapshotDAO.countByProbabilityProductAreaPeriode(
				probabilityEnum, productArea, periode);
	}
	...
	private void checkPeriode(Periode periode, int currentWeek, int currentYear)
		...
		if (periode.getYear() == currentYear
			...
			throw new ProTransException(
					"Salgsdata for gjeldende uke må genereres for seg selv");
		}
	}

	public List<SaleReportData> getSaleReportByProbabilityProductAreaPeriode(
			ProbabilityEnum probabilityEnum, ProductArea productArea,
		...
		if (currentYear == periode.getYear()
				&& currentWeek == periode.getWeek()) {
			return dao.getSaleReportByProbabilityProductAreaPeriode(
					probabilityEnum, productArea, periode, typeName);
		}
		return salesDataSnapshotDAO
				.getSaleReportByProbabilityProductAreaPeriode(probabilityEnum,
						productArea, periode, typeName);
	}
	...
	public List<SaleReportData> getSaleReportByProductAreaPeriode(
		...
		return salesDataSnapshotDAO.getSaleReportByProductAreaPeriode(
				productArea, periode);
	}
	...
	public List<SalesmanGoal> getSalesGoalList(Periode periode)
			throws ProTransException {
		Map<String, SalesmanGoal> salesmanGoals = Maps.newHashMap();

		for (PeriodeEnum periodeEnum : PeriodeEnum.values()) {
			generateForProductAreaAndReportType(periodeEnum
					.convertPeriode(periode), salesmanGoals, periodeEnum);

		}

		setBudgetValues(salesmanGoals, periode);

		List<SalesmanGoal> salemanGoalList = Lists.newArrayList(salesmanGoals
				.values());
	...
	private void generateForProductAreaAndReportType(Periode periode,
			Map<String, SalesmanGoal> salesmanGoals, PeriodeEnum periodeEnum)
			throws ProTransException {
		for (ProductArea productArea : productAreaManager.findAll()) {
			generateForEachReportType(periode, productArea, salesmanGoals,
					periodeEnum);
		}
	}

	@SuppressWarnings("unchecked")
	private void generateForEachReportType(Periode periode,
			ProductArea productArea, Map<String, SalesmanGoal> salesmanGoals,
			PeriodeEnum periodeEnum) throws ProTransException {
		for (SalesReportType reportType : SalesReportType
				.getSalesGoalReportTypes()) {
			// første uka i året
	...
	public SaleReportSum getGroupSumByProbabilityProductAreaPeriod(
		...
		if (currentYear == periode.getYear()
				&& currentWeek == periode.getWeek()) {
			return dao.getGroupSumByProbabilityProductAreaPeriod(
					probability, productArea,
					periode);
		}
		return salesDataSnapshotDAO.getGroupSumByProbabilityProductAreaPeriod(
				probability, productArea,
				periode);
	}
