 ...
 * @author atle.brekka
 */
public class TakstolPackageVManagerImpl extends
		AbstractApplyListManager<PackableListItem> implements
	...
	private void handleCurrentTakstol(
			Map<String, TakstolPackageV> defaultTakstolMap,
			PackableListItem currentTakstol, List<Applyable> relatedArticles,
			List<PackableListItem> producables) {
	...
	private Map<String, TakstolPackageV> addArticleToDefaultTakstol(
			PackableListItem item,
			Map<String, TakstolPackageV> defaultTakstolMap) {
		TakstolPackageV defaultTakstol = defaultTakstolMap.get(item
				.getOrderNr());
	...
	private void addRelatedAttributeInfo(TakstolPackageV defaultTakstol,
			PackableListItem item) {
	...
	private boolean haveNotRelatedTakstol(PackableListItem item) {
		return !item.getArticleName().equalsIgnoreCase(MAIN_ARTICLE_NAME)
				&& !haveRelatedTakstol(item) ? true : false;
	}

	private boolean haveRelatedTakstol(PackableListItem item) {
		List<PackableListItem> list = dao.findApplyableByOrderNrAndArticleName(
				item.getOrderNr(), MAIN_ARTICLE_NAME);
		return list != null && list.size() != 0;
	}

	private void setRelatedArticles(List<PackableListItem> items,
			PackableListItem item) {
		List<Applyable> relatedArticles = checkAndGetRelatedArticles(item,
				items);
		item.setRelatedArticles(relatedArticles);
	}

	private List<Applyable> checkAndGetRelatedArticles(PackableListItem item,
			List<PackableListItem> items) {

		return item.getNumberOfItems() != null && item.getNumberOfItems() > 3 ? getRelatedArticles(
				item, items)
	...
	private boolean canHaveRelatedArticles(PackableListItem item) {
		return item.getArticleName().equalsIgnoreCase(MAIN_ARTICLE_NAME)
				&& item.getNumberOfItems() != null
				&& item.getNumberOfItems() > 3 ? true : false;
	}

	private List<Applyable> getRelatedArticles(final PackableListItem packable,
			final List<PackableListItem> items) {
		List<Applyable> relatedArticles = new ArrayList<Applyable>();

		for (PackableListItem item : items) {
			if (!item.getArticleName().equalsIgnoreCase(MAIN_ARTICLE_NAME)
					&& item.getOrderNr()
							.equalsIgnoreCase(packable.getOrderNr())
	 ...
	 */
	public final List<PackableListItem> findApplyableByOrderNr(
	 ...
	 */
	public final List<PackableListItem> findApplyableByCustomerNr(
	...
	public TakstolPackageV findByOrderNrAndOrderLineId(String orderNr,
			Integer orderLineId) {
		...
		if (takstolPackageV != null) {
			takstolPackageV
					.setRelatedArticles(takstolPackageV.getNumberOfItems() != null
							&& takstolPackageV.getNumberOfItems() > 3 ? getRelatedArticles(
							takstolPackageV, list)
							: new ArrayList<Applyable>());
		}
		return takstolPackageV;
	}

	private TakstolPackageV findByOrderLineId(Integer orderLineId,
			List<PackableListItem> list) {
	...
	public List<PackableListItem> findApplyableByOrderNrAndArticleName(
			String orderNr, String mainArticleName) {
		List<PackableListItem> items = dao
				.findApplyableByOrderNrAndArticleName(orderNr, mainArticleName);
		return setRelated(items);
	}

	public List<PackableListItem> findApplyableByCustomerNrAndProductAreaGroup(
			Integer customerNr, ProductAreaGroup productAreaGroup) {
		List<PackableListItem> items = dao.findByCustomerNrAndProductAreaGroup(
				customerNr, productAreaGroup);
		return setRelated(items);
	}

	public List<PackableListItem> findApplyableByOrderNrAndProductAreaGroup(
			String orderNr, ProductAreaGroup productAreaGroup) {
		List<PackableListItem> items = dao.findByOrderNrAndProductAreaGroup(
				orderNr, productAreaGroup);
		return setRelated(items);
	}
