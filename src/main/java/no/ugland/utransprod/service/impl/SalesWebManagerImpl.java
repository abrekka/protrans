...
public class SalesWebManagerImpl implements SalesWebManager {
    ...
    public SalesStatistic generateSalesStatistics(Integer year, Integer week, String productAreaName) throws ProTransException {
	...
	Multimap<ProbabilityEnum, SaleReportSum> salesMap = salesVManager.getAllReportData(productArea, periode);
	...
	if (week > 1) {
	    ...
	    salesStatistic = setAccumulatedNumberOfAndSumOrder(salesMap, salesStatistic);
    ...
    private void debugSalesStatistic(SalesStatistic salesStatistic, String productAreaName, Integer week) {
	LOGGER.debug("************************* PROTRANS *********************************");
	...
	LOGGER.debug("      AntallTilbud: " + salesStatistic.getNumberOfOffer());
	LOGGER.debug("      AntallOrdre: " + salesStatistic.getNumberOfOrder());
	LOGGER.debug("      AntallAvrop: " + salesStatistic.getNumberOfConfirmedOrder());
	LOGGER.debug("**********************************************************");
	/*
    ...
    private SalesStatistic setOrderReserve(SalesStatistic salesStatistic, ProductArea productArea) {
	BigDecimal orderReserve = BigDecimal.ZERO;
	for (OrderReserveV reserve : orderReserveVManager.findByProductArea(productArea.getProductArea())) {
	    orderReserve = orderReserve.add(reserve.getOwnProduction());
	}
	SaleStatusOrderReserveV salesReserve = saleStatusOrderReserveVManager.findByProductArea(productArea);
	orderReserve = orderReserve.add(salesReserve.getOrderReserve());
	salesStatistic.setOrderReserve(orderReserve);
	return salesStatistic;
    }

    private SalesStatistic setSalesBudget(Integer year, Integer week, ProductArea productArea, SalesStatistic salesStatistic) {
	Budget salesBudget = budgetManager.findByYearAndWeek(year, week, productArea, BudgetType.SALE);
	salesStatistic.setSalesBudget(salesBudget.getBudgetValue());
	Periode periode = new Periode(year, 1, week);
	salesBudget = budgetManager.findSumPrProductAreaAndPeriode(periode, productArea, BudgetType.SALE);
	salesStatistic.setAccumulatedSaleBudget(salesBudget.getBudgetValue());
	return salesStatistic;
    }

    private SalesStatistic setAccumulatedNumberOfAndSumOrder(Multimap<ProbabilityEnum, SaleReportSum> salesMap, SalesStatistic salesStatistic) {
	List<SaleReportSum> salesList = (List<SaleReportSum>) salesMap.get(ProbabilityEnum.PROBABILITY_OFFER);
	if (salesList.size() != 0) {
	    salesStatistic.setAccumulatedNumberOfOffer(salesList.get(0).getOrderCount() + salesStatistic.getNumberOfOffer());
	    // salesStatistic.setAccumulatedNumberOfOffer(salesList.get(0).getOrderCount());
	}

	salesList = (List<SaleReportSum>) salesMap.get(ProbabilityEnum.PROBABILITY_ORDER);
	if (salesList.size() != 0) {
	    salesStatistic.setAccumulatedNumberOfOrder(salesList.get(0).getOrderCount() + salesStatistic.getNumberOfOrder());
	    // salesStatistic.setAccumulatedNumberOfOrder(salesList.get(0).getOrderCount());
	    salesStatistic.setAccumulatedSumOwnProduction(salesList.get(0).getSumOwnProduction().add(salesStatistic.getSumOwnProuction()));

	    salesStatistic.setAccumulatedSumDG(getAccumulatedSumDG(salesList.get(0), salesStatistic));
	}

	salesList = (List<SaleReportSum>) salesMap.get(ProbabilityEnum.PROBABILITY_CONFRIM_ORDER);
	if (salesList.size() != 0) {
	    salesStatistic.setAccumulatedNumberOfConfirmedOrder(salesList.get(0).getOrderCount() + salesStatistic.getNumberOfConfirmedOrder());
    ...
    private BigDecimal getAccumulatedSumDG(SaleReportSum yearSum, SalesStatistic weekStatistic) {
	BigDecimal sumDb = yearSum.getSumDb().add(weekStatistic.getSumDB());
	BigDecimal sumOwnProduction = yearSum.getSumOwnProduction().add(weekStatistic.getSumOwnProuction());
	if (sumDb != null && sumOwnProduction != null && sumOwnProduction.intValue() != 0) {
	    return sumDb.divide(sumOwnProduction, 4, RoundingMode.HALF_EVEN);
	}
	return BigDecimal.ZERO;
    }

    private SalesStatistic setNumberOfAndSumOrder(Multimap<ProbabilityEnum, SaleReportSum> salesMap, SalesStatistic salesStatistic) {
	List<SaleReportSum> salesList = (List<SaleReportSum>) salesMap.get(ProbabilityEnum.PROBABILITY_OFFER);
	if (salesList.size() != 0) {
	...
	if (salesList.size() != 0) {
	    ...
	    salesStatistic.setSumOwnProduction(salesList.get(0).getSumOwnProduction());
	...
	salesList = (List<SaleReportSum>) salesMap.get(ProbabilityEnum.PROBABILITY_CONFRIM_ORDER);
	if (salesList.size() != 0) {
	    salesStatistic.setNumberOfConfirmedOrder(salesList.get(0).getOrderCount());
	}
    ...
    public void setOrderReserveVManager(OrderReserveVManager orderReserveVManager) {
    ...
    public void setSaleStatusOrderReserveVManager(SaleStatusOrderReserveVManager saleStatusOrderReserveVManager) {
	...
	public Builder orderReserveVManager(OrderReserveVManager orderReserveVManager) {
	    salesWebManagerImpl.orderReserveVManager = orderReserveVManager;
	    return this;
	}

	public Builder saleStatusOrderReserveVManager(SaleStatusOrderReserveVManager saleStatusOrderReserveVManager) {
