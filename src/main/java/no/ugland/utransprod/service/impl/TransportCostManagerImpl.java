...
public class TransportCostManagerImpl implements TransportCostManager {
	...
	private void importAddition(final int row, final ExcelUtil excelUtil) {
		...
		transportCostAddition
				.setMemberOfMaxAdditions(excelUtil.readCellAsInteger(row, COLUMN_UPDATE_MEMBER_OF_MAX_ADDITIONS));
	...
	private void importOrUpdateTransportCosts(final TransportCostUpdateable updateable, final ExcelUtil excelUtil)
	...
	private void importZoneAdditionAssembly(final TransportCostUpdateable updateable, final ExcelUtil excelUtil)
			throws ProTransException {
	...
	private List<TransportCostBasis> createTransportCostList(final Map<Supplier, List<Transportable>> supplierList,
	...
	private TransportCostBasis createTransportCostForOrders(final List<Transportable> transportables,
			final Supplier supplier, final Periode period) throws ProTransException {
	...
	private void checkTransportableAndCalculateCost(final Supplier supplier, final Periode period,
			final TransportCostBasis transportCostBasis, final Transportable transportable) throws ProTransException {
	...
	private OrderCost calculateTransportCostForTransportable(final Transportable transportable, final Supplier supplier,
			final Periode period) throws ProTransException {
	...
	private OrderCost getOrderCost(final Transportable transportable, final BigDecimal basisCost,
			final Map<ITransportCostAddition, BigDecimal> additionalCost, final Supplier supplier,
			final String orderCostComment) throws ProTransException {
		OrderCost orderCost = null;
		StringBuilder costDetails = new StringBuilder();
		BigDecimal additionValue = BigDecimal.valueOf(0);

		if (additionalCost != null) {
			additionValue = generateCostDetailsAndGetAdditionValue(basisCost, additionalCost, costDetails,
					additionValue);
		}
	...
	private BigDecimal generateCostDetailsAndGetAdditionValue(final BigDecimal basisCost,
			final Map<ITransportCostAddition, BigDecimal> additionalCost, final StringBuilder costDetails,
			final BigDecimal aAdditionValue) {
	...
	private Map<Supplier, List<Transportable>> createSupplierList(final List<Transportable> transportables)
			throws ProTransException {
	...
	private void addOrderToSupplierList(final Map<Supplier, List<Transportable>> supplierList,
