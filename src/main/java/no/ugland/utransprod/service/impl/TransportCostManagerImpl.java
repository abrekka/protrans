...
import no.ugland.utransprod.service.impl.TransportCostManagerImpl.Builder;
...
public class TransportCostManagerImpl implements TransportCostManager {
	...
	public final void setTransportCostAdditionManager(
			final TransportCostAdditionManager aTransportCostAdditionManager) {
		transportCostAdditionManager = aTransportCostAdditionManager;
	}

	public final void setTransportManager(
			final TransportManager aTransportManager) {
	...
	public final void setTransportCostBasisManager(
			final TransportCostBasisManager aTransportCostBasisManager) {
	...
	public final List<TransportCostBasis> generateTransportCostList(
		...
		if (transportableList != null && transportableList.size() != 0) {
			...
			transportCostBasisList = createTransportCostList(supplierList,
					period);
			transportCostBasisManager
					.saveTransportCostBasisList(transportCostBasisList);
		}
		return transportCostBasisList;

	}

	private List<TransportCostBasis> removeEmptyTransportCostBasis(
		...
		if (transportCostBasisList != null) {
			for (TransportCostBasis basis : transportCostBasisList) {
				if (basis.getOrderCosts() == null
						|| basis.getOrderCosts().size() == 0) {
	...
	private List<Transportable> getTransportableList(final Periode periode)
			throws ProTransException {
	...
	private List<Transportable> getTransportables(final Transport transport)
			throws ProTransException {
	...
	private void importAdditions(final ExcelUtil excelUtil) {
		int row = 1;
		String desc = excelUtil.readCell(row,
				COLUMN_UPDATE_ADDITION_DESCRIPTION, null);
		while (desc != null) {
			importAddition(row, excelUtil);
			row++;
			desc = excelUtil.readCell(row, COLUMN_UPDATE_ADDITION_DESCRIPTION,
					null);
		}
	}
	...
	private void importOrUpdateTransportCosts(
			final TransportCostUpdateable updateable, final ExcelUtil excelUtil)
			throws ProTransException {
		int row = 1;
	...
	final void updatePriceForTransportCosts(final int row,
			final ExcelUtil excelUtil) {
		String postalCodeFrom = excelUtil.readCell(row,
				COLUMN_UPDATE_POSTAL_CODE_FROM, "%1$04.0f");
		String postalCodeTo = excelUtil.readCell(row,
				COLUMN_UPDATE_POSTAL_CODE_TO, "%1$04.0f");
		BigDecimal price = excelUtil.readCellAsBigDecimal(row,
				COLUMN_UPDATE_PRICE);
		BigDecimal addition = excelUtil.readCellAsBigDecimal(row,
				COLUMN_UPDATE_ADDITION);
		Integer maxAddition = excelUtil.readCellAsInteger(row,
				COLUMN_UPDATE_MAX_ADDITION);

		dao.updatePriceForPostalCodes(postalCodeFrom, postalCodeTo, price,
				maxAddition, addition);
	}

	private interface TransportCostUpdateable {
		void updateTransportCost(final int row, final ExcelUtil excelUtil)
				throws ProTransException;
	}
	...
	private List<TransportCostBasis> createTransportCostList(
			final Map<Supplier, List<Transportable>> supplierList,
			final Periode period) throws ProTransException {
		...
		for (Supplier supplier : suppliers) {
			transportCostBasisList.add(createTransportCostForOrders(
					supplierList.get(supplier), supplier, period));
		}
	...
	private void removeFailedTransportCostBasis(
		...
		if (transportCostBasis.getTransportCostBasisId() != null) {
			...
			transportCostBasisManager
					.removeTransportCostBasis(transportCostBasis);
		}
	}
	...
	private OrderCost calculateTransportCostForTransportable(
		...
		if (transportCost != null) {
			additionalCost = transportCostAdditionManager
					.calculateCostAddition(transportable, transportCost, period);
		}

		return getOrderCost(transportable, cost, additionalCost, supplier,
				orderCostComment);
	...
	private BigDecimal generateCostDetailsAndGetAdditionValue(
			final BigDecimal basisCost,
			final Map<ITransportCostAddition, BigDecimal> additionalCost,
			final StringBuilder costDetails, final BigDecimal aAdditionValue) {
		...
		if (additionalCost != null) {
			...
			for (ITransportCostAddition addition : keySet) {
				...
				costDetails.append(addition.getInfo()).append(":").append(
						value.setScale(2)).append(",");
	...
	private Map<Supplier, List<Transportable>> createSupplierList(
			final List<Transportable> transportables) throws ProTransException {
	...
	private void addOrderToSupplierList(
			final Map<Supplier, List<Transportable>> supplierList,
			final Transportable transportable, final Supplier supplier) {
	...
	private void importAreas(final ExcelUtil excelUtil)
			throws ProTransException {
		int row = 1;
		...
		while (areaCode != null) {
			...
			row++;
			areaCode = excelUtil.readCell(row, COLUMN_AREA_AREA_CODE,
					"%1$04.0f");
		}
	}
	...
	private void importCounties(final ExcelUtil excelUtil) {
		int row = 1;
		String countyNr = excelUtil.readCell(row, COLUMN_COUNTY_COUNTY_NR,
				"%1$04.0f");
		...
		while (countyNr != null) {
			...
			row++;
			countyNr = excelUtil.readCell(row, COLUMN_COUNTY_COUNTY_NR,
					"%1$04.0f");
		}
	}
	...
	public static class Builder {
		...
		public Builder transportCostBasisManager(
		...
		public Builder transportCostAdditionManager(
