...
public class SumAvvikVManagerImpl implements SumAvvikVManager {
	...
	setDeviationForPeriode(paramsDeviation.getYear(), paramsDeviation.getMonthEnum().getMonth(), paramsDeviation.getProductArea()
	...
	setDeviationForPeriode(paramsDeviation.getYear() - 1, paramsDeviation.getMonthEnum().getMonth(), paramsDeviation.getProductArea()
    ...
    private void setDeviationOverview(final ExcelReportSettingDeviation paramsDeviation, final Map<Object, Object> map) {
	Map<MonthEnum, Map<String, SumAvvikV>> monthDeviations = new Hashtable<MonthEnum, Map<String, SumAvvikV>>();

	List<MonthEnum> months = MonthEnum.getMonthList();
	for (MonthEnum month : months) {
	    addMonthDeviations(paramsDeviation.getYear(), paramsDeviation.getProductArea().getProductArea(), month, monthDeviations);
	}

	map.put("DeviationList", monthDeviations);
    }

    private void setClosedPreventiveActionsThisPeriode(final ExcelReportSettingDeviation paramsDeviation, final Map<Object, Object> map) {
	List<PreventiveAction> preventiveActionList;
	preventiveActionList = preventiveActionDAO.findAllClosedInMonth(paramsDeviation.getMonthEnum().getMonth());
	...
	List<PreventiveAction> preventiveActionList = preventiveActionDAO.findAllOpen();
    ...
    private void setDeviationForPeriode(final Integer year, final Integer month, final String productArea, final Map<Object, Object> map,
	    final String keyString) {
	List<SumAvvikV> deviations = dao.findByProductAreaYearAndMonth(year, month, productArea);
    ...
    private void setDeviationTotal(final ExcelReportSettingDeviation paramsDeviation, final Map<Object, Object> map) {
	List<SumAvvikV> deviations;
	deviations = dao.findSumByProductAreaYearAndMonth(paramsDeviation.getYear(), paramsDeviation.getMonthEnum().getMonth(), paramsDeviation
		.getProductArea().getProductArea());
     ...
     */
    private void addMonthDeviations(final Integer year, final String productArea, final MonthEnum month,
	    final Map<MonthEnum, Map<String, SumAvvikV>> monthDeviations) {
	List<SumAvvikV> deviations = dao.findByProductAreaYearAndMonthWithClosed(year, month.getMonth(), productArea);
	...
	if (deviations != null) {
		...
		count++;
		if (!jobFunctionName.equalsIgnoreCase(deviation.getJobFunctionName()) && count != 1) {
		    if (currentDeviationOpen != null && currentDeviationClosed != null) {
			currentDeviationOpen.setClosedCount(currentDeviationClosed.getDeviationCount());
			deviationMap.put(currentDeviationOpen.getJobFunctionName(), currentDeviationOpen);
		    } else if (currentDeviationClosed != null) {
			currentDeviationClosed.setClosedCount(currentDeviationClosed.getDeviationCount());
			currentDeviationClosed.setDeviationCount(0);
			deviationMap.put(currentDeviationClosed.getJobFunctionName(), currentDeviationClosed);
