...
public class OrderManagerImpl extends ManagerImpl<Order> implements OrderManager {
	...
	List<Object[]> list = ((OrderDAO) dao).getOrdersPostShipmentsAndCosts(fromString, toString, transportConstraintEnum, productArea);

	if (list != null && list.size() != 0) {
	    TakstolInfoVManager takstolInfoVManager = (TakstolInfoVManager) ModelUtil.getBean(TakstolInfoVManager.MANAGER_NAME);
	    list.addAll(takstolInfoVManager.getBeregnetTidForOrdre(fromString, toString, transportConstraintEnum, productArea.getProductArea()));
    ...
    private void addReportDataTransportToList(final ReportDataTransport reportDataTransport, final List<ReportDataTransport> reportDataTransportList,
	...
	Map<String, Integer> numberOfOrders = ((OrderDAO) dao).getCountOrderAndPostShipment(fromString, toString, transportConstraintEnum,
		productArea);
	List<Object[]> list = ((OrderDAO) dao).countOrderPostShipmentAndCosts(fromString, toString, transportConstraintEnum, productArea);
	TakstolInfoVManager takstolInfoVManager = (TakstolInfoVManager) ModelUtil.getBean(TakstolInfoVManager.MANAGER_NAME);
	...
	if (list != null && list.size() != 0) {
	    List<Object[]> beregnetTid = takstolInfoVManager.summerArbeidsinnsats(fromString, toString, transportConstraintEnum,
    ...
    private int addColliesToReadyCount(final List<ReadyCount> readyCountList, final Order order, final int rows, final List<Colli> collies,
	    final List<OrderLine> missing) {
	int rowCounter = rows;
    ...
    private int addColliToReadyCount(final List<ReadyCount> readyCountList, final Order order, final int rows, final List<Colli> collies,
    ...
    private int addMissingsToReadyCount(final List<ReadyCount> readyCountList, final Order order, final int rows, final List<Colli> collies,
	    final List<OrderLine> missing) {
	int rowCounter = rows;
    ...
    private int addMissingToReadyCount(final List<ReadyCount> readyCountList, final Order order, final int rowCounter, final List<OrderLine> missing,
	    final Colli colli) {
	...
	List<Order> orders = ((OrderDAO) dao).findSentInPeriodByProductArea(params.getYear(), params.getWeekFrom(), params.getWeekTo(),
    ...
    public List<Order> findByConfirmWeekProductAreaGroup(Integer year, Integer weekFrom, Integer weekTo, ProductAreaGroup productAreaGroup) {
