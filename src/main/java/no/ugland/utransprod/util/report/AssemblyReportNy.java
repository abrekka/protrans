...
public class AssemblyReportNy {
	...
	private static final String ORGNR_GARASJE = "Org. Nr: NO 970 953 302 MVA";
	private static final String ORGNR_REKKE = "Org. Nr: NO 998 308 704 MVA";

	private static final String FIRMA_REKKE = "Igland Rekkegarasje AS";

	private static final String FIRMA_GARASJE = "Igland Garasjen AS";

	public AssemblyReportNy(final CraningCostManager aCraningCostManager, final Order order,
			final List<FakturagrunnlagV> fakturagrunnlag) {
		...
		if (getCraningCost() == null) {
			...
			orderManager
					.lazyLoadOrder(order,
							new LazyLoadOrderEnum[] { LazyLoadOrderEnum.ORDER_LINES,
									LazyLoadOrderEnum.ORDER_LINE_ATTRIBUTES, LazyLoadOrderEnum.ORDER_COSTS,
			...
			Craning craning = Craning.with(craningCostManager).trossOrderLine(trossOrderLine)
					.constructionTypeOrderLine(constructionTypeOrderLine).portOrderLine(portOrderLine).build();
	...
	public String getFirma(){
		return "Rekke".equalsIgnoreCase(order.getProductArea().getProductArea()) ? FIRMA_REKKE : FIRMA_GARASJE;
	}
	public String getOrgnr() {
		return "Rekke".equalsIgnoreCase(order.getProductArea().getProductArea()) ? ORGNR_REKKE : ORGNR_GARASJE;
	}
