...
public class GarageTransportLetter extends AbstractTransportLetter {
	...
	protected void addAssemblyInfoToHeading(Transportable transportable,
		...
		if (transportable.getOrder().doAssembly()) {
			heading
					.append(" Garasjen skal monteres av "
							+ (transportable.getAssemblyTeamName() != null ? transportable
									.getAssemblyTeamName()
									: ""));

			if (hasAssemblyData(transportable)) {
				heading.append("(tlf. ").append(
						transportable.getAssembly().getSupplier().getPhone())
						.append(")");
			}
		} else {
	...
	private boolean hasAssemblyData(Transportable transportable) {
		return transportable.getAssembly() != null
				&& transportable.getAssembly().getSupplier() != null
	...
	public IconEnum getLogoIcon() {
		return IconEnum.ICON_UGLAND_GRIMSTAD_GARASJENE;
	}
