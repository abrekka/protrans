...
public final class ApplicationParamUtil {
    ...
    private static Multimap<String, String> colliSetup = null;
     ...
     */
    public static Multimap<String, String> getColliSetup() {
	if (colliSetup == null) {
	    List<ApplicationParam> params = getColliSetupParams();

	    colliSetup = ArrayListMultimap.create();
	    String paramValue;
	...
	if (notPackageList == null) {
	    ...
	    String[] split = notPackageString == null ? null : notPackageString.split(";");
	    if (split != null) {
		notPackageList = new ArrayList<String>(Arrays.asList(split));
	    } else {
		notPackageList = Lists.newArrayList();
	    }
	}
