...
public final class Util {
	 ...
	 * @return true dersom ja
	 */
	private static boolean showOptionFrame(final Component askedBy, final String heading, final String msg,
			final int msgType) {
		askedBy.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
		Object[] options = new Object[] { "Ja", "Nei" };
		int answer = JOptionPane.showInternalOptionDialog(askedBy, msg, heading, JOptionPane.YES_NO_OPTION, msgType,
	...
	public static boolean showConfirmDialog(final Component askedBy, final String title, final String message) {
		...
		int result = JOptionPane.showOptionDialog(askedBy, message, title, JOptionPane.YES_NO_OPTION,
				JOptionPane.QUESTION_MESSAGE, null, options, "Ja");
	...
	public static String showTextAreaInputDialogWithdefaultValue(final WindowInterface window, final String heading,
			final String labelText, final String defaultValue) {
		TextAreaOptionsPaneView optionsPaneView = new TextAreaOptionsPaneView(null, true, false, false, null, labelText,
	...
	private static void showOptionsPane(final WindowInterface window, final String heading,
			final OptionsPaneView optionsPaneView) {
		JDialog dialog = getDialog(window, heading, true);
	...
	private static ArticleTypeManager getArticleTypeManager() {
		return articleTypeManager != null ? articleTypeManager
				: (ArticleTypeManager) ModelUtil.getBean(ArticleTypeManager.MANAGER_NAME);
	}
			...
			if (attributeSplit != null && attributeSplit.length > 1) {
				...
				for (int i = 0; i < size; i++) {
					...
					if (valueSplit != null && valueSplit.length > 1) {
						if (!ArrayUtils.contains(attributtSomIkkeSkalTasMed, valueSplit[0])
								&& buffer.indexOf(valueSplit[0]) == -1) {
	...
	public static Collection<OrderCost> cloneCosts(final Collection<OrderCost> orgCosts) {
		...
		if (orgCosts != null) {
			...
			for (OrderCost orderCost : orgCosts) {
				list.add(new OrderCost(orderCost.getOrderCostId(), orderCost.getOrder(), orderCost.getCostType(),
						orderCost.getCostUnit(), orderCost.getCostAmount(), orderCost.getInclVat(),
						orderCost.getSupplier(), orderCost.getInvoiceNr(), orderCost.getDeviation(),
	...
	public static Map<String, AbstractProductionPackageViewHandler> getProductionPackageHandlers(
			final VismaFileCreator vismaFileCreator, final Login login, OrderViewHandlerFactory orderViewHandlerFactory,
	...
	private static void addGulvsponPackage(final Login login, final ManagerRepository managerRepository,
			...
			final Map<String, AbstractProductionPackageViewHandler> map, VismaFileCreator vismaFileCreator) {
		...
		map.put("Gulvspon",
				(AbstractProductionPackageViewHandler) new PackageViewHandler(login, managerRepository,
						...
						new GulvsponPackageApplyList(login, gulvsponPackageVManager, managerRepository,
								vismaFileCreator),
	...
	@SuppressWarnings("unchecked")
	private static void addTakstolPackage(final Map<String, AbstractProductionPackageViewHandler> map,
			final Login login, ManagerRepository managerRepository,
			DeviationViewHandlerFactory deviationViewHandlerFactory, TakstolPackageApplyList takstolPackageApplyList) {
		AbstractProductionPackageViewHandler abstractProductionPackageViewHandler = (AbstractProductionPackageViewHandler) new TakstolPackageViewHandler(
				...
				ApplicationParamUtil.findParamByName("takstol_artikkel"), managerRepository,
				deviationViewHandlerFactory);
	...
	@SuppressWarnings("unchecked")
	private static void addTakstolProduction(final Map<String, AbstractProductionPackageViewHandler> map,
			final Login login, DeviationViewHandlerFactory deviationViewHandlerFactory,
			ManagerRepository managerRepository, ShowTakstolInfoActionFactory showTakstolInfoActionFactory,
			ArticleType articleTypeTakstol, TakstolProductionApplyList takstolProductionApplyList,
		...
		AbstractProductionPackageViewHandler abstractProductionPackageViewHandler = (AbstractProductionPackageViewHandler) new TakstolProductionViewHandler(
				takstolProductionApplyList, login, managerRepository, deviationViewHandlerFactory,
				showTakstolInfoActionFactory, setProductionUnitActionFactory);
	...
	private static void setProductAreaListForNotAdministrator(final ApplicationUser applicationUser,
		...
		applicationUserManager.lazyLoad(applicationUser,
				new LazyLoadEnum[][] { { LazyLoadEnum.USER_PRODUCT_AREA_GROUPS, LazyLoadEnum.NONE } });
	...
	private static StringBuilder addWordToLine(final int lineSize, final StringBuilder builder, final String[] words,
	...
	private static StringBuilder makeNewLine(final StringBuilder builder, final String[] words,
			final StringBuilder line, final int i) {
	...
	private static void initJobFunctionList() {
		if (jobFunctionList == null) {
			jobFunctionManager = jobFunctionManager != null ? jobFunctionManager
					: (JobFunctionManager) ModelUtil.getBean("jobFunctionManager");
