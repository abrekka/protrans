...
public final class Util {
	...
	int result = JOptionPane.showOptionDialog(askedBy, message, title, JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options,
		"Ja");
	...
	JOptionPane.showInternalMessageDialog(askedBy, message, heading, JOptionPane.INFORMATION_MESSAGE,
	...
	JOptionPane.showMessageDialog(askedBy, message, heading, JOptionPane.INFORMATION_MESSAGE,
		no.ugland.utransprod.gui.IconEnum.ICON_UGLAND.getIcon());
    }
	...
	return new TakstolStatusChecker(takstolArticleType, managerRepository.getTakstolPackageVManager(),
    ...
    public static String removeNoAttributes(final String infoString, String... attributtSomIkkeSkalTasMed) {
	StringBuffer buffer = new StringBuffer();
	if (infoString != null) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!ArrayUtils.contains(attributtSomIkkeSkalTasMed, valueSplit[0]) && buffer.indexOf(valueSplit[0]) == -1) {
	...
	if (orgCosts != null) {
		...
		list.add(new OrderCost(orderCost.getOrderCostId(), orderCost.getOrder(), orderCost.getCostType(), orderCost.getCostUnit(), orderCost
			.getCostAmount(), orderCost.getInclVat(), orderCost.getSupplier(), orderCost.getInvoiceNr(), orderCost.getDeviation(),
			orderCost.getTransportCostBasis(), orderCost.getPostShipment(), orderCost.getComment()));
	    }
	}
	return list;
    }
    ...
    public static Map<String, AbstractProductionPackageViewHandler> getProductionPackageHandlers(final VismaFileCreator vismaFileCreator,
	    final Login login, OrderViewHandlerFactory orderViewHandlerFactory, ManagerRepository managerRepository,
	    DeviationViewHandlerFactory deviationViewHandlerFactory, ShowTakstolInfoActionFactory showTakstolInfoActionFactory,
	    ArticleType articleTypeTakstol, TakstolPackageApplyList takstolPackageApplyList, TakstolProductionApplyList takstolProductionApplyList,
	    SetProductionUnitActionFactory setProductionUnitActionFactory, CostType costTypeTross, CostUnit costUnitTross) {
	...
	addTakstolProduction(map, login, deviationViewHandlerFactory, managerRepository, showTakstolInfoActionFactory, articleTypeTakstol,
		takstolProductionApplyList, setProductionUnitActionFactory);
    ...
    @SuppressWarnings("unchecked")
    private static void addOrderLine(final Login login, final ManagerRepository managerRepository,
	    final DeviationViewHandlerFactory deviationViewHandlerFactory, final Map<String, AbstractProductionPackageViewHandler> map) {
	OrderLineManager orderLineManager = (OrderLineManager) ModelUtil.getBean("orderLineManager");

	map.put("Takstein", (AbstractProductionPackageViewHandler) new PackageViewHandler(login, managerRepository, deviationViewHandlerFactory,
		new TaksteinPackageApplyList(login, orderLineManager, managerRepository), "Pakking takstein", TableEnum.TABLETAKSTEIN,
		ApplicationParamUtil.findParamByName("stein_artikkel")));
    }

    @SuppressWarnings("unchecked")
    private static void addTakstolPackage(final Map<String, AbstractProductionPackageViewHandler> map, final Login login,
	...
	AbstractProductionPackageViewHandler abstractProductionPackageViewHandler = (AbstractProductionPackageViewHandler) new TakstolPackageViewHandler(
		takstolPackageApplyList, login, TableEnum.TABLEPACKAGETAKSTOL, ApplicationParamUtil.findParamByName("takstol_artikkel"),
    ...
    @SuppressWarnings("unchecked")
    private static void addTakstolProduction(final Map<String, AbstractProductionPackageViewHandler> map, final Login login,
	    ...
	    TakstolProductionApplyList takstolProductionApplyList, SetProductionUnitActionFactory setProductionUnitActionFactory) {
	...
	AbstractProductionPackageViewHandler abstractProductionPackageViewHandler = (AbstractProductionPackageViewHandler) new TakstolProductionViewHandler(
		takstolProductionApplyList, login, managerRepository, deviationViewHandlerFactory, showTakstolInfoActionFactory,
		setProductionUnitActionFactory);
	map.put("TakstolProduksjon", abstractProductionPackageViewHandler);
    }

    @SuppressWarnings("unchecked")
    private static void addGavlProduction(final Login login, final Map<String, AbstractProductionPackageViewHandler> map,
	    DeviationViewHandlerFactory deviationViewHandlerFactory, ManagerRepository managerRepository,
	    SetProductionUnitActionFactory setProductionUnitActionFactory) {
	GavlProductionWindow gavlProductionWindow = new GavlProductionWindow(login, managerRepository, deviationViewHandlerFactory,
		setProductionUnitActionFactory);
    ...
    @SuppressWarnings("unchecked")
    private static void addFrontProduction(final Login login, final Map<String, AbstractProductionPackageViewHandler> map,
	    ManagerRepository managerRepository, DeviationViewHandlerFactory deviationViewHandlerFactory,
	    SetProductionUnitActionFactory setProductionUnitActionFactory) {
	FrontProductionWindow frontProductionWindow = new FrontProductionWindow(login, managerRepository, deviationViewHandlerFactory,
		setProductionUnitActionFactory);
    ...
    @SuppressWarnings("unchecked")
    private static void addVeggProduction(final Login login, final Map<String, AbstractProductionPackageViewHandler> map,
	    DeviationViewHandlerFactory deviationViewHandlerFactory, ManagerRepository managerRepository,
	    SetProductionUnitActionFactory setProductionUnitActionFactory) {
	VeggProductionWindow veggProductionWindow = new VeggProductionWindow(login, managerRepository, deviationViewHandlerFactory,
		setProductionUnitActionFactory);
    ...
    @SuppressWarnings("unchecked")
    private static void addPacklist(final Map<String, AbstractProductionPackageViewHandler> map, OrderViewHandlerFactory orderViewHandlerFactory,
	    ManagerRepository managerRepository, DeviationViewHandlerFactory deviationViewHandlerFactory, Login login, CostType costTypeTross,
	    CostUnit costUnitTross) {

	PacklistVManager packlistVManager = (PacklistVManager) ModelUtil.getBean("packlistVManager");

	map.put("Pakkliste", (AbstractProductionPackageViewHandler) new PacklistViewHandler(login, managerRepository, deviationViewHandlerFactory,
		orderViewHandlerFactory, new PacklistApplyList(login, packlistVManager), costTypeTross, costUnitTross));
    }
	...
	if (groupList.size() == 0) {
	    throw new ProTransRuntimeException("Bruker har ikke definert noe produktområde");
	}
	JComboBox comboBox = new JComboBox(new ComboBoxAdapter(groupList,
		productAreaGroupModel.getModel(ProductAreaGroupModel.PROPERTY_PRODUCT_AREA_GROUP)));
	comboBox.setName("ComboBoxProductAreaGroup");
	comboBox.setSelectedItem(userProductAreaGroup);
	return comboBox;
    }

    private static void setProductAreaListForNotAdministrator(final ApplicationUser applicationUser,
	    final List<ProductAreaGroup> aProductAreaGroupList, final ProductAreaGroup userProductAreaGroup) {
	...
	if (productAreaGroupList == null) {
	    productAreaGroupManager = productAreaGroupManager != null ? productAreaGroupManager : (ProductAreaGroupManager) ModelUtil
    ...
    private static StringBuilder addWordToLine(final int lineSize, final StringBuilder builder, final String[] words, final StringBuilder line,
	    final int i) {
	...
	return new CompareToBuilder().append(transport1.getTransportYear(), transport2.getTransportYear())
		.append(transport1.getTransportWeek(), transport2.getTransportWeek()).toComparison();
    }
