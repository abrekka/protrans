...
public class Transport extends BaseObject implements Comparable<Transport> {
     ...
     * @param loadTime
     */
    public Transport(Integer transportId, Integer transportYear, Integer transportWeek, Date loadingDate, Set<Order> orders, String transportName,
	    Date sent, Set<PostShipment> postShipments, Supplier supplier, Employee employee, String loadTime, final String aTrolley,
	    String aTransportComment) {
	super();
	...
	if (orders != null) {
	    for (Order order : orders) {
		if (order.getProductArea() != null && order.getProductArea().getProductAreaGroup().equals(productAreaGroup)
	...
	return new CompareToBuilder().append(transportYear, other.transportYear).append(transportWeek, other.transportWeek)
		.append(transportName, other.transportName).toComparison();
    }
