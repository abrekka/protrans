...
public class TakstolInfoV extends BaseObject{
	...
	public boolean equals(final Object other) {
		if (!(other instanceof TakstolInfoV))
			return false;
		TakstolInfoV castOther = (TakstolInfoV) other;
		return new EqualsBuilder().append(takstolInfoVPK,
				castOther.takstolInfoVPK).isEquals();
	}
	@Override
	public int hashCode() {
	...
	public String toString() {
		return new ToStringBuilder(this).append("takstolInfoVPK",
				takstolInfoVPK).append("leveringsadresse", leveringsadresse)
				.append("kundenr", kundenr).append("navn", navn).append(
						"hoydeOverHavet", hoydeOverHavet).append("beregnetfor",
						beregnetFor).append("snolast", snolast).append(
						"egenvekt", egenvekt)
				.append("utstikkType", utstikkType).append("kode", kode)
				.append("antall", antall).append("prodno", prodno).append(
						"beskrivelse", beskrivelse).append("takstoltype",
						takstoltype).append("virkesbredde", virkesbredde)
				.append("spennvidde", spennvidde).append("utstikkslengde",
						utstikkslengde).append("svilleklaring", svilleklaring)
				.append("rombreddeAStol", rombreddeAStol).append(
						"romhoydeAStol", romhoydeAStol).append("baeringGulv",
						baeringGulv).append("isolasjonshoyde", isolasjonshoyde)
				.append("loddkutt", loddkutt).append("nedstikk", nedstikk)
				.append("beregnetTid", beregnetTid).append("memofilnavn",
						memofilnavn).toString();
	}
