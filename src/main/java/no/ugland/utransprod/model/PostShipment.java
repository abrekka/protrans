 ...
 * @author atle.brekka
 */
public class PostShipment extends AbstractTransportable implements 
        TextRenderable, TransportListable, Packable {
     ...
     */
    public PostShipment(Integer postShipmentId, Order order,
            Date postShipmentReady) {
     ...
     * @param postShipmentReady
     */
    public PostShipment(Integer postShipmentId, Date postShipmentComplete,
            Order order, Transport transport, Deviation deviation, Date sent,
            Date postShipmentReady,String aStatus) {
        this.postShipmentId = postShipmentId;
     ...
     */
    public PostShipment(Integer postShipmentId, Order order,
            Transport transport, Deviation deviation, Date sent,
            Set<Colli> collies, Date postShipmentComplete,
            Date postShipmentReady, Set<OrderLine> orderLines,
            PostShipment postShipmentRef, Set<PostShipment> postShipmentRefs,
            String status, String packedBy, BigDecimal garageColliHeight,
            String cachedComment, OrderCost aOrderCost,final Integer aTakstolHeight) {
        this.postShipmentId = postShipmentId;
        this.order = order;
        this.transport = transport;
        this.deviation = deviation;
    ...
    public String toString() {
        ...
        if (customer != null) {
            return customer.toString() + " " + order.getPostalCode() + " "
                    + order.getPostOffice();
        }
        return "";
    }
    ...
    public String getTransportReportString() {
        ...
        stringBuffer.append(order.getPostalCode()).append(" ").append(
                order.getPostOffice());
        Customer customer = order.getCustomer();
        if (customer != null) {
        ...
        stringBuffer.append(" - ").append(order.getOrderNr()).append(",")
                .append(order.getConstructionTypeString()).append(",").append(
                        order.getInfo());
        ...
        if (deviation != null) {
            ...
            if (comments != null) {
                for (OrderComment comment : comments) {
                    ...
                    //if (Util.convertNumberToBoolean(comment.getForTransport())) {
        ...
        if (comments != null &&Hibernate.isInitialized(comments)) {
            for (OrderComment comment : comments) {
                if (comment.getDeviation() != null
                        && comment.getDeviation().getPostShipment() != null
                        && comment.getDeviation().getPostShipment()
                                .equals(this)) {
                    builder.append(comment.getComment()).append(";");
                }
            }
    ...
    private Boolean isAllOrderLinesDonePackage() {
        for (OrderLine orderLine : orderLines) {
            if (orderLine.hasTopLevelArticle() && orderLine.getColli() == null
        ...
        if (deviation != null) {
            ...
            if (comments != null) {
                for (OrderComment comment : comments) {
                    ...
                    //if (Util.convertNumberToBoolean(comment.getForPackage())) {
                        builder.append(comment.getComment()).append(" ");
                    }
                }
            }
    ...
    public final void setCustTr(final List<CustTr> aCustTrs) {
        custTrs = aCustTrs;
    ...
    public boolean isPaid() {
        if (custTr != null) {
            if (custTr.getRestAmount() != null
                    && custTr.getRestAmount().intValue() == 0) {
    ...
    public void cacheTakstolHeight() {
        ...
        setTakstolHeight(value.length()!=0&&StringUtils.isNumeric(value)?Integer.valueOf(value):Integer.valueOf(0));
    }
	...
	public boolean equals(final Object other) {
		...
		return new EqualsBuilder().append(order, castOther.order).append(
				transport, castOther.transport).append(sent, castOther.sent)
				.isEquals();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder().append(order).append(transport).append(
				sent).toHashCode();
	}
