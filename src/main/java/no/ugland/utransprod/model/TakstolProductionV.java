 ...
 * @author atle.brekka
 */
public class TakstolProductionV extends Production implements TextRenderable,
		Produceable, TakstolV {
	...
	public TakstolProductionV(Integer orderLineId, Integer customerNr,
			...
			Date producedDate, Date productionDate, String productionUnitName,
			Colli aColli) {
		super(orderLineId, customerNr, someCustomerDetails, orderNr, address,
				someInfo, constructionTypeName, articleName, someAttributeInfo,
				numberOfItems, loadingDate, someTransportDetails, comment,
				transportYear, transportWeek, loadTime, postShipmentId,
				productAreaGroupName, dateActionStarted, producedDate,
				productionDate, productionUnitName, aColli);
	}

	public boolean isRelatedArticlesComplete() {
		return relatedArticles == null || relatedArticles.size() == 0 ? true
				: checkIfIsComplete();
	}
	...
	public boolean isRelatedArticlesStarted() {
		return relatedArticles == null || relatedArticles.size() == 0 ? false
				: checkIfIsStarted();
	}
	...
	public Date getRelatedStartedDate() {
		return actionStarted != null ? actionStarted
				: getRelatedArticleStartedDate();
	}
