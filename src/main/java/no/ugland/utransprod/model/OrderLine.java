 ...
 * @author atle.brekka
 */
public class OrderLine extends BaseObject implements Comparable<OrderLine>,
		IArticle, TransportLetterObject, PackableListItem {
	 ...
	 * @param aDeviation
	 */
	public OrderLine(final Integer aOrderLineId, final Order aOrder,
			final ConstructionTypeArticle aConstructionTypeArticle,
			final ArticleType aArticleType,
			final Set<OrderLineAttribute> aOrderLineAttributes,
			final OrderLine aOrderLineRef, final Set<OrderLine> someOrderLines,
			final Integer aNumberOfItems, final Integer aDialogOrder,
			final Date producedDate, final String aArticlePath,
			final Colli aColli, final Integer doHasArticle,
			final String aAttributeInfo, final Integer thisIsDefault,
			final PostShipment aPostShipment,
			final ExternalOrderLine aExternalOrderLine,
			final Deviation aDeviation, final Date actionStartedDate,
			final ProductionUnit aProductionUnit, final Integer aOrdNo,
			final Integer aLnNo
	...
	private String getOrderLineAttributesAsString(OrderLine orderLine) {
		...
		if (attributes != null) {
			for (OrderLineAttribute attribute : attributes) {
				tmpBuffer.append(attribute.getAttributeName()).append(":")
						.append(attribute.getAttributeValue()).append(";");
			}
		}
	...
	public String getArticleName() {
		if (constructionTypeArticle != null) {
			return constructionTypeArticle.getArticleType()
					.getArticleTypeName();
	...
	public Boolean hasArticle() {
		...
		if (hasArticle != null) {
			...
			if (articleName.indexOf("$") != -1) {
				articleName = articleName.substring(articleName
						.lastIndexOf("$"));
			}
			...
			if (orderLineAttributes != null) {
				for (OrderLineAttribute orderLineAttribute : orderLineAttributes) {
					if (orderLineAttribute.getOrderLineAttributeName()
							.equalsIgnoreCase(attributeName)) {
						if (orderLineAttribute.getOrderLineAttributeValue()
								.equalsIgnoreCase("Nei")) {
	...
	public boolean isDefault() {
		...
		if (isDefault != null) {
			...
			for (OrderLineAttribute attribute : orderLineAttributes) {
				constructionAttribute = attribute
						.getConstructionTypeArticleAttribute();
				if (constructionAttribute == null) {
					returnValue = false;
					break;
				}
				if ("Egenordre".equalsIgnoreCase(attribute
						.getOrderLineAttributeName())
						&& !"Nei".equalsIgnoreCase(attribute
								.getOrderLineAttributeValue())) {
					returnValue = false;
					break;
				}

				if (constructionAttribute.isYesNo()) {
					if (constructionAttribute.getAttributeValueBool() != attribute
	 ...
	 * @return toppartikler
	 */
	public static List<OrderLine> getTopArticles(
		...
		if (orderLineAttributes != null) {
			for (OrderLineAttribute attribute : orderLineAttributes) {
				if (attribute.getAttributeValue() != null
						&& attribute.getAttributeValue().equalsIgnoreCase("Ja")
						&& attribute.isSpecialConcern()) {
	...
	public OrderLineAttribute getAttributeByName(String attributeName) {
		if (orderLineAttributes != null) {
			for (OrderLineAttribute attribute : orderLineAttributes) {
				if (attribute.getOrderLineAttributeName().equalsIgnoreCase(
						attributeName)) {
					return attribute;
				}
			}
	 ...
	 * @return ny instans
	 */
	public static OrderLine getInstance(final Order aOrder,
			final ArticleType aArticleType, final Deviation deviation,
			final PostShipment postShipment) {
		return new OrderLine(null, aOrder, null, aArticleType, null, null,
				null, null, null, null, null, null, null, null, null,
				postShipment, null, deviation, null, null, null, null);
	}
	 ...
	 */
	public static OrderLine getInstance(final Order aOrder,
			final ArticleType aArticleType, final OrderLine aOrderLineRef,
			final Deviation deviation) {
		return new OrderLine(null, aOrder, null, aArticleType, null,
				aOrderLineRef, null, null, null, null, null, null, null, null,
				null, null, null, deviation, null, null, null, null);
	}
	 ...
	 */
	public static OrderLine getInstance(final Order aOrder,
			final Integer aDialogOrder, final Deviation deviation) {
		return new OrderLine(null, aOrder, null, null, null, null, null, null,
				aDialogOrder, null, null, null, null, null, null, null, null,
	...
	public static OrderLine getInstance(final Order aOrder,
			final ConstructionTypeArticle aConstructionTypeArticle,
			final Integer aNumberOfItems, final Integer aDialogOrder,
			final Deviation deviation, final Integer prodNo, final Integer lnNo) {
		return new OrderLine(null, aOrder, aConstructionTypeArticle, null,
				null, null, null, aNumberOfItems, aDialogOrder, null, null,
				null, null, null, null, null, null, deviation, null, null,
				prodNo, lnNo);
	}
	...
	public static OrderLine getInstance(final Order aOrder,
			final ConstructionTypeArticle aConstructionTypeArticle,
			final OrderLine aOrderLineRef, final Integer aNumberOfItems,
			final Integer aDialogOrder, final Deviation deviation) {
		return new OrderLine(null, aOrder, aConstructionTypeArticle, null,
				null, aOrderLineRef, null, aNumberOfItems, aDialogOrder, null,
				null, null, null, null, null, null, null, deviation, null,
				null, null, null);
	}
	 ...
	 */
	public static OrderLine getInstance(final Order aOrder,
			final ArticleType aArticleType, final String aArticlePath,
			final Integer hasArticle) {
		return new OrderLine(null, aOrder, null, aArticleType, null, null,
				null, null, null, null, aArticlePath, null, hasArticle, null,
	...
	public boolean equals(final Object other) {
		...
		boolean test = new EqualsBuilder().append(orderLineId,
				castOther.orderLineId).append(order, castOther.order).append(
				constructionTypeArticle, castOther.constructionTypeArticle)
				.append(articleType, castOther.articleType).append(
						orderLineRef, castOther.orderLineRef).append(
						numberOfItems, castOther.numberOfItems).append(
						produced, castOther.produced).append(colli,
						castOther.colli).append(lnNo, castOther.lnNo)
				.isEquals();
		return test;
	}
	...
	@Override
	public int hashCode() {
		return new HashCodeBuilder().append(orderLineId).append(order).append(
				constructionTypeArticle).append(articleType).append(
				orderLineRef).append(numberOfItems).append(produced).append(
				colli).append(lnNo).toHashCode();
	}
...
//			 }
	...
	private String getDetailsString(StringBuilder stringBuilder) {
		...
		if (stringBuilder != null) {
			if (!getProductAreaGroupName().equalsIgnoreCase("Garasje")) {
				details = StringUtils.replace(stringBuilder.toString(),
						"Egenordre:Annet", "");
			} else {
	...
	private String generateTaksteinDetails() {
		...
		if (orderLineAttributes != null) {
			...
			for (OrderLineAttribute attribute : orderLineAttributes) {
				...
				} else if ("Taksteintype".equalsIgnoreCase(attribute
						.getAttributeName())) {
					detailsBuilder.append(attribute.getAttributeName()).append(
	 ...
	 */
	private boolean hasConditions(String[] conditions,
			List<OrderLineAttribute> attributes) {
		if (conditions != null) {
			for (int i = 0; i < conditions.length; i++) {
				String condition = conditions[i];
				String articleName = condition.substring(0, condition
						.indexOf("#"));
				String attributeName = condition.substring(condition
						.indexOf("#") + 1, condition.indexOf("@"));
				String attributeValue = condition.substring(condition
						.indexOf("@") + 1);

				for (OrderLineAttribute attribute : attributes) {
					if (attribute.getAttributeName().equalsIgnoreCase(
							attributeName)
							&& attribute.getArticleName().equalsIgnoreCase(
									articleName)) {
						if (attribute.getAttributeValue() == null
			...
			for (OrderLineAttribute attribute : attributes) {
				if (attribute.getAttributeFormula() != null) {
					...
					if (allFormula.indexOf(";") != -1) {
						String conditionString = allFormula.substring(0,
								allFormula.lastIndexOf(";"));
					...
					if (runFormula && allFormula.indexOf("[") != -1) {
						String formula = allFormula.substring(allFormula
								.indexOf("[") + 1, allFormula.length() - 1);
						value = Calculator.calculate(formula, attributes);
						if (value != null) {
							integer = value.intValue();

							attribute.setAttributeValueBool(Util
									.convertNumberToBoolean(integer));
	...
	public boolean generateDetailsWithoutNoAttributes(StringBuilder builder,
			String colliName) {
	...
	public String getProductAreaGroupName() {
		return order != null ? order.getProductArea().getProductAreaGroup()
				.getProductAreaGroupName() : "";
	}

	public final void setOrderLineAttributeValue(final String attributeName,
	...
	public boolean belongTo(Transportable transportable) {
		return postShipment != null ? transportable.equals(postShipment)
				: transportable.equals(order);
	}
	...
	public void setAttributeValuesFromOrderLine(
			Set<OrderLineAttribute> orderLineAttributes) {
		for (OrderLineAttribute att : orderLineAttributes) {
			if (att.getAttributeValue() != null
					&& att.getAttributeValue().length() != 0) {
				setAttributeValue(att);
			}
		}
	}

	public final void setAttributeValue(
			final OrderLineAttribute orderLineAttribute) {
		if (orderLineAttributes != null) {
			boolean found = false;
			for (OrderLineAttribute attribute : orderLineAttributes) {
				if (attribute.getAttributeName().equalsIgnoreCase(
						orderLineAttribute.getAttributeName())) {
					attribute.setAttributeValue(orderLineAttribute
							.getAttributeValue());
					found = true;
					break;
				}
			...
			found = !found
					&& orderLineAttribute.getAttributeValue().length() != 0 ? orderLineAttributes
					.add(createNewAttribute(orderLineAttribute))
					: false;

		}
	}

	private OrderLineAttribute createNewAttribute(
			final OrderLineAttribute orderLineAttribute) {
		OrderLineAttribute newAttribute = new OrderLineAttribute();
		newAttribute.setArticleTypeAttribute(orderLineAttribute
				.getArticleTypeAttribute());
		newAttribute.setAttributeValue(orderLineAttribute.getAttributeValue());
		newAttribute.setOrderLine(this);
		newAttribute.setOrderLineAttributeName(orderLineAttribute
				.getOrderLineAttributeName());
		return newAttribute;
	}
	...
	public int compareTo(final OrderLine other) {
		return new CompareToBuilder()
				.append(numberOfItems, other.numberOfItems).toComparison();
	}

	public static class OrderLineNumberComparator implements
			Comparator<OrderLine> {

		public int compare(OrderLine orderLine1, OrderLine orderLine2) {
			return new CompareToBuilder().append(orderLine2.getNumberOfItems(),
					orderLine1.getNumberOfItems()).toComparison();
		}
	...
	public String getLoadTime() {
		return order.getTransport() != null ? order.getTransport()
				.getLoadTime() : null;
	}

	public Integer getTransportWeek() {
		return order.getTransport() != null ? order.getTransport()
				.getTransportWeek() : null;
	}

	public Integer getTransportYear() {
		return order.getTransport() != null ? order.getTransport()
				.getTransportYear() : null;
	}
	...
	public String getProductionUnitName() {
		return productionUnit != null ? productionUnit.getProductionUnitName()
				: null;
	}
...
//					takstolDetails += " ";
//				}
//				return StringUtils.isNotBlank(takstolDetails) ? takstolDetails
//						: "";
				return "";
			}
...
//					taksteinDetails += " ";
//				}
//				return StringUtils.isNotBlank(taksteinDetails) ? taksteinDetails
//						: null;
			}
