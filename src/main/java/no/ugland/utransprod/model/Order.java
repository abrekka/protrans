 ...
 * @author atle.brekka
 */
public class Order extends AbstractTransportable implements TextRenderable, Articleable, IAssembly, TransportListable, Packable {
     ...
     * @param productArea
     */
    public Order(String orderNr, String deliveryAddress, String postalCode, String postOffice, Customer customer, ConstructionType constructionType,
	    Date orderDate, ProductArea productArea) {
	this(null, orderNr, deliveryAddress, postalCode, postOffice,

	customer, constructionType, null, null, orderDate, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null,
		null, null, null, null, null, null, null, null, null, null, null, productArea, null, null, null, null, null);
    }

    public Order(Integer orderId, String orderNr, String deliveryAddress, String postalCode, String postOffice, Customer customer,
	    ConstructionType constructionType, Transport transport, Integer doAssembly, Date orderDate, Set<OrderLine> orderLines,
	    Set<OrderCost> orderCosts, Date invoiceDate, Date sent, Assembly assembly, String info, Set<Colli> collies, String status,
	    Integer colliesDone, Date orderReady, Date orderComplete, Date packageStarted, Date agreementDate, String specialConcern,
	    Set<PostShipment> postShipments, Integer deliveryWeek, String telephoneNr, Date packlistReady, Set<ExternalOrder> externalOrders,
	    String packedBy, String salesman, Date paidDate, Set<OrderComment> orderComments, String cachedComment, Integer garageColliHeight,
	    Integer hasMissingCollies, ProductArea productArea, final Set<Deviation> someDeviations, final Date aRegistrationDate,
	...
	String returnString = "";
	if (customer != null) {
	    returnString = customer.toString() + " - " + orderNr + " - " + postalCode + " " + (postOffice != null ? postOffice : "") + " "
	...
	if (customer != null) {
	    return String.format("%1$-30.30s", customer.toString()) + " - " + orderNr + " - " + postalCode + " " + postOffice + " "
		    + constructionType.toString();
	}
	return "";
    }
	...
	stringBuffer.append(" - ").append(orderNr).append("\n").append(postalCode).append(" ").append(postOffice).append(",")
		.append(getConstructionTypeString()).append(",").append(info);
	...
	if (orderLines != null) {
	    for (OrderLine orderLine : orderLines) {
		if (orderLine.hasTopLevelArticle() && orderLine.getColli() == null && orderLine.hasArticle() && orderLine.getPostShipment() == null) {
	...
	if (orderLines != null) {
		...
		if (line.hasTopLevelArticle() && line.hasArticle() && (line.getColli() == null || line.getColli().getSent() == null)) {
	...
	if (orderLines != null) {
		...
		if (line.hasTopLevelArticle() && line.hasArticle()
			&& (line.getColli() == null || (line.getPostShipment() != null) && line.getPostShipment().getSent() == null)) {
		    linesNotSent.add(line);
		}
	    }
	}
	...
	stringBuffer.append(" - ").append(orderNr).append(",").append(getConstructionTypeString()).append(",").append(info);
	...
	if (orderCosts != null) {
		...
		if (cost.getCostUnit() != null) {
		    ...
		    if (orderCostTypeName.equalsIgnoreCase(costTypeName) && orderCostUnitName.equalsIgnoreCase(costUnitName)) {
	...
	if (!lastProcentDoneFetched) {
		...
		ProcentDone procentDone = (ProcentDone) CollectionUtils.get(procentDones.iterator(), procentDones.size() - 1);
