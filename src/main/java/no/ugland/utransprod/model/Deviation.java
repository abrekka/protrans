...
public class Deviation extends AbstractTransportable implements Articleable, IAssembly {
	...
	public Deviation(final Integer aDeviationId, final Date aRegistrationDate, final String aUserName,
			final String aDescription, final JobFunction aOwnFunction, final JobFunction aDeviationFunction,
			final Integer aCustomerNr, final String aOrderNr, final DeviationStatus aDeviationStatus,
			final Date aDateClosed, final String aProductName, final FunctionCategory aFunctionCategory,
			final PreventiveAction aPreventiveAction, final String aCustomerName, final PostShipment aPostShipment,
			final Set<OrderComment> aOrderComments, final ProductArea aProductArea, final Order aOrder,
			final Set<OrderLine> someOrderLines, final Set<OrderCost> someOrderCosts, final Integer shouldDoAssembly,
			final Assembly aAssembly, final String aResponsible, final Integer isChecked, final String isInitiatedBy,
			final String isLastChanged, final String aProjectNr, final Date aProcedureCheck) {
		super();
	...
	public final String toString() {
		return new ToStringBuilder(this, ToStringStyle.SIMPLE_STYLE).append("deviationId", deviationId)
				.append("registrationDate", registrationDate).append("userName", userName)
				.append("description", description).append("ownFunction", ownFunction)
				.append("deviationFunction", deviationFunction).append("customerNr", customerNr)
				.append("customerName", customerName).append("orderNr", orderNr)
				.append("deviationStatus", deviationStatus).append("dateClosed", dateClosed)
				.append("productName", productName).append("functionCategory", functionCategory)
				.append("preventiveAction", preventiveAction).append("postShipment", postShipment)
				.append("productArea", productArea).append("order", order).toString();
	}
	...
	public final boolean equals(final Object other) {
		...
		return new EqualsBuilder().append(deviationId, castOther.deviationId)
				.append(registrationDate, castOther.registrationDate).append(userName, castOther.userName)
				.append(description, castOther.description).append(ownFunction, castOther.ownFunction)
				.append(deviationFunction, castOther.deviationFunction).append(customerNr, castOther.customerNr)
				.append(customerName, castOther.customerName).append(orderNr, castOther.orderNr)
				.append(deviationStatus, castOther.deviationStatus).append(dateClosed, castOther.dateClosed)
				.append(productName, castOther.productName).append(functionCategory, castOther.functionCategory)
				.append(preventiveAction, castOther.preventiveAction).append(postShipment, castOther.postShipment)
				.append(productArea, castOther.productArea).append(order, castOther.order)
	...
	public final int hashCode() {
		return new HashCodeBuilder().append(deviationId).append(registrationDate).append(userName).append(description)
				.append(ownFunction).append(deviationFunction).append(customerNr).append(customerName).append(orderNr)
				.append(deviationStatus).append(dateClosed).append(productName).append(functionCategory)
				.append(preventiveAction).append(postShipment).append(productArea).append(order).append(doAssembly)
				.append(assembly).toHashCode();
	}
