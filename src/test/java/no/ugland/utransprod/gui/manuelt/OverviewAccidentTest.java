...
public class OverviewAccidentTest {
	...
	when(accidentManager.findByObject((Accident) anyObject())).thenReturn(accidentList);

	final AccidentViewHandler accidentViewHandler = new AccidentViewHandler(login, managerRepository);
	final OverviewView<Accident, AccidentModel> view = new OverviewView<Accident, AccidentModel>(accidentViewHandler);
	...
	DialogFixture editDialog = WindowFinder.findDialog("EditAccidentView").withTimeout(40000).using(dialogFixture.robot);
	...
	editDialog = WindowFinder.findDialog("EditAccidentView").withTimeout(20000).using(dialogFixture.robot);
    }
	...
	WindowFinder.findDialog("EditAccidentView").withTimeout(60000).using(dialogFixture.robot);
