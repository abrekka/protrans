...
public class AttachmentViewTest {
	...
	ApplicationParamManager applicationParamManager = (ApplicationParamManager) ModelUtil.getBean(ApplicationParamManager.MANAGER_NAME);
	ApplicationParamUtil.setApplicationParamManger(applicationParamManager);

	AttachmentViewHandler attachmentViewHandler = new AttachmentViewHandler("testdir");
	...
	assertEquals(lenght + 1, dialogFixture.list("ListAttachments").contents().length);
    }
	...
	addFile("Budsjett_import_villa.xls");
	dialogFixture.list("ListAttachments").selectItem("Budsjett_import_villa.xls");
