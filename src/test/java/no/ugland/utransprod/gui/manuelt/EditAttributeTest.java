...
public class EditAttributeTest {
	...
	when(managerRepository.getAttributeChoiceManager()).thenReturn(attributeChoiceManager);
	...
	userTypeAccesses.add(new UserTypeAccess(null, 1, userType, new WindowAccess(null, "Attributter", null)));
	...
	final EditAttributeView editAttributeView = new EditAttributeView(attributeViewHandler, new Attribute(), false);
	...
	JOptionPaneFinder.findOptionPane().using(dialogFixture.robot).buttonWithText("Nei").click();
	dialogFixture.button("AddAttributeChoice").click();

	DialogFixture choiceDialog = WindowFinder.findDialog("EditAttributeChoiceView").using(dialogFixture.robot);
	...
	assertEquals("testvalg", dialogFixture.table("TableAttributeChoice").target.getValueAt(0, 0));
