...
public class ConstructionTypeViewTest {
	...
	ConstructionTypeViewHandler viewHandler = new ConstructionTypeViewHandler(login, managerRepository, false, false);

	final OverviewView<ConstructionType, ConstructionTypeModel> view = new OverviewView<ConstructionType, ConstructionTypeModel>(viewHandler);
	...
	WindowFinder.findDialog("EditConstructionTypeView").using(dialogFixture.robot);
	...
	DialogFixture editDialog = WindowFinder.findDialog("EditConstructionTypeView").using(dialogFixture.robot);

	editDialog.textBox("TextFieldName").enterText("test");
	editDialog.comboBox("ComboBoxProductArea").selectItem("Service");

	editDialog.button("ButtonAddArticle").click();

	DialogFixture editArticleDialog = WindowFinder.findDialog("ArticleTypeView").using(dialogFixture.robot);
	editArticleDialog.list("ListArticles").target.setSelectedIndex(127);
	assertEquals(true, editArticleDialog.list("ListArticles").target.getSelectedIndex() != -1);
