...
public class OrderOverviewViewEditTest {
	...
	assertEquals(true, dialogFixture.robot.finder().findByName("AssemblyPanel").isVisible());
	...
	DialogFixture garasjeDialog = new DialogFixture(dialogFixture.robot, (JDialog) dialogFixture.robot.finder().findByName("UpdateOrderLineView"));

	garasjeDialog.button("ButtonOk").click();

	JDateChooser orderDate = (JDateChooser) dialogFixture.robot.finder().findByName("OrderDate");

	orderDate.setDate(Calendar.getInstance().getTime());

	JDateChooser productionDate = (JDateChooser) dialogFixture.robot.finder().findByName("DateChooserProduction");

	productionDate.setDate(Calendar.getInstance().getTime());

	JButtonFixture addCostButton = editDialog.button("ButtonAddCost");
	addCostButton.click();
	DialogFixture kostnadDialog = new DialogFixture(dialogFixture.robot, (JDialog) dialogFixture.robot.finder().findByName("OrderCostView"));
	...
	addCostButton.click();
	kostnadDialog = new DialogFixture(dialogFixture.robot, (JDialog) dialogFixture.robot.finder().findByName("OrderCostView"));
	...
	DialogFixture commentDialog = WindowFinder.findDialog("EditCommentView").withTimeout(60000).using(dialogFixture.robot);
	...
	orderManager.lazyLoadOrder(order, new LazyLoadOrderEnum[] { LazyLoadOrderEnum.COMMENTS });

	Set<OrderComment> comments = order.getOrderComments();
	assertNotNull(comments);
	assertEquals(1, comments.size());
	OrderComment orderComment = comments.iterator().next();
	Set<OrderCommentCommentType> commentTypes = orderComment.getOrderCommentCommentTypes();
	...
	for (OrderCommentCommentType type : commentTypeList) {
	    count++;
	    if (count == 1) {
		assertEquals("Transport", type.getCommentType().getCommentTypeName());
	    } else if (count == 2) {
		assertEquals("Pakking", type.getCommentType().getCommentTypeName());
	    } else if (count == 3) {
		assertEquals("Ordre", type.getCommentType().getCommentTypeName());
	    }
	}
