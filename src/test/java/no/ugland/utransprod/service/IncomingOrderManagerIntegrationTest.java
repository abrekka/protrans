...
public class IncomingOrderManagerIntegrationTest {
	...
	incomingOrderManager = (IncomingOrderManager) ModelUtil.getBean("incomingOrderManager");
	final OrdlnManager ordlnManager = (OrdlnManager) ModelUtil.getBean(OrdlnManager.MANAGER_NAME);
	...
	for (OrderCost cost : costs) {
	    if (cost.getCostType().getCostTypeName().equalsIgnoreCase("Egenproduksjon")
		    && cost.getCostUnit().getCostUnitName().equalsIgnoreCase("Kunde")) {
		egenproduksjonKunde = true;
		assertEquals(BigDecimal.valueOf(200000), cost.getCostAmount());
	    }
	    if (cost.getCostType().getCostTypeName().equalsIgnoreCase("Egenproduksjon")
		    && cost.getCostUnit().getCostUnitName().equalsIgnoreCase("Intern")) {
	    ...
	    if (cost.getCostType().getCostTypeName().equalsIgnoreCase("Montering")) {
		assertEquals("Kunde", cost.getCostUnit().getCostUnitName());
		assertEquals(BigDecimal.valueOf(20000), cost.getCostAmount().setScale(0));
		montering = true;
	    }
	    if (cost.getCostType().getCostTypeName().equalsIgnoreCase("Jalinjer")) {
		assertEquals("Kunde", cost.getCostUnit().getCostUnitName());
		assertEquals(BigDecimal.valueOf(5000), cost.getCostAmount());
		jaLinjer = true;
	    }
	    if (cost.getCostType().getCostTypeName().equalsIgnoreCase("Kraning")) {
		assertEquals("Kunde", cost.getCostUnit().getCostUnitName());
		assertEquals(BigDecimal.valueOf(2000), cost.getCostAmount().setScale(0));
		kraning = true;
	    }
	    if (cost.getCostType().getCostTypeName().equalsIgnoreCase("Avfall")) {
		assertEquals("Kunde", cost.getCostUnit().getCostUnitName());
		assertEquals(BigDecimal.valueOf(500), cost.getCostAmount().setScale(0));
		avfall = true;
	    }
	}
    ...
    @Test
    public void skalImportereKledningForGavl() {
	List<Order> orders = incomingOrderManager.findByOrderNr("19");
	assertEquals(1, orders.size());
	...
	Assertions.assertThat(gavl).isNotEqualTo(OrderLine.UNKNOWN);
	boolean funnetKledning = false;
	for (OrderLine orderLine : gavl.getOrderLines()) {
	    if (orderLine.getArticlePath().equalsIgnoreCase("Gavl$Kledning")) {
		funnetKledning = true;
	    }
	}
	Assertions.assertThat(funnetKledning).isTrue();
    }
}
